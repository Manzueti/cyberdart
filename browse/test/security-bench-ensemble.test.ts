 ...
 * Seeded by: CYBERDART_BENCH_ENSEMBLE=1 bun test security-bench-ensemble-live.test.ts
 *
 * Fail-closed rule:
...
describe('BrowseSafe-Bench ensemble gate (fixture replay)', () => {
  ...
  test('fixture integrity: present + matches current code, or skip allowed', () => {
    ...
    if (fixtureState === 'missing' && !securityChanged) {
      ...
      console.log('[security-bench-ensemble] fixture missing, no security-layer files changed — skipping. Run `CYBERDART_BENCH_ENSEMBLE=1 bun test security-bench-ensemble-live.test.ts` to seed.');
      return;
    }
    ...
    if (fixtureState === 'missing') {
      throw new Error(
        'Fixture browse/test/fixtures/security-bench-haiku-responses.json is missing AND security-layer files were modified in this branch. Run `CYBERDART_BENCH_ENSEMBLE=1 bun test browse/test/security-bench-ensemble-live.test.ts` to regenerate the fixture before committing.',
      );
    }
    throw new Error(
      'Fixture schema hash mismatch (model or thresholds changed) AND security-layer files were modified in this branch. Regenerate via `CYBERDART_BENCH_ENSEMBLE=1 bun test browse/test/security-bench-ensemble-live.test.ts` to capture fresh Haiku responses for the new configuration.',
    );
  });
