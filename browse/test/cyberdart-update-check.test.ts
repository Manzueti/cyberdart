/**
 * Tests for bin/cyberdart-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * CYBERDART_DIR / CYBERDART_STATE_DIR / CYBERDART_REMOTE_URL env overrides
 * for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'cyberdart-update-check');
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  const result = Bun.spawnSync(['bash', SCRIPT, ...args], {
    env: {
      ...process.env,
      CYBERDART_DIR: cyberdartDir,
      CYBERDART_STATE_DIR: stateDir,
      CYBERDART_REMOTE_URL: `file://${join(cyberdartDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  cyberdartDir = mkdtempSync(join(tmpdir(), 'cyberdart-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'cyberdart-state-test-'));
  // Link real cyberdart-config so update_check config check works
  const binDir = join(cyberdartDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'cyberdart-config'), join(binDir, 'cyberdart-config'));
});

afterEach(() => {
  rmSync(cyberdartDir, { recursive: true, force: true });
...
describe('cyberdart-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(cyberdartDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(cyberdartDir, 'VERSION'), '0.4.0\n');
  ...
  test('just-upgraded marker does not mask newer remote version', () => {
    writeFileSync(join(cyberdartDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(cyberdartDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('just-upgraded with no further updates writes UP_TO_DATE cache', () => {
    writeFileSync(join(cyberdartDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(cyberdartDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(cyberdartDir, 'VERSION'), '0.3.3\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(cyberdartDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(cyberdartDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(cyberdartDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(cyberdartDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      CYBERDART_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ CYBERDART_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(cyberdartDir, 'VERSION'), version + '\n');
