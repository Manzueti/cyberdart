...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/cyberdart-cli-');
    const execPath = path.join(root, '.claude/skills/cyberdart/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/cyberdart/browse/src/server.ts');
