/**
 * Tests for bin/cyberdart-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * CYBERDART_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'cyberdart-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'cyberdart-config-test-'));
});
...
describe('cyberdart-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { CYBERDART_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
  ...
  test('first set writes annotated header with docs', () => {
    ...
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    expect(content).toContain('# cyberdart configuration');
  ...
  test('header written only once, not duplicated on second set', () => {
    ...
    run(['set', 'baz', 'qux']);
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    const headerCount = (content.match(/# cyberdart configuration/g) || []).length;
  ...
  test('existing config file is not overwritten with header', () => {
    ...
    expect(content).not.toContain('# cyberdart configuration');
  });
