...
beforeEach(() => {
  ...
  tiers = {
    project: path.join(tmpRoot, 'project', '.cyberdart', 'browser-skills'),
    global: path.join(tmpRoot, 'home', '.cyberdart', 'browser-skills'),
    bundled: path.join(tmpRoot, 'cyberdart-install', 'browser-skills'),
  };
...
describe('buildSpawnEnv', () => {
  ...
  beforeEach(() => {
    ...
    process.env.CYBERDART_TOKEN = 'root-token';
  ...
  it('untrusted: drops $HOME and secrets', () => {
    ...
    expect(env.CYBERDART_TOKEN).toBeUndefined();
  });
  ...
  it('untrusted: injects CYBERDART_PORT + CYBERDART_SKILL_TOKEN', () => {
    const env = buildSpawnEnv({ trusted: false, port: 1234, skillToken: 'tok-xyz' });
    expect(env.CYBERDART_PORT).toBe('1234');
    expect(env.CYBERDART_SKILL_TOKEN).toBe('tok-xyz');
  });
  ...
  it('trusted: still strips CYBERDART_TOKEN (defense in depth)', () => {
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'tok' });
    expect(env.CYBERDART_TOKEN).toBeUndefined();
  });
  ...
  it('CYBERDART_PORT/CYBERDART_SKILL_TOKEN can never be overridden by parent env', () => {
    process.env.CYBERDART_PORT = '99999'; // attacker-set
    process.env.CYBERDART_SKILL_TOKEN = 'attacker-tok';
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'real-tok' });
    expect(env.CYBERDART_PORT).toBe('1234');
    expect(env.CYBERDART_SKILL_TOKEN).toBe('real-tok');
  });
});
...
describe.skipIf(SKIP_SPAWN)('spawnSkill: lifecycle', () => {
  ...
  it('untrusted spawn: CYBERDART_SKILL_TOKEN visible, root env scrubbed', async () => {
    const dir = makeSkillDir(tiers.bundled, 'env-probe',
      'name: env-probe\nhost: x.com',  // trusted defaults to false
      `console.log(JSON.stringify({
        port: process.env.CYBERDART_PORT,
        token: process.env.CYBERDART_SKILL_TOKEN,
        home: process.env.HOME ?? null,
        gh: process.env.GITHUB_TOKEN ?? null,
        cyberdart: process.env.CYBERDART_TOKEN ?? null,
      }));`,
    );
    const origEnv = { ...process.env };
    process.env.GITHUB_TOKEN = 'gh-secret';
    process.env.CYBERDART_TOKEN = 'root';
    try {
      ...
      expect(parsed.cyberdart).toBeNull();
    } finally {
      process.env = origEnv;
    }
  });
