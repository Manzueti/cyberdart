...
const TMP_HOME = path.join(os.tmpdir(), `cyberdart-telemetry-test-${process.pid}-${Date.now()}`);
const TELEMETRY_FILE = path.join(TMP_HOME, 'analytics', 'browse-telemetry.jsonl');

// Use CYBERDART_HOME env to redirect telemetry writes (read each call,
// not cached at module-load).
process.env.CYBERDART_HOME = TMP_HOME;
process.env.CYBERDART_TELEMETRY_OFF = '0';
...
describe('telemetry: signals fire to ~/.cyberdart/analytics/browse-telemetry.jsonl', () => {
  ...
  it('CYBERDART_TELEMETRY_OFF=1 silences all events', async () => {
    process.env.CYBERDART_TELEMETRY_OFF = '1';
    const { logTelemetry, _resetTelemetryCache } = await import('../src/telemetry');
    _resetTelemetryCache();
