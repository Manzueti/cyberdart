...
describe('cleanup heuristics (write-commands.ts)', () => {
  ...
  test('sticky cleanup skips cyberdart control indicator', () => {
    expect(wcSrc).toContain("cyberdart-ctrl");
  });
...
describe('welcome page', () => {
  ...
  test('welcome page has extension-ready listener to hide prompt', () => {
    expect(welcomeSrc).toContain('cyberdart-extension-ready');
...
describe('sidebar arrow hint hide flow (4-step signal chain)', () => {
  ...
  //   3. content.js receives 'sidebarOpened' → dispatches 'cyberdart-extension-ready'
  //   4. welcome.html listens for 'cyberdart-extension-ready' → hides arrow
  //
  const contentSrc = fs.readFileSync(path.join(ROOT, '..', 'extension', 'content.js'), 'utf-8');
  const bgSrc = fs.readFileSync(path.join(ROOT, '..', 'extension', 'background.js'), 'utf-8');
  ...
  // Step 3: content.js fires cyberdart-extension-ready ONLY on sidebarOpened
  test('step 3: content.js dispatches extension-ready on sidebarOpened message', () => {
    ...
    expect(contentSrc).toContain("new CustomEvent('cyberdart-extension-ready')");
  });
  ...
  test('step 3: content.js does NOT auto-fire extension-ready on load', () => {
    ...
    expect(beforeListener).not.toContain("dispatchEvent(new CustomEvent('cyberdart-extension-ready'))");
  });

  // Step 4: welcome page hides arrow on cyberdart-extension-ready
  test('step 4: welcome page hides arrow on cyberdart-extension-ready event', () => {
    expect(welcomeSrc).toContain("'cyberdart-extension-ready'");
    expect(welcomeSrc).toContain("classList.add('hidden')");
  });

  test('step 4: welcome page does NOT auto-hide via status pill polling', () => {
    // The old fallback (checkPill/cyberdart-status-pill) would hide the arrow
    // as soon as the content script injected the pill, even without sidebar open.
    expect(welcomeSrc).not.toContain('checkPill');
    expect(welcomeSrc).not.toContain('cyberdart-status-pill');
  });
});
...
describe('BROWSE_NO_AUTOSTART (sidebar headless prevention)', () => {
  ...
  test('cli.ts shows actionable error message when BROWSE_NO_AUTOSTART blocks', () => {
    expect(cliSrc).toContain('/open-cyberdart-browser');
