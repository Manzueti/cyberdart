...
describe('browse-client', () => {
  ...
  beforeEach(async () => {
    ...
    for (const k of ['CYBERDART_PORT', 'CYBERDART_SKILL_TOKEN', 'BROWSE_STATE_FILE', 'BROWSE_TAB']) {
  ...
  describe('resolveBrowseAuth', () => {
    it('uses CYBERDART_PORT + CYBERDART_SKILL_TOKEN env when present', () => {
      process.env.CYBERDART_PORT = String(server.port);
      process.env.CYBERDART_SKILL_TOKEN = 'scoped-token';
    ...
    it('rejects CYBERDART_PORT env values with trailing characters', () => {
      process.env.CYBERDART_PORT = `${server.port}abc`;
      process.env.CYBERDART_SKILL_TOKEN = 'scoped-token';
