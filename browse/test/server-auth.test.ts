...
describe('Server auth security', () => {
  ...
  test('/activity/history requires authentication', () => {
    const historyBlock = sliceBetween(SERVER_SRC, "url.pathname === '/activity/history'", "url.pathname === '/batch'");
  ...
  test('pair-agent disables parent PID monitoring via BROWSE_PARENT_PID=0', () => {
    ...
    const connectBlock = sliceBetween(CLI_SRC, 'Launching headed Chromium', 'Terminal agent started');
    const serverEnvMatch = connectBlock.match(/const serverEnv: Record<string, string> = \{([\s\S]+?)\n      \};/);
    expect(serverEnvMatch).not.toBeNull();
    expect(serverEnvMatch![1]).toContain("BROWSE_PARENT_PID: '0'");
  });
