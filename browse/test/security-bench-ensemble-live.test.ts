 ...
 *
 * Opt-in: only runs when `CYBERDART_BENCH_ENSEMBLE=1` is set. Otherwise the
 ...
 *   - ~/.cyberdart-dev/evals/security-bench-ensemble-{timestamp}.json (per-run
 *     audit record with TP/FN/FP/TN + Wilson 95% CIs + knob state)
 *
 * Stop-loss iterations: when detection or FP fails the gate, set
 * `CYBERDART_BENCH_STOP_LOSS_ITER=N` where N in {1,2,3}. The bench writes to
 ...
 *
 * Run: CYBERDART_BENCH_ENSEMBLE=1 bun test browse/test/security-bench-ensemble-live.test.ts
 */
...
const RUN = process.env.CYBERDART_BENCH_ENSEMBLE === '1';
const STOP_LOSS_ITER = process.env.CYBERDART_BENCH_STOP_LOSS_ITER
  ? Number(process.env.CYBERDART_BENCH_STOP_LOSS_ITER)
  : 0;
...
const CASES_LIMIT = process.env.CYBERDART_BENCH_ENSEMBLE_CASES
  ? Math.max(10, Number(process.env.CYBERDART_BENCH_ENSEMBLE_CASES))
  : 0;

const REPO_ROOT = path.resolve(__dirname, '..', '..');
const FIXTURE_PATH = path.resolve(__dirname, 'fixtures', 'security-bench-haiku-responses.json');
const EVALS_DIR = path.join(os.homedir(), '.cyberdart-dev', 'evals');

const CACHE_DIR = path.join(os.homedir(), '.cyberdart', 'cache', 'browsesafe-bench-smoke');
...
describe('BrowseSafe-Bench ensemble LIVE (opt-in, real Haiku)', () => {
  ...
  test.skipIf(!RUN || !ML_AVAILABLE)('runs full ensemble on smoke, writes fixture, records evals', async () => {
    ...
    // CYBERDART_BENCH_ENSEMBLE_CONCURRENCY if rate limits hit.
    const CONCURRENCY = Number(process.env.CYBERDART_BENCH_ENSEMBLE_CONCURRENCY ?? 8);
