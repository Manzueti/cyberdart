...
describe('pair-agent flow end-to-end (HTTP only, no ngrok)', () => {
  ...
  test('POST /sse-session with root Bearer returns a Set-Cookie for cyberdart_sse', async () => {
    ...
    expect(setCookie!).toContain('cyberdart_sse=');
  ...
  test('/welcome regex gate: safe slug resolves; dangerous slug does not path-traverse', async () => {
    // The regex gate lives in server.ts — we can't easily flip CYBERDART_SLUG
