...
describe('logAttempt', () => {
  test('writes attempts.jsonl with correct shape', () => {
    ...
    const logPath = path.join(os.homedir(), '.cyberdart', 'security', 'attempts.jsonl');
...
describe('resolveBashBinary', () => {
  ...
  test('honors CYBERDART_BASH_BIN absolute-path override', () => {
    ...
    const out = resolveBashBinary({ CYBERDART_BASH_BIN: fake, PATH: '' });
  ...
  test('BASH_BIN works as a fallback when CYBERDART_BASH_BIN is unset', () => {
    const fake = process.platform === 'win32' ? 'C:\\opt\\bash.exe' : '/opt/custom/bash';
...
describe('buildTelemetrySpawnCommand', () => {
  const bin = '/home/user/.claude/skills/cyberdart/bin/cyberdart-telemetry-log';
  ...
  test('on win32 with bash resolvable, wraps the call in bash with the script as first arg', () => {
    ...
    const out = buildTelemetrySpawnCommand(bin, args, { CYBERDART_BASH_BIN: fakeBash, PATH: '' });
    expect(out).not.toBeNull();
