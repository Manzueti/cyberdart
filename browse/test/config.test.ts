...
describe('config', () => {
  ...
  describe('resolveConfig', () => {
    test('uses git root by default', () => {
      ...
      expect(config.stateDir).toBe(path.join(gitRoot!, '.cyberdart'));
      expect(config.stateFile).toBe(path.join(gitRoot!, '.cyberdart', 'browse.json'));
    });

    test('derives paths from BROWSE_STATE_FILE when set', () => {
      const stateFile = '/tmp/test-config/.cyberdart/browse.json';
      const config = resolveConfig({ BROWSE_STATE_FILE: stateFile });
      expect(config.stateFile).toBe(stateFile);
      expect(config.stateDir).toBe('/tmp/test-config/.cyberdart');
  ...
  describe('ensureStateDir', () => {
    test('creates directory if it does not exist', () => {
      ...
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.cyberdart', 'browse.json') });
    ...
    test('is a no-op if directory already exists', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-config-test-${Date.now()}`);
      const stateDir = path.join(tmpDir, '.cyberdart');
    ...
    test('adds .cyberdart/ to .gitignore if not present', () => {
      ...
      expect(content).toContain('.cyberdart/');
      expect(content).toBe('node_modules/\n.cyberdart/\n');
      fs.rmSync(tmpDir, { recursive: true, force: true });
    });

    test('does not duplicate .cyberdart/ in .gitignore', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n.cyberdart/\n');
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.cyberdart', 'browse.json') });
    ...
    test('logs warning to browse-server.log on non-ENOENT gitignore error', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      // Create a read-only .gitignore (no .cyberdart/ entry → would try to append)
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n');
  ...
  describe('getRemoteSlug', () => {
    ...
    test('parses SSH remote URLs', () => {
      ...
      const url = 'git@github.com:garrytan/cyberdart.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('garrytan-cyberdart');
    });

    test('parses HTTPS remote URLs', () => {
      const url = 'https://github.com/garrytan/cyberdart.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('garrytan-cyberdart');
    });
    ...
    test('parses HTTPS remote URLs without .git suffix', () => {
      const url = 'https://github.com/garrytan/cyberdart';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('garrytan-cyberdart');
    });
