...
describe('claude-bin', () => {
  ...
  test('absolute-path override returned as-is', () => {
    const got = resolveClaudeCommand({
      ...EMPTY_ENV,
      CYBERDART_CLAUDE_BIN: '/opt/custom/claude',
    });
    expect(got).toEqual({ command: '/opt/custom/claude', argsPrefix: [] });
  });

  test('CLAUDE_BIN works as fallback alias for CYBERDART_CLAUDE_BIN', () => {
    const got = resolveClaudeCommand({
      ...EMPTY_ENV,
  ...
  test('CYBERDART_CLAUDE_BIN takes precedence over CLAUDE_BIN', () => {
    const got = resolveClaudeCommand({
      ...EMPTY_ENV,
      CYBERDART_CLAUDE_BIN: '/explicit/path',
  ...
  test('PATH-resolvable override goes through Bun.which (the bug the fork shipped)', () => {
    ...
    try {
      const got = resolveClaudeCommand({
        PATH: tmpDir,
        CYBERDART_CLAUDE_BIN: 'fake-claude-cli',
      });
  ...
  test('override pointing at missing binary → null (no silent fallback to bare claude)', () => {
    const got = resolveClaudeCommand({
      ...EMPTY_ENV,
      CYBERDART_CLAUDE_BIN: 'definitely-not-a-real-binary-xyz',
    });
    expect(got).toBeNull();
  });

  test('CYBERDART_CLAUDE_BIN_ARGS as JSON array → parsed argsPrefix', () => {
    const got = resolveClaudeCommand({
      ...EMPTY_ENV,
      CYBERDART_CLAUDE_BIN: '/opt/custom/claude',
      CYBERDART_CLAUDE_BIN_ARGS: '["--no-cache", "--verbose"]',
    });
    expect(got?.argsPrefix).toEqual(['--no-cache', '--verbose']);
  });

  test('CYBERDART_CLAUDE_BIN_ARGS as scalar string → treated as single argument', () => {
    const got = resolveClaudeCommand({
      ...EMPTY_ENV,
      CYBERDART_CLAUDE_BIN: '/opt/custom/claude',
      CYBERDART_CLAUDE_BIN_ARGS: 'claude',
    });
