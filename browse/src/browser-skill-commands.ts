 ...
 * and passes it via CYBERDART_SKILL_TOKEN. The skill never sees the daemon root
 ...
 *    CYBERDART_PORT and CYBERDART_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get CYBERDART_PORT +
 * CYBERDART_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'CYBERDART_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip CYBERDART_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.CYBERDART_PORT = String(opts.port);
  out.CYBERDART_SKILL_TOKEN = opts.skillToken;
