 ...
 *   project:  <project>/.cyberdart/browser-skills/<name>/
 *   global:   ~/.cyberdart/browser-skills/<name>/
 *   bundled:  <cyberdart-install>/browser-skills/<name>/   (read-only, ships with cyberdart)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  ...
  return {
    project: projectRoot ? path.join(projectRoot, '.cyberdart', 'browser-skills') : null,
    global: path.join(home, '.cyberdart', 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <cyberdart-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <cyberdart-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the cyberdart root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with cyberdart and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the cyberdart source tree.
 */
