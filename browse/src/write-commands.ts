...
export async function handleWriteCommand(
  ...
  switch (command) {
    ...
    case 'load-html': {
      ...
      if (fromFilePayload) {
        const MAX_BYTES = parseInt(process.env.CYBERDART_BROWSE_MAX_HTML_BYTES || '', 10) || (50 * 1024 * 1024);
        if (Buffer.byteLength(fromFilePayload.html, 'utf8') > MAX_BYTES) {
          throw new Error(
            `load-html: --from-file html too large (> ${MAX_BYTES} bytes). ` +
            'Raise with CYBERDART_BROWSE_MAX_HTML_BYTES=<N>.'
          );
        }
      ...
      if (stat.size > MAX_BYTES) {
        throw new Error(
          `load-html: file too large (${stat.size} bytes > ${MAX_BYTES} cap). Raise with CYBERDART_BROWSE_MAX_HTML_BYTES=<N> or split the HTML.`
        );
      }
      ...
      if (doSticky) {
        const stickyCount = await page.evaluate(() => {
          ...
          for (const { el, top, width, height } of stickyEls) {
            ...
            // Skip the cyberdart control indicator
            if ((el as HTMLElement).id === 'cyberdart-ctrl') continue;
