 ...
 * X_OK, case-insensitive Path/PATH on Windows). Adds the cyberdart-specific
 * override + arg-prefix logic on top.
 *
 * Override precedence:
 *   1. CYBERDART_CLAUDE_BIN (or CLAUDE_BIN as fallback) — absolute path or
 ...
 *
 * Arg prefix:
 *   CYBERDART_CLAUDE_BIN_ARGS (or CLAUDE_BIN_ARGS) prepends arguments to every
...
function parseOverrideArgs(env: NodeJS.ProcessEnv): string[] {
  const raw = env.CYBERDART_CLAUDE_BIN_ARGS ?? env.CLAUDE_BIN_ARGS;
  if (!raw?.trim()) return [];
  try {
...
export function resolveClaudeCommand(
  ...
  const override = (env.CYBERDART_CLAUDE_BIN ?? env.CLAUDE_BIN)?.trim();
  ...
  if (override) {
    ...
    // overrides like CYBERDART_CLAUDE_BIN=wsl find the actual binary.
