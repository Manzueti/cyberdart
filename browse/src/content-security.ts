 ...
 * Marks hidden elements with data-cyberdart-hidden attribute.
  ...
  return page.evaluate((ariaPatterns: string[]) => {
    ...
    for (const el of elements) {
      if (el instanceof HTMLElement) {
        ...
        if (isHidden) {
          el.setAttribute('data-cyberdart-hidden', 'true');
...
export async function getCleanTextWithStripping(page: Page | Frame): Promise<string> {
  return page.evaluate(() => {
    ...
    clone.querySelectorAll('[data-cyberdart-hidden]').forEach(el => el.remove());
...
/**
 * Clean up data-cyberdart-hidden attributes from the page.
 * Should be called after extraction is complete.
 */
export async function cleanupHiddenMarkers(page: Page | Frame): Promise<void> {
  await page.evaluate(() => {
    document.querySelectorAll('[data-cyberdart-hidden]').forEach(el => {
      el.removeAttribute('data-cyberdart-hidden');
    });
  });
}
