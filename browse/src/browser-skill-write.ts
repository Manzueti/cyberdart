 ...
 *
 * /skillify stages a candidate skill into ~/.cyberdart/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.cyberdart/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.cyberdart', '.tmp');
