 ...
 * cyberdart daemon over loopback HTTP.
 *
 * Distribution model:
 ...
 *
 * Auth resolution:
 *   1. CYBERDART_PORT + CYBERDART_SKILL_TOKEN env vars (set by `$B skill run` when
 ...
 *   2. State file fallback: read `BROWSE_STATE_FILE` env or `<git-root>/.cyberdart/browse.json`
...
export interface BrowseClientOptions {
  /** Override port. Default: CYBERDART_PORT env or state file. */
  port?: number;
  /** Override token. Default: CYBERDART_SKILL_TOKEN env, then state file root token. */
  token?: string;
  /** Tab id to target (every command can scope to a tab). Default: BROWSE_TAB env or undefined (active tab). */
  tabId?: number;
  /** Per-request timeout in milliseconds. Default: 30_000. */
  timeoutMs?: number;
  /** Override state-file path. Default: BROWSE_STATE_FILE env or <git-root>/.cyberdart/browse.json. */
  stateFile?: string;
}
...
export function resolveBrowseAuth(opts: BrowseClientOptions = {}): ResolvedAuth {
  ...
  const envPort = process.env.CYBERDART_PORT;
  const envToken = process.env.CYBERDART_SKILL_TOKEN;
  if (envPort && envToken) {
  ...
  throw new Error(
    ...
    '(sets CYBERDART_PORT + CYBERDART_SKILL_TOKEN) or run from a project with a live daemon ' +
    '(.cyberdart/browse.json must exist).'
  );
}
...
function defaultStateFile(): string | null {
  try {
    ...
    return path.join(base, '.cyberdart', 'browse.json');
  } catch {
    return path.join(process.cwd(), '.cyberdart', 'browse.json');
  }
}
