 ...
 *
 * To upgrade: bump this string, run `CYBERDART_BENCH_ENSEMBLE=1 bun test
 ...
 * layout at ~/.cyberdart/models/testsavant-small/ on first use.
 *
 ...
 */
const MODELS_DIR = path.join(os.homedir(), '.cyberdart', 'models');
...
// CYBERDART_SECURITY_ENSEMBLE=deberta. Not forced on every install because
...
function isDebertaEnabled(): boolean {
  const setting = (process.env.CYBERDART_SECURITY_ENSEMBLE ?? '').toLowerCase();
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.CYBERDART_SECURITY_OFF === '1') {
    testsavantState = 'failed';
    testsavantLoadError = 'CYBERDART_SECURITY_OFF=1 — ML classifier kill switch engaged';
...
export function loadDeberta(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.CYBERDART_SECURITY_OFF === '1') return Promise.resolve();
...
export async function checkTranscript(params: {
  ...
  return new Promise((resolve) => {
    ...
    // repo with a prompt-injection-defense CLAUDE.md (like cyberdart itself),
    ...
    // Env var CYBERDART_HAIKU_TIMEOUT_MS (milliseconds) overrides for benches
    // that want a different budget.
    const timeoutMs = process.env.CYBERDART_HAIKU_TIMEOUT_MS
      ? Number(process.env.CYBERDART_HAIKU_TIMEOUT_MS)
      : 45000;
    setTimeout(() => {
