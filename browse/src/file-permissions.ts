/**
 * Cross-platform file permission restriction for sensitive cyberdart state.
 *
 * Why this exists
 * ----------------
 * POSIX mode bits (`0o600` for files, `0o700` for dirs) are how cyberdart marks
 ...
 * install, sensitive cyberdart state files inherit whatever ACL the parent
...
function warnIcaclsFailure(fsPath: string, err: unknown): void {
  ...
  console.warn(
    `[cyberdart] Failed to restrict Windows ACL on ${fsPath}: ${msg}\n` +
