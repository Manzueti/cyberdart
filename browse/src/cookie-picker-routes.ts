 ...
 *
 * The session cookie (cyberdart_picker) is isolated from the scoped token system.
...
/** Extract session ID from the cyberdart_picker cookie. */
function getSessionFromCookie(req: Request): string | null {
  const cookie = req.headers.get('cookie');
  if (!cookie) return null;
  const match = cookie.match(/cyberdart_picker=([^;]+)/);
...
export async function handleCookiePickerRoute(
  ...
  try {
    ...
    if (pathname === '/cookie-picker' && req.method === 'GET') {
      ...
      if (code) {
        ...
        return new Response(null, {
          ...
          headers: {
            ...
            'Set-Cookie': `cyberdart_picker=${session}; HttpOnly; SameSite=Strict; Path=/cookie-picker; Max-Age=3600`,
      ...
      return new Response('Access denied. Open the cookie picker from cyberdart.', {
        status: 403,
        headers: { 'Content-Type': 'text/plain' },
      });
    }
