/**
 * cyberdart CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .cyberdart/browse.json for port + token
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and cyberdart
...
async function ensureServer(flags?: GlobalFlags): Promise<ServerState> {
  ...
  if (process.env.BROWSE_NO_AUTOSTART === '1') {
    ...
    console.error('[browse] The headed browser may have been closed. Run /open-cyberdart-browser to restart.');
    process.exit(1);
  }
  ...
  if (state && state.mode === 'headed' && isProcessAlive(state.pid)) {
    ...
    console.error(`[browse] Run '/open-cyberdart-browser' to restart.`);
    process.exit(1);
  }
...
/** Check if ngrok is installed and authenticated (native config or cyberdart env). */
function isNgrokAvailable(): boolean {
  // Check cyberdart's own ngrok env
  const ngrokEnvPath = path.join(process.env.HOME || '/tmp', '.cyberdart', 'ngrok.env');
...
async function handlePairAgent(state: ServerState, args: string[]): Promise<void> {
  ...
  if (localHost) {
    try {
      ...
      let globalRoot = `.${localHost}/skills/cyberdart`;
      try {
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`cyberdart browse — Fast headless browser for AI coding agents
