 ...
 *
 * Piggybacks on ~/.cyberdart/analytics/skill-usage.jsonl pattern (existing
 * cyberdart telemetry). Hostname + aggregate counters only; no body content,
 * no agent text, no command args. Respects the user's telemetry tier
 * setting (off | anonymous | community) via cyberdart-config.
 *
...
function cyberdartHome(): string {
  return process.env.CYBERDART_HOME || path.join(os.homedir(), '.cyberdart');
}

function analyticsDir(): string {
  return path.join(cyberdartHome(), 'analytics');
}
...
function isDisabled(): boolean {
  ...
  if (process.env.CYBERDART_TELEMETRY_OFF === '1') {
    telemetryDisabled = true;
    return true;
  }
  // Conservative default: telemetry ON unless explicitly off. Users opt out via
  // cyberdart-config set telemetry off (preamble reads this; we trust the env hint).
