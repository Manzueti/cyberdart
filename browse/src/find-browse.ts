/**
 * find-browse — locate the cyberdart browse binary.
 *
...
export function locateBinary(): string | null {
  ...
  if (root) {
    for (const m of markers) {
      const local = join(root, m, 'skills', 'cyberdart', 'browse', 'dist', 'browse');
  ...
  // Global fallback
  for (const m of markers) {
    const global = join(home, m, 'skills', 'cyberdart', 'browse', 'dist', 'browse');
