 ...
 *
 * Cross-process state lives at ~/.cyberdart/security/session-state.json
...
export type LayerName =
  | 'testsavant_content'
  | 'deberta_content'        // opt-in ensemble layer (CYBERDART_SECURITY_ENSEMBLE=deberta)
...
export interface AttemptRecord {
  ...
  layer: LayerName;
  verdict: Verdict;
  cyberdartVersion?: string;
}

const SECURITY_DIR = path.join(os.homedir(), '.cyberdart', 'security');
 ...
 * ~/.cyberdart/security/device-salt (0600). Random per-device, prevents rainbow
...
/**
 * Try to locate the cyberdart-telemetry-log binary. Resolution order matches
 * the existing skill preamble pattern (never relies on PATH — packaged
 * binary layouts can break that).
 *
 * Order:
 *  1. ~/.claude/skills/cyberdart/bin/cyberdart-telemetry-log  (global install)
 *  2. .claude/skills/cyberdart/bin/cyberdart-telemetry-log    (symlinked dev)
 *  3. bin/cyberdart-telemetry-log                          (in-repo dev)
 */
function findTelemetryBinary(): string | null {
  const candidates = [
    path.join(os.homedir(), '.claude', 'skills', 'cyberdart', 'bin', 'cyberdart-telemetry-log'),
    path.resolve(process.cwd(), '.claude', 'skills', 'cyberdart', 'bin', 'cyberdart-telemetry-log'),
    path.resolve(process.cwd(), 'bin', 'cyberdart-telemetry-log'),
  ];
 ...
 * CYBERDART_*_BIN override pattern from `browse/src/claude-bin.ts:resolveClaudeCommand`
 * (introduced in v1.24.0.0 #1252) so users on WSL/MSYS2/non-default Git Bash
 * installs can redirect.
 *
 * Override precedence:
 *   1. CYBERDART_BASH_BIN (or BASH_BIN) — absolute path or PATH-resolvable command.
...
export function resolveBashBinary(env: NodeJS.ProcessEnv = process.env): string | null {
  ...
  const override = (env.CYBERDART_BASH_BIN ?? env.BASH_BIN)?.trim();
  if (override) {
 ...
 * Win32: wraps in bash explicitly. `cyberdart-telemetry-log` is a shell script
 ...
 * Bun.which + CYBERDART_*_BIN override pattern as claude-bin.ts.
 *
 * Returns null when bash can't be resolved on Windows (rare — Git Bash ships
 * with the standard cyberdart install path). Caller skips spawn; the local
...
/**
 * Fire-and-forget subprocess invocation of cyberdart-telemetry-log with the
 ...
 * cyberdart-telemetry-log (which respects the user's telemetry tier setting).
...
// per-tab file under ~/.cyberdart/security/decisions/, sidebar-agent polls
