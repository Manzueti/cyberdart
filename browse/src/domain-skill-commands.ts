 ...
 *     suggested-action. Matches the cyberdart house style.
 *
...
function formatSavedOk(row: DomainSkillRow, slug: string): string {
  return [
    ...
    `Version: ${row.version}`,
    `Stored at: ~/.cyberdart/projects/${slug}/learnings.jsonl`,
    '',
...
async function handleEdit(args: string[]): Promise<string> {
  ...
  const tmpFile = path.join(os.tmpdir(), `cyberdart-domain-skill-${process.pid}-${Date.now()}.md`);
...
async function handlePromoteToGlobal(args: string[]): Promise<string> {
  ...
  return [
    ...
    `Stored at: ~/.cyberdart/global-domain-skills.jsonl`,
    '',
