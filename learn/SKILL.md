...
version: 1.0.0
description: |
  Manage project learnings. Review, search, prune, and export what cyberdart
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"learn","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"learn","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"learn","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
see what cyberdart has learned across sessions on this project, search for relevant
...
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)"
~/.claude/skills/cyberdart/bin/cyberdart-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
cyberdart will automatically capture patterns, pitfalls, and insights it discovers."
...
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)"
~/.claude/skills/cyberdart/bin/cyberdart-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)"
~/.claude/skills/cyberdart/bin/cyberdart-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)"
~/.claude/skills/cyberdart/bin/cyberdart-learnings-search --limit 50 2>/dev/null
```
...
LEARN_FILE="$CYBERDART_STATE_ROOT/projects/$SLUG/learnings.jsonl"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```
