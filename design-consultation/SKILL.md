...
description: |
  ...
  design system or DESIGN.md. (cyberdart)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-consultation","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"design-consultation","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"design-consultation","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)"
ls ~/.cyberdart/projects/$SLUG/*office-hours* 2>/dev/null | head -5
...
**Find the cyberdart designer (optional — enables AI mockup generation):**
...
```bash
_TASTE_PROFILE=~/.cyberdart/projects/$SLUG/taste-profile.json
...
the legacy approved.json aggregate — `~/.claude/skills/cyberdart/bin/cyberdart-taste-update`
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the cyberdart designer is available.
...
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.cyberdart/projects/$SLUG/designs/design-system-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-learnings-log '{"skill":"design-consultation","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
