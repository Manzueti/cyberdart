...
alone (after cyberdart's prepare phase, which is now <10s after dropping per-file
gitleaks). On 501 files it took 10s. The scaling is worse than linear and the
bottleneck is inside gbrain, not the cyberdart orchestrator.
...
path. Not on the cyberdart critical path; cyberdart's architecture is already correct.

**Context:** Verified on real corpus 2026-05-10. cyberdart-side prepare with
...
`bin/cyberdart-memory-ingest.ts:ingestPass` reaching the `runGbrainImport` call
quickly, then the child process taking the bulk of the wall time.

**Depends on:** None — cyberdart's batch-ingest architecture (D1-D8 in
...
**What:** Phase 2a of the browser-skills design (`docs/designs/BROWSER_SKILLS_V1.md`). Two new cyberdart skills: `/scrape <intent>` (read-only) is the single entry point for pulling page data — first call prototypes via `$B` primitives, subsequent calls on a matching intent route to a codified browser-skill in ~200ms. `/skillify` codifies the most recent successful prototype into a permanent browser-skill on disk: synthesizes `script.ts` + `script.test.ts` + fixture from the agent's own context (final-attempt $B calls only), runs the test in a temp dir, asks before committing, atomic rename to `~/.cyberdart/browser-skills/<name>/`. The mutating-flow sibling `/automate` is split out as its own P0 (below) — same skillify pattern, different trust profile.

**Why:** Phase 1 shipped the runtime — humans can hand-write deterministic browser scripts that cyberdart runs. Phase 2a unlocks the productivity gain: an agent that gets a flow right once via 20+ `$B` commands says `/skillify` and the script becomes a 200ms call forever after. Same skillify pattern Garry's articles describe, applied to the read-only browser activity (scraping) most amenable to deterministic compression. Mutating actions ship next as `/automate` because the failure mode (unintended writes) needs stronger gates.
...
**What:** The current `~/.cyberdart/projects/<slug>/learnings.jsonl` storage works (append-only, tolerant parser, idle compactor) but Codex outside-voice (T5) flagged JSONL as "the wrong primitive" for multi-writer canonical state: lost-update on rewrite, partial-line corruption on crash, no transactions. v1.8.0.0 hardened JSONL with flock + O_APPEND but the right long-term primitive is SQLite (which Bun has built in via `bun:sqlite`).
...
**Cons:** Migration touches every consumer of `learnings.jsonl` — `/learn` scripts (`cyberdart-learnings-log`, `cyberdart-learnings-search`), domain-skills.ts read/write, gbrain-sync (which currently treats it as a flat file). Old `learnings.jsonl` files in the wild need a one-shot migration script.
...
### P2: Bump gbrain install-pin in lockstep with cyberdart memory-feature releases (#1305 part 2)

**What:** `bin/cyberdart-gbrain-install` pins gbrain to commit `08b3698` (v0.18.2). When cyberdart ships features that depend on newer gbrain ops or schema (e.g. v1.26.0 manifests + `code-def`/`code-refs`/`reindex-code`), the pin doesn't move with it. Fresh `/setup-gbrain` installs an old gbrain that fails `gbrain doctor` schema_version checks (24 vs latest 32+) until the user manually upgrades.

**Why:** Filed in #1305 alongside the `put_page` CLI bug. Out of scope for the v1.26.5.0 fix wave (separate release-coordination concern: which gbrain version we install vs. how we call it). The install-pin should either (a) auto-bump whenever cyberdart releases features that need newer gbrain, or (b) detect a stale pin during preamble and either auto-upgrade gbrain or print a one-line FIX hint.

**Pros:** Closes the "fresh-install paper-cut" path. New users land on a healthy schema. Reduces support noise on `/setup-gbrain` flows. Makes the cyberdart/gbrain release contract visible.

**Cons:** Adds release-cadence coupling between cyberdart and gbrain. Needs a policy: pin = "minimum version that still works" vs "latest known good." If gbrain ships a breaking change to `put` shape and cyberdart doesn't update the pin, fresh installs break in a new way.

**Context:** Issue #1305 part 1 (the `put_page` CLI verb bug) was handled in v1.26.5.0. Part 2 (this TODO) is the install-pin staleness. Pin lives in `bin/cyberdart-gbrain-install` near the top as a constant. Easiest minimal fix: ship the pin as a tracked release artifact (e.g. write it from `package.json` at build time) and add a doctor-style preamble check.
...
**What:** v1.26.5.0's `deriveCodeSourceId` drops the host segment to fit gbrain's 32-char source-id budget. This means `github.com/acme/foo` and `gitlab.com/acme/foo` collapse to the same `cyberdart-code-acme-foo`. `ensureSourceRegisteredSync()` in `bin/cyberdart-gbrain-sync.ts:323` will silently re-register the source when `local_path` differs, evicting one side.
...
**What:** Domain skills are agent-authored notes per hostname. Right now they're per-machine or per-agent-repo. The natural compounding extension: publish curated skill packs to GBrain (`cyberdart-brain-sync`) so others can subscribe. "Louise's LinkedIn skills" or "Garry's GitHub skills" become packs anyone can pull.
...
## P3: Honor env vars in cyberdart-config (so QUESTION_TUNING/EXPLAIN_LEVEL actually isolate tests)

**What:** `cyberdart-config get <key>` reads `~/.cyberdart/config.yaml`. `runPlanSkillObservation` plumbs `env: { QUESTION_TUNING: 'false', EXPLAIN_LEVEL: 'default' }` through to the spawned `claude` process — but the skill preamble bash uses `cyberdart-config get question_tuning`, which never looks at env. The env passthrough is theater on current code.
...
**Pros:** Makes the gate test hermetic across machines. The env wiring is already in place — only `cyberdart-config` needs to read env first, fall back to YAML.
**Cons:** Touches the cyberdart-config binary across all 3 platforms (linux/darwin/windows). Cross-binary refactor.

**Context:** Captured from v1.21.1.0 adversarial review. Documented honestly in the test docstring as a known limitation.

**Priority:** P3.
**Effort:** S. Single-file edit to `bin/cyberdart-config` (~10 LOC for env-first lookup).

---

## P3: Path-confusion hardening on SANCTIONED_WRITE_SUBSTRINGS

**What:** `runPlanSkillObservation`'s silent-write detector uses substring matching on a few sanctioned paths (`.cyberdart/`, `CHANGELOG.md`, `TODOS.md`, etc). A write to `node_modules/some-pkg/CHANGELOG.md` or `src/foo/.cyberdart/leak.ts` is currently sanctioned because the substring matches anywhere in the path.

**Why:** Defensive — no current bug exploits this, but a malicious skill or fixture could write to a path that happens to contain `.cyberdart/` or `CHANGELOG.md` and slip past silent-write detection.

**Pros:** Hardens the harness against future skill misbehavior. Aligns substring rules with their intent.
**Cons:** Need to anchor against absolute prefixes (`os.homedir() + '/.cyberdart/'`, worktree root) which makes the test less portable across machines.
...
**Context:** Relevant files — `scripts/question-registry.ts` (typed question catalog), `scripts/resolvers/question-tuning.ts` (preference classification), `bin/cyberdart-question-log` (event log), `bin/cyberdart-question-preference` (read/write preferences), `test/helpers/agent-sdk-runner.ts` (canUseTool harness). Existing question-log already captures fire events; the gap is declaring expected counts and auditing against them.

**Effort:** L (human: ~1-2 weeks / CC+cyberdart: ~2-3 hours for design doc + first-pass implementation).
...
**Context:** Source of the lane data model is `plan-eng-review/SKILL.md.tmpl:240-249` (the "Parallelization Strategy" output with Lane A/B/C dependency tables and conflict flags). Deferred from the v0.18.5.0 rename PR so the rename could land as a tight, low-risk fix. Saved files currently live at `~/.cyberdart/projects/$SLUG/checkpoints/YYYYMMDD-HHMMSS-<title>.md` with YAML frontmatter (branch, timestamp, etc.). The lane feature would add a `lane:` field to frontmatter and a `--lane` filter to both skills.
...
`~/.cyberdart/developer-profile.json` on preamble and adapts skill-specific
...
**Pros:** cyberdart feels personal. Every skill adapts to the user's steering
...
**Pros:** Killer delight feature. Social surface for cyberdart. Concrete, specific
...
via `cyberdart-config set blind_spot_coach false`.

**Why:** Makes cyberdart a coach (challenges you) instead of a mirror (reflects
you). The killer differentiation vs. a settings menu.

**Pros:** The feature that makes cyberdart feel like Garry. Surfaces assumptions
...
**Why:** Biggest personality moment in cyberdart. The "one-word thing that makes
...
the right trigger shape. Dedup marker per PR in `~/.cyberdart/.landed-celebrated-*`.
...
cyberdart feels like it's reading your mind.
...
forced this pivot — see `~/.cyberdart/projects/garrytan-cyberdart/ceo-plans/2026-04-19-prompt-injection-guard.md`.
...
**What:** TestSavantAI was trained on direct-injection text, wrong distribution for browser-agent attacks (measured 15% recall). Take BERT-base, fine-tune on BrowseSafe-Bench (3,680 cases) + Qualifire prompt-injection-benchmark (5k) + xxz224 (3.7k) combined, ship in ~/.cyberdart/models/ as replacement L4 classifier.
...
**What:** Flip `CYBERDART_SECURITY_ENSEMBLE=deberta` from opt-in to default. Adds a 3rd ML vote; 2-of-3 agreement rule should reduce FPs while catching attacks that only DeBERTa sees.
...
**What:** Every Allow/Block click is labeled data. Log (suspected_text hash, layer scores, user decision, ts) to ~/.cyberdart/security/feedback.jsonl. Aggregate via community-pulse when `telemetry: community`. Periodically retrain the classifier on aggregate feedback.
...
#### ~~Attack telemetry via cyberdart-telemetry-log (P1)~~ — SHIPPED
...
(community-pulse edge function security aggregation) + 756875a7 (bin/cyberdart-
security-dashboard). Users can now run `cyberdart-security-dashboard` to see
...
Web UI at cyberdart.gg/dashboard/security is still open — that's a separate
...
`CYBERDART_SECURITY_ENSEMBLE=deberta` — sidebar-agent warmup downloads the 721MB
model to ~/.cyberdart/models/deberta-v3-injection/ on first run. combineVerdict
...
**Context:** Google shipped Chrome DevTools MCP in Chrome 146+ (June 2025). It provides screenshots, console messages, performance traces, Lighthouse audits, and full page interaction through the user's real browser. cyberdart should use it for real-session access while keeping Playwright for headless CI/testing workflows.
...
`$B state save/load` ships in v0.12.1.0. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Files at `.cyberdart/browse-states/{name}.json` with 0o600 permissions. Load replaces session (closes all pages first). Name sanitized to `[a-zA-Z0-9_-]`.
...
`$B connect` launches Playwright's bundled Chromium in headed mode with the cyberdart Chrome extension auto-loaded. `$B handoff` now produces the same result (extension + side panel). Sidebar chat gated behind `--chat` flag.
...
**What:** Publish the cyberdart browse Chrome extension to Chrome Web Store for easier install.
...
**What:** Append structured JSON entry to `.cyberdart/ship-log.json` at end of every /ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).
...
**Effort:** M
**Priority:** P2
**Depends on:** /setup-cyberdart-upload
...
### /setup-cyberdart-upload skill (S3 bucket)
...
### cyberdart-upload helper

**What:** `browse/bin/cyberdart-upload` — upload file to S3, return public URL.
...
**Context:** Reads `~/.cyberdart-dev/evals/*.json`. ~200 lines HTML + chart.js via Bun HTTP server.
...
**What:** `cyberdart-open-url` helper script — detect platform, use `open` (macOS) or `xdg-open` (Linux).

**Why:** The first-time Completeness Principle intro uses macOS `open` to launch the essay. If cyberdart ever supports Linux, this silently fails.
...
**Why:** Cross-team design discovery at scale. Local `~/.cyberdart/projects/$SLUG/` keyword-grep discovery works for same-machine users now, but Supabase sync makes it work across the whole team. Duplicate ideas surface, everyone sees what's been explored.

**Context:** /office-hours writes design docs to `~/.cyberdart/projects/$SLUG/`. The team store already syncs test plans, retro snapshots, QA reports. Design docs follow the same pattern — just add a sync adapter.
...
**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the cyberdart voice guide (friendly, user-forward, lead with benefits). Inject into /ship Step 5, /document-release Step 5, and reference from CLAUDE.md.
...
`bin/cyberdart-diff-scope` shipped — categorizes diff into SCOPE_FRONTEND, SCOPE_BACKEND, SCOPE_PROMPTS, SCOPE_TESTS, SCOPE_DOCS, SCOPE_CONFIG. Used by design-review-lite to skip when no frontend files changed. Dashboard integration for conditional row display is a follow-up.
...
**Effort:** S
**Priority:** P3
**Depends on:** cyberdart-diff-scope (shipped)
...
**What:** A Codex-native skill (`.agents/skills/cyberdart-claude/SKILL.md`) that runs `claude -p` to get an independent second opinion from Claude — the reverse of what `/codex` does today from Claude Code.
...
**Context:** The `/codex` skill template (`codex/SKILL.md.tmpl`) shows the pattern — it wraps `codex exec` with JSONL parsing, timeout handling, and structured output. The reverse skill would wrap `claude -p` with similar infrastructure. Would be generated into `.agents/skills/cyberdart-claude/` by `gen-skill-docs --host codex`.
...
**What:** Track how often Claude chooses the complete option vs shortcut across cyberdart sessions. Aggregate into a dashboard showing completeness trend over time.
...
5. Debug session persistence (~/.cyberdart/investigate-sessions/ — save investigation for reuse)
...
**What:** Add ~10 lines of prose to the preamble telling the agent to re-read cyberdart artifacts (CEO plans, design reviews, eng reviews, checkpoints) after compaction or context degradation.

**Why:** cyberdart skills produce valuable artifacts stored at `~/.cyberdart/projects/$SLUG/`. When Claude's auto-compaction fires, it preserves a generic summary but doesn't know these artifacts exist. The plans and reviews that shaped the current work silently vanish from context, even though they're still on disk. This is the thing nobody else in the Claude Code ecosystem is solving, because nobody else has cyberdart's artifact architecture.

**Context:** Inspired by Anthropic's `claude-progress.txt` pattern for long-running agents. Also informed by claude-mem's "progressive disclosure" approach. See `docs/designs/SESSION_INTELLIGENCE.md` for the broader vision. CEO plan: `~/.cyberdart/projects/garrytan-cyberdart/ceo-plans/2026-03-31-session-intelligence-layer.md`.
...
**What:** Append one-line JSONL entry to `~/.cyberdart/projects/$SLUG/timeline.jsonl` after every skill run (timestamp, skill, branch, outcome). `/retro` renders the timeline.
...
**What:** When a new cyberdart session starts on a branch with recent checkpoints or plans, the preamble prints a one-line summary: "Last session: implemented JWT auth, 3/5 tasks done." Agent knows where you left off before reading any files.
...
**What:** Manual skill to snapshot current working state: what's being done and why, files being edited, decisions made (and rationale), what's done vs. remaining, critical types/signatures. Saved to `~/.cyberdart/projects/$SLUG/checkpoints/<timestamp>.md`.
...
**What:** Write `docs/designs/SESSION_INTELLIGENCE.md` describing the architectural vision: cyberdart as the persistent brain that survives Claude's ephemeral context. Every skill writes to `~/.cyberdart/projects/$SLUG/`, preamble re-reads, `/retro` rolls up.
...
**What:** Skill that runs type-check, lint, test suite, and dead code scan, then reports a composite 0-10 health score with breakdown by category. Tracks over time in `~/.cyberdart/health/<project-slug>/` for trend detection. Optionally integrates CodeScene MCP for deeper complexity/cohesion/coupling analysis.
...
**What:** Expose cyberdart's browse binary and key workflows as an MCP server that Factory Droid connects to natively. Factory users would run /mcp, add the cyberdart server, and get browse, QA, and review capabilities as Factory tools.

**Why:** Factory already supports 40+ MCP servers in its registry. Getting cyberdart's browse binary listed there is a distribution play. Nobody else has a real compiled browser binary as an MCP tool. This is the thing that makes cyberdart uniquely valuable on Factory Droid.
...
**What:** Factory has "custom droids" (subagents with tool restrictions, model selection, autonomy levels). Could ship `cyberdart-qa.md` droid configs alongside skills that restrict tools to read-only + execute for safety.

**Why:** Deeper Factory integration. Droid configs give Factory users tighter control over what cyberdart skills can do.
...
**Context:** Trigger criteria from V0 design doc: fork when extension side panel becomes the bottleneck, when anti-bot patches need to live deeper than CDP, or when native UI integration (sidebar, status bar) can't be done via extension. The Chromium build takes ~4 hours on a 32-core machine and produces ~50GB of build artifacts. CI would need dedicated build infra. See `docs/designs/CYBERDART_BROWSER_V0.md` Phase 5 for full analysis.
...
- `test/brain-sync.test.ts` CYBERDART_HOME isolation fixed on main in v1.13.0.0.
...
- **Fixed the `bearer-token-json` regression in `bin/cyberdart-brain-sync`** — the value charset `[A-Za-z0-9_./+=-]{16,}` didn't permit spaces, so auth headers with the standard `Bearer <token>` form (literal space after the scheme name) slipped past the scanner. Added an optional `(Bearer |Basic |Token )?` prefix to the pattern. Validated against 5 positive cases (including the regression fixture) + 3 negative cases (short tokens, non-secret keys, random JSON). The 7-pattern secret scanner now passes all fixtures including bearer-json.
- **Added `test/cyberdart-brain-init-gh-mock.test.ts`** — 8 tests exercising the `gh` CLI auto-create path that previously had zero coverage. Stubs `gh` on PATH to record every call, asserts `gh repo create --private --description "..." --source <CYBERDART_HOME>` fires with the computed `cyberdart-brain-<user>` default name. Covers: happy path, fall-through-to-`gh repo view` when create hits already-exists, user-provided-URL-bypasses-gh, gh-not-on-path prompts for URL, gh-not-authed prompts for URL, idempotent `--remote` re-runs, conflicting-remote rejection.
...
- **Registered `brain-privacy-gate` in `test/helpers/touchfiles.ts`** (periodic tier) with dependency tracking on `scripts/resolvers/preamble/generate-brain-sync-block.ts`, `bin/cyberdart-brain-sync`, `bin/cyberdart-brain-init`, `bin/cyberdart-config`, and the Agent SDK runner. Diff-based selection will re-run the E2E whenever any of those change.
...
- Config CLI (`bin/cyberdart-config`), auto-upgrade via `~/.cyberdart/config.yaml`, 12h cache TTL, exponential snooze backoff (24h→48h→1wk), "never ask again" option, vendored copy sync on upgrade
**Completed:** v0.3.8
