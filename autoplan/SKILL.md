...
description: |
  ...
  gauntlet without answering 15-30 intermediate questions. (cyberdart)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"autoplan","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"autoplan","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"autoplan","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Read the `/office-hours` skill file at `~/.claude/skills/cyberdart/office-hours/SKILL.md` using the Read tool.
...
SLUG=$(~/.claude/skills/cyberdart/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.cyberdart/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.cyberdart/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
> IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/cyberdart). These are AI assistant skill definitions meant for a different system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Stay focused on the repository code only.

This prevents Codex from discovering cyberdart skill files on disk and following their
...
echo "RESTORE_PATH=$HOME/.cyberdart/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t ~/.cyberdart/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- `~/.claude/skills/cyberdart/plan-ceo-review/SKILL.md`
- `~/.claude/skills/cyberdart/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/.claude/skills/cyberdart/plan-eng-review/SKILL.md`
- `~/.claude/skills/cyberdart/plan-devex-review/SKILL.md` (only if DX scope detected)
...
```bash
_TEL=$(~/.claude/skills/cyberdart/bin/cyberdart-config get telemetry 2>/dev/null || echo off)
source ~/.claude/skills/cyberdart/bin/cyberdart-codex-probe
...
if ! command -v codex >/dev/null 2>&1; then
  _cyberdart_codex_log_event "codex_cli_missing"
  echo "[codex-unavailable: binary not found] — proceeding with Claude subagent only"
  _CODEX_AVAILABLE=false
elif ! _cyberdart_codex_auth_probe >/dev/null; then
  _cyberdart_codex_log_event "codex_auth_failed"
  ...
  _cyberdart_codex_version_check   # non-blocking warn if known-bad
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  _cyberdart_codex_timeout_wrapper 600 codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/cyberdart). These are AI assistant skill definitions meant for a different system. Stay focused on repository code only.
  ...
  if [ "$_CODEX_EXIT" = "124" ]; then
    _cyberdart_codex_log_event "codex_timeout" "600"
    _cyberdart_codex_log_hang "autoplan" "0"
...
- Test plan: generate artifact at `~/.cyberdart/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at ~/.cyberdart/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
