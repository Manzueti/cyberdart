# cyberdart memory ingest — what it does, what stays local, what you can do with it
...
| Eureka log | `eureka` | `~/.cyberdart/analytics/eureka.jsonl` | Medium — your insights, often non-secret |
| Project learnings | `learning` | `~/.cyberdart/projects/<slug>/learnings.jsonl` | Medium |
| Project timeline | `timeline` | `~/.cyberdart/projects/<slug>/timeline.jsonl` | Low |
| CEO plans | `ceo-plan` | `~/.cyberdart/projects/<slug>/ceo-plans/*.md` | Medium |
| Design docs | `design-doc` | `~/.cyberdart/projects/<slug>/*-design-*.md` | Medium |
| Retros | `retro` | `~/.cyberdart/projects/<slug>/retros/*.md` | Medium |
| Builder profile | `builder-profile-entry` | `~/.cyberdart/builder-profile.jsonl` | Low |

## What stays local

- **State files** (`~/.cyberdart/.gbrain-sync-state.json`,
  `~/.cyberdart/.transcript-ingest-state.json`,
  `~/.cyberdart/.gbrain-engine-cache.json`,
  `~/.cyberdart/.gbrain-errors.jsonl`) are local-only per ED1 (state file
...
The cross-machine secret boundary is `cyberdart-brain-sync` (the git push
...
```bash
cyberdart-memory-ingest --bulk --scan-secrets
# or
CYBERDART_MEMORY_INGEST_SCAN_SECRETS=1 cyberdart-memory-ingest --bulk
```
...
was skipped via `bun run bin/cyberdart-memory-ingest.ts --probe` (which
...
- **Supabase configured:** code + transcripts go to Supabase Storage
  ...
  brain-linked git repo via `cyberdart-brain-sync`.
...
- **Read a specific page:**
  ```bash
  gbrain get_page transcripts/claude-code/garrytan-cyberdart/2026-05-01-abc123
  ```
...
- **Bulk-delete by criteria** (V1.0.1 follow-up — `cyberdart-transcript-prune`
  helper). For V1.0, use `gbrain delete_page <slug>` per-page or write
  a small loop over `gbrain list_pages` output.

- **Disable entirely:**
  ```bash
  cyberdart-config set transcript_ingest_mode off
  cyberdart-config set gbrain_context_load off  # also disables retrieval
  ```

## How the agent uses it

At every cyberdart skill start, the preamble runs
`cyberdart-brain-context-load` which:
...
- **Salience block is empty** — your transcripts may not be ingested
  yet. Run `cyberdart-gbrain-sync --full` to do a full pass.
...
- **A page has stale or wrong content** — `gbrain delete_page <slug>`,
  then re-run `cyberdart-gbrain-sync --incremental` to re-ingest from
  source if the source file is still on disk and unchanged.

## Privacy + audit

- Every `secretScanFile` finding is logged to stderr at ingest time.
- Every gbrain put/delete is logged to `~/.cyberdart/.gbrain-errors.jsonl`
  with `{ts, op, duration_ms, outcome}` for forensic tracing.
- `~/.cyberdart/.gbrain-engine-cache.json` shows which storage tier is
...
1. Verifies the URL + token via `cyberdart-gbrain-mcp-verify`. Three failure
...
3. Skips local install, local doctor, transcript ingest, and federated
   ...
   doesn't install.
4. Optionally provisions a `cyberdart-artifacts-$USER` private repo on
...
`cyberdart-artifacts-init` always prints the `gbrain sources add` command
...
- Artifacts repo: github.com/garrytan/cyberdart-artifacts-garrytan (private)
...
(See `cyberdart/setup-gbrain/SKILL.md.tmpl` for the full Path 4 flow plus
the gbrain enhancement requests around scoped tokens that would let
cyberdart auto-rotate in V2.)
