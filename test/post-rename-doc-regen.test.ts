...
// `cyberdart-brain-init` or `gbrain_sync_mode` strings appear in any of the
...
const FORBIDDEN_PATTERNS = [
  ...
  /^.*\bcyberdart-brain-init\b.*$/m,
  /^.*\bgbrain_sync_mode\b.*$/m,
];

// Per the preamble resolver: generated docs DO contain the
// "~/.cyberdart-brain-remote.txt" string in the migration-window fallback. We
...
describe('post-rename doc-regen regression (codex Finding #12)', () => {
  test('no generated SKILL.md contains "cyberdart-brain-init"', () => {
    const offenders: string[] = [];
    for (const file of findSkillMdFiles()) {
      const content = fs.readFileSync(file, 'utf-8');
      const m = content.match(/^.*\bcyberdart-brain-init\b.*$/m);
      if (m) offenders.push(`${path.relative(ROOT, file)}: ${m[0].slice(0, 100)}`);
    }
    if (offenders.length > 0) {
      console.error(`Stale "cyberdart-brain-init" in generated SKILL.md files:\n${offenders.map((o) => '  ' + o).join('\n')}`);
    }
