/**
 * Taste engine — end-to-end tests for `cyberdart-taste-update`.
 *
 ...
 *
 * All tests use CYBERDART_STATE_DIR pointing at a temp dir so no real home dir is
...
const BIN = path.join(ROOT, 'bin', 'cyberdart-taste-update');
...
beforeEach(() => {
  ...
  // Initialize a git repo so cyberdart-taste-update's getSlug() finds a toplevel
...
function run(args: string[]): { status: number | null; stdout: string; stderr: string } {
  const result = spawnSync('bun', ['run', BIN, ...args], {
    cwd: workdir,
    env: { ...process.env, CYBERDART_STATE_DIR: stateDir, HOME: stateDir },
    encoding: 'utf-8',
    timeout: 10000,
  });
  return {
