/**
 * cyberdart-gbrain-source-wireup — unit tests with mocked gbrain CLI.
 *
 * The helper registers the cyberdart brain repo as a gbrain federated source
...
const WIREUP_BIN = path.join(BIN_DIR, 'cyberdart-gbrain-source-wireup');

let tmpHome: string;
let cyberdartHome: string;
...
function run(
  ...
  const env = {
    ...
    CYBERDART_BRAIN_WORKTREE: worktreeDir,
    CYBERDART_BRAIN_NO_SYNC: '0',
    ...(opts.env || {}),
  };
...
function setupCyberDartRepo(remoteUrl: string) {
  // Real git repo at cyberdartHome with at least one commit + an origin remote.
  fs.mkdirSync(cyberdartHome, { recursive: true });
  spawnSync('git', ['-C', cyberdartHome, 'init', '-q', '-b', 'main'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cyberdartHome, 'config', 'user.email', 'test@example.com'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cyberdartHome, 'config', 'user.name', 'test'], { stdio: 'pipe' });
  fs.writeFileSync(path.join(cyberdartHome, '.brain-allowlist'), '# allowlist\n');
  spawnSync('git', ['-C', cyberdartHome, 'add', '.'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cyberdartHome, 'commit', '-q', '-m', 'init'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cyberdartHome, 'remote', 'add', 'origin', remoteUrl], { stdio: 'pipe' });
}

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-wireup-test-'));
  cyberdartHome = path.join(tmpHome, '.cyberdart');
  worktreeDir = path.join(tmpHome, '.cyberdart-brain-worktree');
...
describe('cyberdart-gbrain-source-wireup — wireup mode', () => {
  test('fresh state: registers source + creates worktree + syncs', () => {
    setupCyberDartRepo('git@github.com:user/cyberdart-brain-user.git');
    makeFakeGbrain({});
    const r = run([], { env: { CYBERDART_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(fs.existsSync(worktreeDir)).toBe(true);
    const state = readState();
    expect(state.sources).toHaveLength(1);
    expect(state.sources[0].id).toBe('cyberdart-brain-user');
  ...
  test('idempotent re-run after success: no new sources add call', () => {
    setupCyberDartRepo('git@github.com:user/cyberdart-brain-user.git');
    makeFakeGbrain({});
    run([], { env: { CYBERDART_BRAIN_NO_SYNC: '1' } });
  ...
  test('drift recovery: existing source with different path triggers remove + add', () => {
    ...
    fs.writeFileSync(
      gbrainStateFile,
      JSON.stringify({
        sources: [{ id: 'cyberdart-brain-user', local_path: '/old/stale/path', federated: true }],
      })
    );
    const r = run([], { env: { CYBERDART_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
    expect(calls.some((c) => c.startsWith('gbrain sources remove cyberdart-brain-user'))).toBe(true);
    expect(calls.some((c) => c.includes(`gbrain sources add cyberdart-brain-user --path ${worktreeDir}`))).toBe(true);
    const state = readState();
    expect(state.sources[0].local_path).toBe(worktreeDir);
  });
  ...
  test('source-id derived from origin URL', () => {
    setupCyberDartRepo('git@github.com:user/cyberdart-brain-alice.git');
    makeFakeGbrain({});
    const r = run([], { env: { CYBERDART_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(readState().sources[0].id).toBe('cyberdart-brain-alice');
  });

  test('source-id fallback to ~/.cyberdart-brain-remote.txt when .git is gone', () => {
    // No git repo at cyberdartHome; just the remote-file
    fs.mkdirSync(tmpHome, { recursive: true });
    fs.writeFileSync(
      path.join(tmpHome, '.cyberdart-brain-remote.txt'),
      'git@github.com:user/cyberdart-brain-bob.git\n'
    );
    makeFakeGbrain({});
    // No --strict: helper should benign-skip because .cyberdart/.git is missing
    const r = run([]);
  ...
  test('source-id from --source-id flag overrides everything', () => {
    setupCyberDartRepo('git@github.com:user/cyberdart-brain-different.git');
    makeFakeGbrain({});
    run(['--source-id', 'custom-id'], { env: { CYBERDART_BRAIN_NO_SYNC: '1' } });
    const state = readState();
  ...
  test('--probe: read-only, prints state without mutating', () => {
    ...
    const r = run(['--probe']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('source_id=cyberdart-brain-user');
...
describe('cyberdart-gbrain-source-wireup — --database-url lock (defends against external config rewrites)', () => {
  test('--database-url flag is exported as GBRAIN_DATABASE_URL to child gbrain calls', () => {
    ...
    const r = run(['--database-url', TARGET], { env: { CYBERDART_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
...
describe('cyberdart-gbrain-source-wireup — uninstall mode', () => {
...
describe('cyberdart-gbrain-source-wireup — defensive paths', () => {
  test('--no-pull skips HEAD advance on existing worktree', () => {
    ...
    fs.writeFileSync(path.join(cyberdartHome, 'newfile.md'), 'new');
    spawnSync('git', ['-C', cyberdartHome, 'add', '.'], { stdio: 'pipe' });
    spawnSync('git', ['-C', cyberdartHome, 'commit', '-q', '-m', 'second commit'], { stdio: 'pipe' });
    const parentHeadAfter = spawnSync('git', ['-C', cyberdartHome, 'rev-parse', 'HEAD'], {
      encoding: 'utf-8',
    }).stdout.trim();
    ...
    const r = run(['--no-pull'], { env: { CYBERDART_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
