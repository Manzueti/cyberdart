/**
 * v1.27.0.0 migration — cyberdart-brain → cyberdart-artifacts rename.
 *
...
const MIGRATION = path.join(ROOT, 'cyberdart-upgrade', 'migrations', 'v1.27.0.0.sh');
  ...
  repo)
    ...
    case "$1" in
      view)
        # gh repo view <name>
        shift
        ${alreadyRenamed ? `if echo "$@" | grep -q cyberdart-artifacts; then exit 0; else exit 1; fi` : `exit 1`}
        ;;
...
case "$1 $2" in
  "sources list")
    ${hasOld ? `echo "cyberdart-brain-testuser ~/.cyberdart-brain-worktree"` : 'true'}
    exit 0
    ;;
...
beforeEach(() => {
  ...
  fs.mkdirSync(path.join(tmpHome, '.cyberdart'), { recursive: true });
});
...
describe('v1.27.0.0 migration — nothing to migrate', () => {
  test('no legacy state → exits 0, writes done touchfile, no journal', () => {
    // Fresh HOME: no brain-remote.txt, no .cyberdart/.git
    const r = run();
    expect(r.code).toBe(0);
    expect(r.stderr).toContain('nothing to migrate');
    expect(fs.existsSync(path.join(tmpHome, '.cyberdart/.migrations/v1.27.0.0.done'))).toBe(true);
    expect(fs.existsSync(path.join(tmpHome, '.cyberdart/.migrations/v1.27.0.0.journal'))).toBe(false);
  });

  test('done touchfile present → exits 0 silently (no re-prompt)', () => {
    fs.mkdirSync(path.join(tmpHome, '.cyberdart/.migrations'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.cyberdart/.migrations/v1.27.0.0.done'), '');
    const r = run();
    expect(r.code).toBe(0);
  ...
  test('skipped-by-user touchfile → exits 0 silently', () => {
    fs.mkdirSync(path.join(tmpHome, '.cyberdart/.migrations'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.cyberdart/.migrations/v1.27.0.0.skipped-by-user'), '');
    fs.writeFileSync(path.join(tmpHome, '.cyberdart-brain-remote.txt'), 'https://github.com/x/cyberdart-brain-testuser');
    const r = run();
    expect(r.code).toBe(0);
    expect(r.stderr).toBe('');
  });
...
describe('v1.27.0.0 migration — GitHub host (non-interactive)', () => {
  beforeEach(() => {
    fs.writeFileSync(
      ...
      'https://github.com/testuser/cyberdart-brain-testuser\n'
    );
    fs.writeFileSync(
      path.join(tmpHome, '.cyberdart/config.yaml'),
  ...
  test('renames repo, mvs remote.txt, rewrites config key, writes done', () => {
    ...
    expect(fs.existsSync(path.join(tmpHome, '.cyberdart-brain-remote.txt'))).toBe(false);
    const newUrl = fs.readFileSync(path.join(tmpHome, '.cyberdart-artifacts-remote.txt'), 'utf-8').trim();
    expect(newUrl).toBe('https://github.com/testuser/cyberdart-artifacts-testuser');
    // Config key renamed.
    const cfg = fs.readFileSync(path.join(tmpHome, '.cyberdart/config.yaml'), 'utf-8');
...
describe('v1.27.0.0 migration — interruption resume', () => {
  ...
  test('partial journal: skips already-done steps', () => {
    ...
    const migDir = path.join(tmpHome, '.cyberdart/.migrations');
...
describe('v1.27.0.0 migration — local CLI sources swap (codex Finding #6 ordering)', () => {
  test('add-new before remove-old (verify by call order in log)', () => {
    ...
    fs.mkdirSync(path.join(tmpHome, '.cyberdart/.git'), { recursive: true }); // brain repo present
    makeFakeGh({});
    ...
    const log = fs.readFileSync(path.join(fakeBinDir, 'gbrain-calls.log'), 'utf-8');
    const addIdx = log.indexOf('gbrain sources add cyberdart-artifacts-testuser');
    const removeIdx = log.indexOf('gbrain sources remove cyberdart-brain-testuser');
  ...
  test('add fails → old source stays registered (no silent loss)', () => {
    ...
    fs.mkdirSync(path.join(tmpHome, '.cyberdart/.git'), { recursive: true });
    makeFakeGh({});
