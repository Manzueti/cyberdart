/**
 * cyberdart-config explain_level round-trip + validation tests.
 *
 * Coverage:
...
const BIN_CONFIG = path.join(ROOT, 'bin', 'cyberdart-config');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-cfg-test-'));
});
...
function run(...args: string[]): { stdout: string; stderr: string; status: number } {
  const res = spawnSync(BIN_CONFIG, args, {
    env: { ...process.env, CYBERDART_STATE_DIR: tmpHome },
    encoding: 'utf-8',
    cwd: ROOT,
  });
  return {
...
describe('cyberdart-config explain_level', () => {
