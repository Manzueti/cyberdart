...
function runProfile(): Record<string, string> {
  ...
  const stdout = execSync(`${BIN}/cyberdart-builder-profile`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-profile-'));
});
...
describe('cyberdart-builder-profile', () => {
