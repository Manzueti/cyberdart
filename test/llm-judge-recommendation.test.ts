...
A) Server-side (gbrain ships the smarts)
  ...
  ❌ Cross-repo work; gbrain release tied to cyberdart release; slower V1
B) Client-side (cyberdart ships the smarts) (recommended)
  ✅ Ships entirely in cyberdart — no gbrain release dependency; faster V1
...
describeIfSelected('judgeRecommendation rubric sanity', ['llm-judge-recommendation'], () => {
  testIfSelected('llm-judge-recommendation', async () => {
    ...
    const good5 = await judgeRecommendation(buildAUQ(
      'Recommendation: Choose C because hybrid ships V1 in cyberdart-only without blocking on cross-repo gbrain coordination, and locks the migration path before other agents take a hard dependency.',
    ));
    ...
    const good4 = await judgeRecommendation(buildAUQ(
      'Recommendation: Choose B because client-side composition uses MCP tools that already exist in cyberdart and avoids any gbrain release dependency for V1.',
    ));
