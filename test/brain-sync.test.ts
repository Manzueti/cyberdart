 ...
 *   - bin/cyberdart-config gbrain keys (validation, isolation)
 *   - bin/cyberdart-brain-enqueue (atomicity, skip list, no-op gates)
 *   - bin/cyberdart-jsonl-merge (3-way, ts-sort, hash-fallback)
 *   - bin/cyberdart-brain-sync --once (drain, commit, push, secret-scan, skip-file)
 *   - bin/cyberdart-artifacts-init + --restore round-trip
 *   - bin/cyberdart-brain-uninstall preserves user data
 *   - env isolation (CYBERDART_HOME never bleeds into real ~/.cyberdart/config.yaml)
 *
 * Runs each test against a temp CYBERDART_HOME and a local bare git repo as
...
function run(argv: string[], opts: { env?: Record<string, string>; input?: string } = {}) {
  ...
  const res = spawnSync(full, argv.slice(1), {
    env: { ...process.env, CYBERDART_HOME: tmpHome, ...(opts.env || {}) },
    encoding: 'utf-8',
    input: opts.input,
    cwd: ROOT,
  });
...
afterEach(() => {
  ...
  const remoteFile = path.join(os.homedir(), '.cyberdart-brain-remote.txt');
...
// ---------------------------------------------------------------
describe('cyberdart-config gbrain keys', () => {
  test('default artifacts_sync_mode is off', () => {
    const r = run(['cyberdart-config', 'get', 'artifacts_sync_mode']);
    expect(r.status).toBe(0);
    expect(r.stdout.trim()).toBe('off');
  });

  test('default artifacts_sync_mode_prompted is false', () => {
    const r = run(['cyberdart-config', 'get', 'artifacts_sync_mode_prompted']);
    expect(r.stdout.trim()).toBe('false');
  });

  test('accepts full / artifacts-only / off', () => {
    for (const val of ['full', 'artifacts-only', 'off']) {
      const set = run(['cyberdart-config', 'set', 'artifacts_sync_mode', val]);
      expect(set.status).toBe(0);
      const get = run(['cyberdart-config', 'get', 'artifacts_sync_mode']);
      expect(get.stdout.trim()).toBe(val);
    }
  });

  test('invalid artifacts_sync_mode value warns + defaults', () => {
    const r = run(['cyberdart-config', 'set', 'artifacts_sync_mode', 'bogus']);
    expect(r.stderr).toContain('not recognized');
    const get = run(['cyberdart-config', 'get', 'artifacts_sync_mode']);
    expect(get.stdout.trim()).toBe('off');
  });

  test('CYBERDART_HOME overrides real config dir', () => {
    // Real ~/.cyberdart/config.yaml must not change, regardless of what it
    // already contains on the developer's machine.
    const realConfig = path.join(os.homedir(), '.cyberdart', 'config.yaml');
    const before = fs.existsSync(realConfig) ? fs.readFileSync(realConfig, 'utf-8') : null;

    run(['cyberdart-config', 'set', 'artifacts_sync_mode', 'full']);
    ...
    // Real ~/.cyberdart/config.yaml must not be touched.
...
// Enqueue behavior
// ---------------------------------------------------------------
describe('cyberdart-brain-enqueue', () => {
  test('no-op when feature not initialized', () => {
    const r = run(['cyberdart-brain-enqueue', 'projects/foo/learnings.jsonl']);
    expect(r.status).toBe(0);
  ...
  test('enqueues when mode is full and .git exists', () => {
    fs.mkdirSync(path.join(tmpHome, '.git'), { recursive: true });
    run(['cyberdart-config', 'set', 'artifacts_sync_mode', 'full']);
    run(['cyberdart-brain-enqueue', 'projects/foo/learnings.jsonl']);
  ...
  test('skip list honored', () => {
    ...
    run(['cyberdart-brain-enqueue', 'projects/foo/secret.jsonl']);
    run(['cyberdart-brain-enqueue', 'projects/foo/ok.jsonl']);
    const queue = fs.readFileSync(path.join(tmpHome, '.brain-queue.jsonl'), 'utf-8');
  ...
  test('concurrent enqueues all land (atomic append)', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      procs.push(new Promise<void>((resolve) => {
        const r = spawnSync(path.join(BIN, 'cyberdart-brain-enqueue'), [`file-${i}.jsonl`], {
          env: { ...process.env, CYBERDART_HOME: tmpHome },
          encoding: 'utf-8',
        });
        resolve();
      }));
  ...
  test('no args does not crash', () => {
    const r = run(['cyberdart-brain-enqueue']);
    expect(r.status).toBe(0);
  });
});
...
// JSONL merge driver
// ---------------------------------------------------------------
describe('cyberdart-jsonl-merge', () => {
  test('3-way merge dedups + sorts by ts', () => {
    ...
    const r = run([path.join(BIN, 'cyberdart-jsonl-merge'), base, ours, theirs]);
    expect(r.status).toBe(0);
  ...
  test('falls back to hash order for lines without ts', () => {
    ...
    run([path.join(BIN, 'cyberdart-jsonl-merge'), base, ours, theirs]);
    const lines = fs.readFileSync(ours, 'utf-8').trim().split('\n');
    expect(lines.length).toBe(3);
    // Order is deterministic (sha256 of each line).
    const again = spawnSync(path.join(BIN, 'cyberdart-jsonl-merge'), [base, ours, theirs]);
...
describe('init + sync + restore round-trip', () => {
  test('init creates canonical files + registers drivers', () => {
    const r = run(['cyberdart-artifacts-init', '--remote', bareRemote]);
    expect(r.status).toBe(0);
    ...
    expect(cfg.stdout).toContain('cyberdart-jsonl-merge');
  });

  test('refuses init on different remote', () => {
    run(['cyberdart-artifacts-init', '--remote', bareRemote]);
    const otherRemote = fs.mkdtempSync(path.join(os.tmpdir(), 'brain-other-'));
    spawnSync('git', ['init', '--bare', '-q', '-b', 'main', otherRemote]);
    const r = run(['cyberdart-artifacts-init', '--remote', otherRemote]);
  ...
  test('full sync: init → enqueue → --once → commit pushed', () => {
    ...
    run(['cyberdart-brain-enqueue', 'projects/p/learnings.jsonl']);
    const r = run(['cyberdart-brain-sync', '--once']);
    expect(r.status).toBe(0);
  ...
  test('restore round-trip: writes on machine A visible on machine B', () => {
    ...
    run(['cyberdart-brain-enqueue', 'projects/myproj/learnings.jsonl']);
    run(['cyberdart-brain-sync', '--once']);

    // Machine B (new temp home).
    const machineB = fs.mkdtempSync(path.join(os.tmpdir(), 'brain-machineB-'));
    const r = run(['cyberdart-brain-restore', bareRemote], {
      env: { CYBERDART_HOME: machineB },
    });
    expect(r.status).toBe(0);
...
// ---------------------------------------------------------------
describe('cyberdart-brain-sync secret scan', () => {
  ...
  test('--skip-file unblocks specific file', () => {
    ...
    run(['cyberdart-brain-enqueue', leakPath]);
    run(['cyberdart-brain-sync', '--once']);  // blocked
    run(['cyberdart-brain-sync', '--skip-file', leakPath]);
...
// ---------------------------------------------------------------
describe('cyberdart-brain-uninstall', () => {
  test('removes sync config but preserves learnings/project data', () => {
    ...
    const r = run(['cyberdart-brain-uninstall', '--yes']);
    expect(r.status).toBe(0);
    ...
    // Config key reset.
    const mode = run(['cyberdart-config', 'get', 'artifacts_sync_mode']);
...
// ---------------------------------------------------------------
describe('cyberdart-brain-sync --discover-new', () => {
  test('enqueues new allowlisted files; idempotent on re-run', () => {
    ...
    run(['cyberdart-brain-sync', '--discover-new']);
