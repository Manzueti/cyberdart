/**
 * Unit tests for lib/cyberdart-memory-helpers.ts (Lane 0 foundation).
 *
...
describe("canonicalizeRemote", () => {
  it("strips https scheme and .git suffix", () => {
    expect(canonicalizeRemote("https://github.com/garrytan/cyberdart.git")).toBe("github.com/garrytan/cyberdart");
  });

  it("normalizes git@host:path scp-style remotes", () => {
    expect(canonicalizeRemote("git@github.com:garrytan/cyberdart.git")).toBe("github.com/garrytan/cyberdart");
  });
...
describe("secretScanFile", () => {
  ...
  it("returns scanner=missing or runs gitleaks (env-dependent)", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "cyberdart-test-"));
...
`
    ...
    expect(m!.context_queries[1].glob).toBe("~/.cyberdart/builder-profile.jsonl");
...
describe("withErrorContext", () => {
  ...
  beforeEach(() => {
    savedHome = process.env.CYBERDART_HOME;
    testHome = mkdtempSync(join(tmpdir(), "cyberdart-test-home-"));
    process.env.CYBERDART_HOME = testHome;
  });

  afterAll(() => {
    if (savedHome === undefined) delete process.env.CYBERDART_HOME;
    else process.env.CYBERDART_HOME = savedHome;
  });
...
describe("detectEngineTier", () => {
  ...
  beforeEach(() => {
    savedHome = process.env.CYBERDART_HOME;
    testHome = mkdtempSync(join(tmpdir(), "cyberdart-test-engine-"));
    process.env.CYBERDART_HOME = testHome;
  });
  ...
  it("writes a cache file at ~/.cyberdart/.gbrain-engine-cache.json", () => {
    detectEngineTier();
    const cachePath = join(testHome, ".gbrain-engine-cache.json");
    expect(existsSync(cachePath)).toBe(true);
    const cached = JSON.parse(readFileSync(cachePath, "utf-8"));
    expect(cached.schema_version).toBe(1);
    expect(cached.last_writer).toBe("cyberdart-memory-helpers.detectEngineTier");
  });
