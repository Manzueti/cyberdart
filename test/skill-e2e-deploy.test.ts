...
5. Write a deploy report skeleton to .cyberdart/deploy-reports/report.md showing the
...
Do NOT use AskUserQuestion. Do NOT run gh or fly commands.`,
    ...
    const reportDir = path.join(landDir, '.cyberdart', 'deploy-reports');
...
5. Write the dry-run report to .cyberdart/deploy-reports/dry-run-validation.md
...
Just demonstrate the first-run dry-run output.`,
    ...
    const reportDir = path.join(firstRunDir, '.cyberdart', 'deploy-reports');
    expect(fs.existsSync(reportDir)).toBe(true);
...
(cyberdart-review-read would return NO_REVIEWS).

Simulate what the readiness gate would show:
1. Run cyberdart-review-read equivalent (simulate NO_REVIEWS output)
2. Determine review staleness: Eng Review should be "NOT RUN"
3. Note that Step 3.5a-bis would offer an inline review
4. Write a simulated readiness report to .cyberdart/deploy-reports/readiness-report.md
...
Show what the readiness gate output would look like.`,
    ...
    const reportDir = path.join(reviewDir, '.cyberdart', 'deploy-reports');
    expect(fs.existsSync(reportDir)).toBe(true);
...
1. Create the .cyberdart/canary-reports/ directory structure
2. Write a simulated baseline.json to .cyberdart/canary-reports/baseline.json with the
...
3. Write a simulated canary report to .cyberdart/canary-reports/canary-report.md following
...
Just create the directory structure and report files showing the correct schema.`,
    ...
    expect(fs.existsSync(path.join(canaryDir, '.cyberdart', 'canary-reports'))).toBe(true);
    const reportDir = path.join(canaryDir, '.cyberdart', 'canary-reports');
...
Instead, demonstrate you understand the workflow:
1. Create the .cyberdart/benchmark-reports/ directory structure including baselines/
2. Write a simulated baseline.json to .cyberdart/benchmark-reports/baselines/baseline.json
...
3. Write a simulated benchmark report to .cyberdart/benchmark-reports/benchmark-report.md
...
Just create the files showing the correct schema and report format.`,
    ...
    expect(fs.existsSync(path.join(benchDir, '.cyberdart', 'benchmark-reports'))).toBe(true);
    const baselineDir = path.join(benchDir, '.cyberdart', 'benchmark-reports', 'baselines');
