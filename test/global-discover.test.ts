...
const scriptPath = join(import.meta.dir, "..", "bin", "cyberdart-global-discover.ts");

describe("cyberdart-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/cyberdart-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:garrytan/cyberdart.git");
      const https = normalizeRemoteUrl("https://github.com/garrytan/cyberdart.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/garrytan/cyberdart");
  ...
  describe("codex large session_meta parsing", () => {
    ...
    beforeEach(() => {
      tmpDir = mkdtempSync(join(tmpdir(), "cyberdart-codex-test-"));
