...
describeIfSelected('Learnings E2E', ['learnings-show'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of ['cyberdart-learnings-search', 'cyberdart-learnings-log', 'cyberdart-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // Seed learnings JSONL — slug must match what cyberdart-slug computes.
    // With no git remote, cyberdart-slug falls back to basename(workDir).
...
- Replace any references to ~/.claude/skills/cyberdart/bin/cyberdart-slug with ./bin/cyberdart-slug.
