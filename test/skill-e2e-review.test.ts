...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // cyberdart-review-read reads from ~/.cyberdart/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock cyberdart-review-read script that returns our test data
    ...
    // Write a mock cyberdart-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'cyberdart-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'cyberdart-review-read'), 0o755);
...
Instead of running ~/.claude/skills/cyberdart/bin/cyberdart-review-read, run this mock: ${mockBinDir}/cyberdart-review-read
