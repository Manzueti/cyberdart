 ...
 * via the runner's env option. Today these are advisory — `cyberdart-config`
 * reads `~/.cyberdart/config.yaml`, not env vars, so a contributor with
 * `question_tuning: true` set in their YAML config can still see AUTO_DECIDE
 * masking. The env passthrough is wired so a future cyberdart-config change to
