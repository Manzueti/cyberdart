...
const CODEX_FORMAT_TOUCHFILES: Record<string, string[]> = {
  'codex-plan-ceo-format-mode':      ['.agents/skills/cyberdart-plan-ceo-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
  'codex-plan-ceo-format-approach':  ['.agents/skills/cyberdart-plan-ceo-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
  'codex-plan-eng-format-coverage':  ['.agents/skills/cyberdart-plan-eng-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
  'codex-plan-eng-format-kind':      ['.agents/skills/cyberdart-plan-eng-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
};
...
function setupCodexSkillDir(tmpPrefix: string, skillName: 'plan-ceo-review' | 'plan-eng-review'): { skillDir: string; planDir: string; outFile: string } {
  ...
  // Codex skill lives in .agents/skills/cyberdart-{name}/ per the cyberdart host convention.
  const codexSkillSource = path.join(ROOT, '.agents', 'skills', `cyberdart-${skillName}`);
  const skillDir = path.join(planDir, '.agents', 'skills', `cyberdart-${skillName}`);
...
describeCodex('Codex Plan Format — CEO Mode Selection', () => {
  ...
  testIfSelected('codex-plan-ceo-format-mode', async () => {
    const result = await runCodexSkill({
      ...
      timeoutMs: 300_000,
      cwd: planDir,
      skillName: 'cyberdart-plan-ceo-review',
...
describeCodex('Codex Plan Format — Eng Coverage Issue', () => {
  ...
  testIfSelected('codex-plan-eng-format-coverage', async () => {
    const result = await runCodexSkill({
      ...
      timeoutMs: 300_000,
      cwd: planDir,
      skillName: 'cyberdart-plan-eng-review',
      sandbox: 'workspace-write',
    });
