/**
 * cyberdart-artifacts-url — URL canonicalization helper.
 *
...
const URL_BIN = path.join(ROOT, 'bin', 'cyberdart-artifacts-url');
...
describe('cyberdart-artifacts-url', () => {
  test('--to ssh from canonical https', () => {
    const r = run(['--to', 'ssh', 'https://github.com/garrytan/cyberdart-artifacts-garrytan']);
    expect(r.code).toBe(0);
    expect(r.stdout).toBe('git@github.com:garrytan/cyberdart-artifacts-garrytan.git');
  });

  test('--to ssh from https-with-.git', () => {
    const r = run(['--to', 'ssh', 'https://github.com/garrytan/cyberdart-artifacts-garrytan.git']);
    expect(r.stdout).toBe('git@github.com:garrytan/cyberdart-artifacts-garrytan.git');
  });

  test('--to https is idempotent on https input', () => {
    const r = run(['--to', 'https', 'https://github.com/garrytan/cyberdart-artifacts-garrytan']);
    expect(r.stdout).toBe('https://github.com/garrytan/cyberdart-artifacts-garrytan');
  });

  test('--to https from git@host:owner/repo.git', () => {
    const r = run(['--to', 'https', 'git@github.com:garrytan/cyberdart-artifacts-garrytan.git']);
    expect(r.stdout).toBe('https://github.com/garrytan/cyberdart-artifacts-garrytan');
  });

  test('--to https from ssh:// scheme (gitlab self-hosted style)', () => {
    const r = run(['--to', 'https', 'ssh://git@gitlab.example.org/team/cyberdart-artifacts-team.git']);
    expect(r.stdout).toBe('https://gitlab.example.org/team/cyberdart-artifacts-team');
  });
  ...
  test('--owner-repo extracts the path segment', () => {
    expect(run(['--owner-repo', 'https://github.com/garrytan/cyberdart-artifacts-garrytan']).stdout)
      .toBe('garrytan/cyberdart-artifacts-garrytan');
    expect(run(['--owner-repo', 'git@github.com:team/cyberdart-artifacts-team.git']).stdout)
      .toBe('team/cyberdart-artifacts-team');
  });
  ...
  test('round-trip: https → ssh → https is identity', () => {
    const original = 'https://github.com/garrytan/cyberdart-artifacts-garrytan';
