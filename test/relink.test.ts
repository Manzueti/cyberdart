...
// Every test in this file shells out to cyberdart-config + cyberdart-relink (bash scripts
...
function run(cmd: string, env: Record<string, string> = {}, expectFail = false): string {
  try {
    return execSync(cmd, {
      cwd: ROOT,
      env: { ...process.env, CYBERDART_STATE_DIR: tmpDir, ...env },
      encoding: 'utf-8',
      timeout: 10000,
...
// Create a mock cyberdart install directory with skill subdirs
function setupMockInstall(skills: string[]): void {
  installDir = path.join(tmpDir, 'cyberdart-install');
  skillsDir = path.join(tmpDir, 'skills');
  fs.mkdirSync(installDir, { recursive: true });
  fs.mkdirSync(skillsDir, { recursive: true });

  // Copy the real cyberdart-config and cyberdart-relink to the mock install
  const mockBin = path.join(installDir, 'bin');
  fs.mkdirSync(mockBin, { recursive: true });
  fs.copyFileSync(path.join(BIN, 'cyberdart-config'), path.join(mockBin, 'cyberdart-config'));
  fs.chmodSync(path.join(mockBin, 'cyberdart-config'), 0o755);
  if (fs.existsSync(path.join(BIN, 'cyberdart-relink'))) {
    fs.copyFileSync(path.join(BIN, 'cyberdart-relink'), path.join(mockBin, 'cyberdart-relink'));
    fs.chmodSync(path.join(mockBin, 'cyberdart-relink'), 0o755);
  }
  if (fs.existsSync(path.join(BIN, 'cyberdart-patch-names'))) {
    fs.copyFileSync(path.join(BIN, 'cyberdart-patch-names'), path.join(mockBin, 'cyberdart-patch-names'));
    fs.chmodSync(path.join(mockBin, 'cyberdart-patch-names'), 0o755);
  }
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-relink-test-'));
});
...
describe('cyberdart-relink (#578)', () => {
  // Test 11: prefixed symlinks when skill_prefix=true
  test('creates cyberdart-* symlinks when skill_prefix=true', () => {
    setupMockInstall(['qa', 'ship', 'review']);
    // Set config to prefix mode (pass install/skills env so auto-relink uses mock install)
    run(`${path.join(installDir, 'bin', 'cyberdart-config')} set skill_prefix true`, {
      CYBERDART_INSTALL_DIR: installDir,
      CYBERDART_SKILLS_DIR: skillsDir,
    });
    // Run relink with env pointing to the mock install
    const output = run(`${path.join(installDir, 'bin', 'cyberdart-relink')}`, {
      CYBERDART_INSTALL_DIR: installDir,
      CYBERDART_SKILLS_DIR: skillsDir,
    });
    // Verify cyberdart-* symlinks exist
    expect(fs.existsSync(path.join(skillsDir, 'cyberdart-qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'cyberdart-ship'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'cyberdart-review'))).toBe(true);
    expect(output).toContain('cyberdart-');
  });

  // Test 12: flat symlinks when skill_prefix=false
  test('creates flat symlinks when skill_prefix=false', () => {
    setupMockInstall(['qa', 'ship', 'review']);
    run(`${path.join(installDir, 'bin', 'cyberdart-config')} set skill_prefix false`, {
      CYBERDART_INSTALL_DIR: installDir,
      CYBERDART_SKILLS_DIR: skillsDir,
    });
    const output = run(`${path.join(installDir, 'bin', 'cyberdart-relink')}`, {
  ...
  // e.g., `qa -> cyberdart/qa` gets discovered as "cyberdart-qa", not "qa".
  ...
  test('unprefixed skills are real directories with SKILL.md symlinks, not dir symlinks', () => {
    ...
    run(`${path.join(installDir, 'bin', 'cyberdart-relink')}`, {
      CYBERDART_INSTALL_DIR: installDir,
      CYBERDART_SKILLS_DIR: skillsDir,
    });
  ...
  test('prefixed skills are real directories with SKILL.md symlinks, not dir symlinks', () => {
    ...
    for (const skill of ['cyberdart-qa', 'cyberdart-ship']) {
      const skillPath = path.join(skillsDir, skill);
      const skillMdPath = path.join(skillPath, 'SKILL.md');
      expect(fs.lstatSync(skillPath).isDirectory()).toBe(true);
      expect(fs.lstatSync(skillPath).isSymbolicLink()).toBe(false);
  ...
  // FIRST INSTALL: --no-prefix must create ONLY flat names, zero cyberdart-* pollution
  test('first install --no-prefix: only flat names exist, zero cyberdart-* entries', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'cyberdart-upgrade']);
    ...
    // Expected: qa, ship, review, plan-ceo-review, cyberdart-upgrade (its real name)
    expect(entries.sort()).toEqual(['cyberdart-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    // No cyberdart-qa, cyberdart-ship, cyberdart-review, cyberdart-plan-ceo-review
    const leaked = entries.filter(e => e.startsWith('cyberdart-') && e !== 'cyberdart-upgrade');
    expect(leaked).toEqual([]);
  });

  // FIRST INSTALL: --prefix must create ONLY cyberdart-* names, zero flat-name pollution
  test('first install --prefix: only cyberdart-* entries exist, zero flat names', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'cyberdart-upgrade']);
    run(`${path.join(installDir, 'bin', 'cyberdart-config')} set skill_prefix true`, {
      CYBERDART_INSTALL_DIR: installDir,
      CYBERDART_SKILLS_DIR: skillsDir,
    ...
    const entries = fs.readdirSync(skillsDir);
    // Expected: cyberdart-qa, cyberdart-ship, cyberdart-review, cyberdart-plan-ceo-review, cyberdart-upgrade
    expect(entries.sort()).toEqual([
      'cyberdart-plan-ceo-review', 'cyberdart-qa', 'cyberdart-review', 'cyberdart-ship', 'cyberdart-upgrade',
    ]);
    // No unprefixed qa, ship, review, plan-ceo-review
    const leaked = entries.filter(e => !e.startsWith('cyberdart-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('non-TTY first install defaults to flat names via relink', () => {
    ...
    // cyberdart-relink reads config; on fresh install config returns empty → defaults to false
    run(`${path.join(installDir, 'bin', 'cyberdart-relink')}`, {
      CYBERDART_INSTALL_DIR: installDir,
      CYBERDART_SKILLS_DIR: skillsDir,
    });
  ...
  // SWITCH: prefix → no-prefix must clean up ALL cyberdart-* entries
  test('switching prefix to no-prefix removes all cyberdart-* entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'cyberdart-upgrade']);
    ...
    expect(entries.filter(e => !e.startsWith('cyberdart-'))).toEqual([]);
    ...
    // Only flat names + cyberdart-upgrade (its real name)
    expect(entries.sort()).toEqual(['cyberdart-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    const leaked = entries.filter(e => e.startsWith('cyberdart-') && e !== 'cyberdart-upgrade');
    expect(leaked).toEqual([]);
  });
  ...
  test('switching no-prefix to prefix removes all flat entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'cyberdart-upgrade']);
    ...
    let entries = fs.readdirSync(skillsDir);
    expect(entries.filter(e => e.startsWith('cyberdart-') && e !== 'cyberdart-upgrade')).toEqual([]);
    ...
    entries = fs.readdirSync(skillsDir);
    // Only cyberdart-* names
    expect(entries.sort()).toEqual([
      'cyberdart-qa', 'cyberdart-review', 'cyberdart-ship', 'cyberdart-upgrade',
    ]);
    const leaked = entries.filter(e => !e.startsWith('cyberdart-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('cleans up stale symlinks from opposite mode', () => {
    ...
    expect(fs.existsSync(path.join(skillsDir, 'qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'cyberdart-qa'))).toBe(false);
  });

  // Test 14: error when install dir missing
  test('prints error when install dir missing', () => {
    const output = run(`${BIN}/cyberdart-relink`, {
      CYBERDART_INSTALL_DIR: '/nonexistent/path/cyberdart',
      CYBERDART_SKILLS_DIR: '/nonexistent/path/skills',
    }, true);
    expect(output).toContain('setup');
  });

  // Test: cyberdart-upgrade does NOT get double-prefixed
  test('does not double-prefix cyberdart-upgrade directory', () => {
    setupMockInstall(['qa', 'ship', 'cyberdart-upgrade']);
    run(`${path.join(installDir, 'bin', 'cyberdart-config')} set skill_prefix true`, {
      CYBERDART_INSTALL_DIR: installDir,
      CYBERDART_SKILLS_DIR: skillsDir,
    });
    ...
    // cyberdart-upgrade should keep its name, NOT become cyberdart-cyberdart-upgrade
    expect(fs.existsSync(path.join(skillsDir, 'cyberdart-upgrade'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'cyberdart-cyberdart-upgrade'))).toBe(false);
    // Regular skills still get prefixed
    expect(fs.existsSync(path.join(skillsDir, 'cyberdart-qa'))).toBe(true);
  });

  // Test 15: cyberdart-config set skill_prefix triggers relink
  test('cyberdart-config set skill_prefix triggers relink', () => {
    setupMockInstall(['qa', 'ship']);
    // Run cyberdart-config set which should auto-trigger relink
    run(`${path.join(installDir, 'bin', 'cyberdart-config')} set skill_prefix true`, {
      CYBERDART_INSTALL_DIR: installDir,
      CYBERDART_SKILLS_DIR: skillsDir,
    });
...
describe('upgrade migrations', () => {
  const MIGRATIONS_DIR = path.join(ROOT, 'cyberdart-upgrade', 'migrations');
  ...
  test('v0.15.2.0 migration runs cyberdart-relink', () => {
    const content = fs.readFileSync(path.join(MIGRATIONS_DIR, 'v0.15.2.0.sh'), 'utf-8');
    expect(content).toContain('cyberdart-relink');
  });
  ...
  test('v0.15.2.0 migration fixes stale directory symlinks', () => {
    ...
    run(`${path.join(installDir, 'bin', 'cyberdart-config')} set skill_prefix false`, {
      CYBERDART_SETUP_RUNNING: '1',
    });
    // Verify old state: symlinks
    expect(fs.lstatSync(path.join(skillsDir, 'qa')).isSymbolicLink()).toBe(true);

    // Run the migration (it calls cyberdart-relink internally)
...
describe('cyberdart-patch-names (#620/#578)', () => {
  ...
  test('prefix=true patches name: field in SKILL.md', () => {
    ...
    // Verify name: field is patched with cyberdart- prefix
    expect(readSkillName(path.join(installDir, 'qa'))).toBe('cyberdart-qa');
    expect(readSkillName(path.join(installDir, 'ship'))).toBe('cyberdart-ship');
    expect(readSkillName(path.join(installDir, 'review'))).toBe('cyberdart-review');
  });
  ...
  test('cyberdart-upgrade name: not double-prefixed', () => {
    setupMockInstall(['qa', 'cyberdart-upgrade']);
    run(`${path.join(installDir, 'bin', 'cyberdart-config')} set skill_prefix true`, {
      CYBERDART_INSTALL_DIR: installDir,
      CYBERDART_SKILLS_DIR: skillsDir,
    });
    ...
    // cyberdart-upgrade should keep its name, NOT become cyberdart-cyberdart-upgrade
    expect(readSkillName(path.join(installDir, 'cyberdart-upgrade'))).toBe('cyberdart-upgrade');
