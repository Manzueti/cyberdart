...
const ANCHOR = '<!-- CYBERDART-THROUGHPUT-PLACEHOLDER -->';
const PENDING = 'CYBERDART-THROUGHPUT-PENDING';
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-readme-test-'));
...
describe('update-readme-throughput script', () => {
  test('happy path: JSON present → anchor replaced with number', () => {
    fs.writeFileSync(tmpReadme, `cyberdart hero: ${ANCHOR} 2013 pro-rata.\n`);
  ...
  test('anchor already replaced: script is a no-op', () => {
    fs.writeFileSync(tmpReadme, 'cyberdart hero: 7.0× already set.\n');
    ...
    const updated = fs.readFileSync(tmpReadme, 'utf-8');
    expect(updated).toBe('cyberdart hero: 7.0× already set.\n');
  });
});
...
describe('CI gate: committed README must not contain PENDING marker', () => {
  ...
  test('real README.md does not contain CYBERDART-THROUGHPUT-PENDING', () => {
