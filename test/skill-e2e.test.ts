...
// Eval result collector — accumulates test results, writes to ~/.cyberdart-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.cyberdart', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/cyberdart/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with cyberdart installed globally, the fallback path
    // ~/.claude/skills/cyberdart/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a cyberdart skill. The session preamble detected _SESSIONS=4 (the user has 4 cyberdart windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.cyberdart/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.cyberdart', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.cyberdart/projects/', async () => {
...
// --- cyberdart-upgrade E2E ---

describeIfSelected('cyberdart-upgrade E2E', ['cyberdart-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-remote-'));
    ...
    // Create mock cyberdart install directory (local-git type)
    const mockCyberDart = path.join(upgradeDir, '.claude', 'skills', 'cyberdart');
    fs.mkdirSync(mockCyberDart, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockCyberDart);
    run('git', ['config', 'user.email', 'test@test.com'], mockCyberDart);
    run('git', ['config', 'user.name', 'Test'], mockCyberDart);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockCyberDart);

    // Write old version files
    fs.writeFileSync(path.join(mockCyberDart, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockCyberDart, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockCyberDart, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockCyberDart);
    run('git', ['commit', '-m', 'initial'], mockCyberDart);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockCyberDart);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockCyberDart, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockCyberDart, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockCyberDart);
    run('git', ['commit', '-m', 'release 0.6.0'], mockCyberDart);
    run('git', ['push', 'origin', 'HEAD:main'], mockCyberDart);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockCyberDart);

    // Copy cyberdart-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'cyberdart-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'cyberdart-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'cyberdart-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('cyberdart-upgrade-happy-path', async () => {
    const mockCyberDart = path.join(upgradeDir, '.claude', 'skills', 'cyberdart');
    const result = await runSkillTest({
      prompt: `Read cyberdart-upgrade/SKILL.md for the upgrade workflow.

You are running /cyberdart-upgrade standalone. The cyberdart installation is at ./.claude/skills/cyberdart (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/cyberdart — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'cyberdart-upgrade-happy-path',
      runId,
    });

    logCost('/cyberdart-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockCyberDart, 'VERSION'), 'utf-8').trim();
    const output = result.output || '';
    ...
    recordE2E('/cyberdart-upgrade happy path', 'cyberdart-upgrade E2E', result, {
