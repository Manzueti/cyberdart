 ...
 *      ~/.cyberdart/ artifacts (eureka, learning, ceo-plan, design-doc, retro,
 *      builder-profile)
 *   2. Run cyberdart-memory-ingest --probe → verify counts match disk
 *   3. Run cyberdart-memory-ingest --bulk → verify state file gets written +
 *      session_id dedup works on re-run (idempotency)
 *   4. Run cyberdart-gbrain-sync --dry-run → verify all 3 stages preview
 *   5. Run cyberdart-brain-context-load against a real V1 skill manifest
 ...
 * may or may not be available in CI. We assert on side effects cyberdart
...
const INGEST = join(REPO_ROOT, "bin", "cyberdart-memory-ingest.ts");
const SYNC = join(REPO_ROOT, "bin", "cyberdart-gbrain-sync.ts");
const CONTEXT = join(REPO_ROOT, "bin", "cyberdart-brain-context-load.ts");

function makeFixtureHome(): string {
  return mkdtempSync(join(tmpdir(), "cyberdart-e2e-pipeline-"));
}

function setupFixture(home: string): { cyberdartHome: string; counts: Record<string, number> } {
  const cyberdartHome = join(home, ".cyberdart");
  mkdirSync(cyberdartHome, { recursive: true });
  mkdirSync(join(cyberdartHome, "analytics"), { recursive: true });
  mkdirSync(join(cyberdartHome, "projects", "test-repo", "ceo-plans"), { recursive: true });
  mkdirSync(join(cyberdartHome, "projects", "test-repo", "retros"), { recursive: true });
  ...
  // cyberdart artifacts
  writeFileSync(join(cyberdartHome, "analytics", "eureka.jsonl"), '{"insight":"boil the lake"}\n', "utf-8");
  writeFileSync(join(cyberdartHome, "builder-profile.jsonl"), '{"date":"2026-05-01","mode":"startup"}\n', "utf-8");
  writeFileSync(join(cyberdartHome, "projects", "test-repo", "learnings.jsonl"), '{"key":"a","insight":"b","confidence":8}\n', "utf-8");
  writeFileSync(join(cyberdartHome, "projects", "test-repo", "timeline.jsonl"), '{"skill":"office-hours","event":"completed"}\n', "utf-8");
  writeFileSync(join(cyberdartHome, "projects", "test-repo", "ceo-plans", "2026-05-01-test.md"), "# CEO Plan: Test\n\nbody\n", "utf-8");
  writeFileSync(join(cyberdartHome, "projects", "test-repo", "garrytan-main-design-20260501-090000.md"), "# Design: Test\n", "utf-8");
  writeFileSync(join(cyberdartHome, "projects", "test-repo", "retros", "2026-05-01-week.md"), "# Retro\n", "utf-8");
...
describe("V1 memory ingest pipeline E2E", () => {
  it("--probe finds all 9 fixture files across all source types", () => {
    ...
    const { cyberdartHome, counts } = setupFixture(home);
    const env = { HOME: home, CYBERDART_HOME: cyberdartHome, CYBERDART_MEMORY_INGEST_NO_WRITE: "1" };
  ...
  it("--incremental writes a state file with schema_version: 1 + last_writer", () => {
    const home = makeFixtureHome();
    const { cyberdartHome } = setupFixture(home);
    const env = { HOME: home, CYBERDART_HOME: cyberdartHome, CYBERDART_MEMORY_INGEST_NO_WRITE: "1" };

    runBun(INGEST, ["--incremental", "--quiet"], env);

    const statePath = join(cyberdartHome, ".transcript-ingest-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("cyberdart-memory-ingest");
  ...
  it("--incremental is idempotent — re-run reports 0 changes", () => {
    ...
    // First run
    runBun(INGEST, ["--incremental", "--quiet"], env);
    const stateAfterFirst = readFileSync(join(cyberdartHome, ".transcript-ingest-state.json"), "utf-8");
