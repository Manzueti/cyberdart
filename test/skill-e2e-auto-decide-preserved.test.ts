 ...
 * auto-deciding a specific question via `cyberdart-question-preference --write
 ...
 *
 * Set up:
 *   - tmpDir as CYBERDART_HOME (isolated state, doesn't touch the user's
 *     real ~/.cyberdart)
...
describeE2E('AUTO_DECIDE opt-in preserved under Conductor flags (periodic)', () => {
  test('user-opted-in question still auto-decides when AskUserQuestion is --disallowedTools', async () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-auto-decide-'));
    try {
      // 1. Bootstrap the tmp CYBERDART_HOME with question_tuning=true.
      const configBin = path.join(ROOT, 'bin', 'cyberdart-config');
      const setRes = spawnSync(configBin, ['set', 'question_tuning', 'true'], {
        env: { ...process.env, CYBERDART_HOME: tmpHome },
        encoding: 'utf-8',
      });
      if (setRes.status !== 0) {
        throw new Error(`cyberdart-config set failed: ${setRes.stderr || setRes.stdout}`);
      }
      ...
      const slugBin = path.join(ROOT, 'bin', 'cyberdart-slug');
      const slugRes = spawnSync(slugBin, [], {
        cwd: ROOT,
        env: { ...process.env, CYBERDART_HOME: tmpHome },
        encoding: 'utf-8',
      });
      // cyberdart-slug emits `eval`-able shell exports like `SLUG=garrytan-cyberdart`.
      ...
      if (writeRes.status !== 0) {
        throw new Error(`cyberdart-question-preference --write failed: ${writeRes.stderr || writeRes.stdout}`);
      }
