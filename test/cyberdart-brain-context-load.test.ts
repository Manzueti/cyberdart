/**
 * Unit tests for bin/cyberdart-brain-context-load.ts (Lane C).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "cyberdart-brain-context-load.ts");
...
describe("cyberdart-brain-context-load CLI", () => {
  it("--help exits 0 with usage", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: cyberdart-brain-context-load");
...
describe("cyberdart-brain-context-load — manifest dispatch", () => {
  ...
  it("uses skill manifest when --skill-file points at a valid SKILL.md", () => {
    const dir = mkdtempSync(join(tmpdir(), "cyberdart-bcl-"));
...
describe("cyberdart-brain-context-load — graceful gbrain absence", () => {
