/**
 * cyberdart-artifacts-init — provider-selection + brain-admin-hookup tests.
 *
 * Mirrors the cyberdart-brain-init-gh-mock.test.ts pattern: install fake gh /
 ...
 * ~/.cyberdart-artifacts-remote.txt, (c) prints the "Send this to your brain
...
const INIT_BIN = path.join(ROOT, 'bin', 'cyberdart-artifacts-init');
...
function makeFakeGh(opts: { authStatus?: 'ok' | 'fail'; repoCreate?: 'success' | 'already-exists' | 'fail'; webUrl?: string } = {}) {
  ...
  const webUrl = opts.webUrl ?? `https://github.com/testuser/cyberdart-artifacts-testuser`;
...
function makeFakeGlab(opts: { authStatus?: 'ok' | 'fail'; repoCreate?: 'success' | 'fail'; webUrl?: string } = {}) {
  const authStatus = opts.authStatus ?? 'ok';
  const repoCreate = opts.repoCreate ?? 'success';
  const webUrl = opts.webUrl ?? 'https://gitlab.com/testuser/cyberdart-artifacts-testuser';
  const script = `#!/bin/bash
...
describe('cyberdart-artifacts-init provider selection', () => {
  test('--host github invokes gh repo create with cyberdart-artifacts-$USER', () => {
    makeFakeGh({});
    ...
    expect(createCall).toContain('cyberdart-artifacts-testuser');
  ...
  test('neither authed → falls through to manual URL paste', () => {
    // No gh, no glab fakes.
    const r = run([], { input: 'https://github.com/testuser/cyberdart-artifacts-testuser\n' });
    expect(r.status).toBe(0);
    expect(r.stderr).toContain('Neither gh nor glab');
  });
});

describe('cyberdart-artifacts-init canonical URL storage (codex Finding #10)', () => {
  test('stores HTTPS URL canonical in ~/.cyberdart-artifacts-remote.txt', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/cyberdart-artifacts-testuser' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const remoteFile = path.join(tmpHome, '.cyberdart-artifacts-remote.txt');
    ...
    expect(stored).toBe('https://github.com/testuser/cyberdart-artifacts-testuser');
  });

  test('strips trailing .git from gh repo view output', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/cyberdart-artifacts-testuser.git' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const stored = fs.readFileSync(path.join(tmpHome, '.cyberdart-artifacts-remote.txt'), 'utf-8').trim();
    expect(stored).toBe('https://github.com/testuser/cyberdart-artifacts-testuser');
  });
  ...
  test('configures git origin with SSH form (derived from canonical HTTPS)', () => {
    ...
    expect(remote.stdout.trim()).toBe('git@github.com:testuser/cyberdart-artifacts-testuser.git');
  });
});

describe('cyberdart-artifacts-init brain-admin hookup printout (codex Finding #3)', () => {
  ...
  test('--url-form-supported true prints the one-liner with --url', () => {
    ...
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('Send this to your brain admin');
    expect(r.stdout).toContain('gbrain sources add cyberdart-artifacts-testuser --url');
  ...
  test('the gbrain command line uses canonical HTTPS, not SSH', () => {
    ...
    expect(gbrainLine).toContain('https://github.com/testuser/cyberdart-artifacts-testuser');
...
describe('cyberdart-artifacts-init idempotency', () => {
  test('--remote <url> bypasses provider selection entirely', () => {
    makeFakeGh({});
    const r = run(['--remote', 'https://github.com/testuser/cyberdart-artifacts-testuser']);
    expect(r.status).toBe(0);
  ...
  test('re-run with same --remote is safe (no conflict error)', () => {
    makeFakeGh({});
    const url = 'https://github.com/testuser/cyberdart-artifacts-testuser';
    run(['--remote', url]);
  ...
  test('re-run with DIFFERENT --remote exits 1 with conflict message', () => {
    makeFakeGh({});
    run(['--remote', 'https://github.com/testuser/cyberdart-artifacts-testuser']);
