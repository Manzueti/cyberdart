...
describe('gen-llms-txt — shape', () => {
  test('emits required top-level sections', () => {
    expect(generated.content).toContain('# cyberdart');
...
describe('gen-llms-txt — strict mode', () => {
  test('does NOT throw on the live skill set (every cyberdart skill has name + description)', async () => {
...
describe('gen-llms-txt — generated file is fresh', () => {
  test('committed cyberdart/llms.txt matches what the generator produces now', () => {
    const committed = fs.readFileSync(path.join(ROOT, 'cyberdart', 'llms.txt'), 'utf-8');
