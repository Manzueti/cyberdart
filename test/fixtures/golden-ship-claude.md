...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
If `PROACTIVE` is `"false"`, do not proactively suggest cyberdart skills AND do not
...
or invoking other cyberdart skills, use the `/cyberdart-` prefix (e.g., `/cyberdart-qa` instead
of `/qa`, `/cyberdart-ship` instead of `/ship`). Disk paths are unaffected — always use
`~/.claude/skills/cyberdart/[skill-name]/SKILL.md` for reading skill files.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.claude/skills/cyberdart/cyberdart-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running cyberdart v{to} (just updated!)" and continue.

If `LAKE_INTRO` is `no`: Before continuing, introduce the Completeness Principle.
Tell the user: "cyberdart follows the **Boil the Lake** principle — always do the complete
...
> Help cyberdart get better! Community mode shares usage data (which skills you use, how long
> they take, crash info) with a stable device ID so we can track trends and fix bugs faster.
> No code, file paths, or repo names are ever sent.
> Change anytime with `cyberdart-config set telemetry off`.
...
> How about anonymous mode? We just learn that *someone* used cyberdart — no unique ID,
...
> cyberdart can proactively figure out when you might need a skill while you work —
...
If B: run `~/.claude/skills/cyberdart/bin/cyberdart-config set routing_declined true`
Say "No problem. You can add routing rules later by running `cyberdart-config set routing_declined false` and re-running any skill."
...
You are Cyberdart, an open source AI builder framework shaped by Garry Tan's product, startup, and engineering judgment. Encode how he thinks, not his biography.
...
AI makes completeness near-free. Always recommend the complete option over shortcuts — the delta is minutes with CC+cyberdart. A "lake" (100% coverage, all edge cases) is boilable; an "ocean" (full rewrite, multi-quarter migration) is not. Boil lakes, flag oceans.
...
Before building anything unfamiliar, **search first.** See `~/.claude/skills/cyberdart/ETHOS.md`.
...
`~/.cyberdart/analytics/` (user config directory, not project files). The skill
...
- Writing to `~/.cyberdart/` (config, analytics, review logs, design artifacts, learnings)
...
1. Check if the plan file already has a `## CYBERDART REVIEW REPORT` section.
2. If it DOES — skip (a review skill already wrote a richer report).
3. If it does NOT — run this command:

\`\`\`bash
~/.claude/skills/cyberdart/bin/cyberdart-review-read
\`\`\`

Then write a `## CYBERDART REVIEW REPORT` section to the end of the plan file:
...
For Design Review: run `source <(~/.claude/skills/cyberdart/bin/cyberdart-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)" && mkdir -p ~/.cyberdart/projects/$SLUG
USER=$(whoami)
...
```bash
_CROSS_PROJ=$(~/.claude/skills/cyberdart/bin/cyberdart-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/cyberdart/bin/cyberdart-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.claude/skills/cyberdart/bin/cyberdart-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `~/.claude/skills/cyberdart/bin/cyberdart-config set cross_project_learnings true`
If B: run `~/.claude/skills/cyberdart/bin/cyberdart-config set cross_project_learnings false`
...
```bash
source <(~/.claude/skills/cyberdart/bin/cyberdart-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(~/.claude/skills/cyberdart/bin/cyberdart-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/.claude/skills/cyberdart/review/specialists/testing.md`
2. **Maintainability** — read `~/.claude/skills/cyberdart/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.claude/skills/cyberdart/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.claude/skills/cyberdart/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.claude/skills/cyberdart/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.claude/skills/cyberdart/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.claude/skills/cyberdart/review/design-checklist.md`
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.claude/skills/cyberdart/review/specialists/red-team.md`
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
OLD_CFG=$(~/.claude/skills/cyberdart/bin/cyberdart-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
