...
description: |
  ...
  is ready, asks about deploying, wants to push code up, or asks to create a PR. (cyberdart)
user-invocable: true
...
CYBERDART_ROOT="$HOME/.factory/skills/cyberdart"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.factory/skills/cyberdart" ] && CYBERDART_ROOT="$_ROOT/.factory/skills/cyberdart"
CYBERDART_BIN="$CYBERDART_ROOT/bin"
CYBERDART_BROWSE="$CYBERDART_ROOT/browse/dist"
CYBERDART_DESIGN="$CYBERDART_ROOT/design/dist"
_UPD=$($CYBERDART_BIN/cyberdart-update-check 2>/dev/null || .factory/skills/cyberdart/bin/cyberdart-update-check 2>/dev/null || true)
...
_PROACTIVE=$($CYBERDART_BIN/cyberdart-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.cyberdart/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$($CYBERDART_BIN/cyberdart-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <($CYBERDART_BIN/cyberdart-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.cyberdart/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$($CYBERDART_BIN/cyberdart-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f ~/.cyberdart/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$($CYBERDART_BIN/cyberdart-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$($CYBERDART_BIN/cyberdart-config get question_tuning 2>/dev/null || echo "false")
echo "QUESTION_TUNING: $_QUESTION_TUNING"
mkdir -p ~/.cyberdart/analytics
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
for _PF in $(find ~/.cyberdart/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "$CYBERDART_BIN/cyberdart-telemetry-log" ]; then
      $CYBERDART_BIN/cyberdart-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$($CYBERDART_BIN/cyberdart-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${CYBERDART_HOME:-$HOME/.cyberdart}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    $CYBERDART_BIN/cyberdart-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
$CYBERDART_BIN/cyberdart-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$($CYBERDART_BIN/cyberdart-config get routing_declined 2>/dev/null || echo "false")
echo "HAS_ROUTING: $_HAS_ROUTING"
echo "ROUTING_DECLINED: $_ROUTING_DECLINED"
_VENDORED="no"
if [ -d ".factory/skills/cyberdart" ] && [ ! -L ".factory/skills/cyberdart" ]; then
  if [ -f ".factory/skills/cyberdart/VERSION" ] || [ -d ".factory/skills/cyberdart/.git" ]; then
    _VENDORED="yes"
  fi
fi
echo "VENDORED_CYBERDART: $_VENDORED"
echo "MODEL_OVERLAY: claude"
_CHECKPOINT_MODE=$($CYBERDART_BIN/cyberdart-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$($CYBERDART_BIN/cyberdart-config get checkpoint_push 2>/dev/null || echo "false")
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/cyberdart-*` names. Disk paths stay `$CYBERDART_ROOT/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `$CYBERDART_ROOT/cyberdart-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).
...
- Missing `$CYBERDART_ROOT/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `$CYBERDART_BIN/cyberdart-config set checkpoint_mode continuous`. Always touch marker.
- Missing `$CYBERDART_ROOT/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `$CYBERDART_BIN/cyberdart-config set explain_level terse`.
...
If A: run `$CYBERDART_BIN/cyberdart-config set telemetry community`
...
If B→A: run `$CYBERDART_BIN/cyberdart-config set telemetry anonymous`
If B→B: run `$CYBERDART_BIN/cyberdart-config set telemetry off`
...
If A: run `$CYBERDART_BIN/cyberdart-config set proactive true`
If B: run `$CYBERDART_BIN/cyberdart-config set proactive false`
...
If B: run `$CYBERDART_BIN/cyberdart-config set routing_declined true` and say they can re-enable with `cyberdart-config set routing_declined false`.
...
> This project has cyberdart vendored in `.factory/skills/cyberdart/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .factory/skills/cyberdart/`
2. Run `echo '.factory/skills/cyberdart/' >> .gitignore`
3. Run `$CYBERDART_BIN/cyberdart-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate cyberdart from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd $CYBERDART_ROOT && ./setup --team`"
...
_BRAIN_SYNC_BIN="$CYBERDART_BIN/cyberdart-brain-sync"
_BRAIN_CONFIG_BIN="$CYBERDART_BIN/cyberdart-config"
...
```bash
"$CYBERDART_BIN/cyberdart-brain-sync" --discover-new 2>/dev/null || true
"$CYBERDART_BIN/cyberdart-brain-sync" --once 2>/dev/null || true
```
...
```bash
eval "$($CYBERDART_BIN/cyberdart-slug 2>/dev/null)"
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `$CYBERDART_BIN/cyberdart-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.

After answer, log best-effort:
```bash
$CYBERDART_BIN/cyberdart-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$CYBERDART_BIN/cyberdart-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `$CYBERDART_ROOT/ETHOS.md`.
- **Layer 1** (tried and true) — don't reinvent. **Layer 2** (new and popular) — scrutinize. **Layer 3** (first principles) — prize above all.

**Eureka:** When first-principles reasoning contradicts conventional wisdom, name it and log:
```bash
jq -n --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "SKILL_NAME" --arg branch "$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:$ts,skill:$skill,branch:$branch,insight:$insight}' >> ~/.cyberdart/analytics/eureka.jsonl 2>/dev/null || true
```
...
```bash
$CYBERDART_BIN/cyberdart-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
$CYBERDART_ROOT/bin/cyberdart-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x $CYBERDART_ROOT/bin/cyberdart-telemetry-log ]; then
  $CYBERDART_ROOT/bin/cyberdart-telemetry-log \
...
In plan mode before ExitPlanMode: if the plan file lacks `## CYBERDART REVIEW REPORT`, run `$CYBERDART_ROOT/bin/cyberdart-review-read` and append the standard runs/status/findings table. With `NO_REVIEWS` or empty, append a 5-row placeholder with verdict "NO REVIEWS YET — run `/autoplan`". If a richer report exists, skip.
...
```bash
$CYBERDART_ROOT/bin/cyberdart-review-read
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`cyberdart-config set skip_eng_review true\` (the "don't bother me" setting).
...
For Design Review: run `source <($CYBERDART_ROOT/bin/cyberdart-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
[ -f .cyberdart/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.cyberdart/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.cyberdart/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.cyberdart/no-test-bootstrap` and re-run." Continue without tests.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.factory/skills/cyberdart/review/TODOS-format.md`).
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** cyberdart /ship on <date>" \
...
```bash
eval "$($CYBERDART_ROOT/bin/cyberdart-slug 2>/dev/null)" && mkdir -p ~/.cyberdart/projects/$SLUG
USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```

Write to `~/.cyberdart/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
# Compute project slug for ~/.cyberdart/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.cyberdart/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".cyberdart/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (`## CYBERDART REVIEW REPORT`)
...
```bash
_CROSS_PROJ=$($CYBERDART_BIN/cyberdart-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  $CYBERDART_BIN/cyberdart-learnings-search --limit 10 --query "release ship version changelog merge pr" --cross-project 2>/dev/null || true
else
  $CYBERDART_BIN/cyberdart-learnings-search --limit 10 --query "release ship version changelog merge pr" 2>/dev/null || true
fi
```

If `CROSS_PROJECT` is `unset` (first time): Use AskUserQuestion:

> cyberdart can search learnings from your other projects on this machine to find
...
If A: run `$CYBERDART_BIN/cyberdart-config set cross_project_learnings true`
If B: run `$CYBERDART_BIN/cyberdart-config set cross_project_learnings false`
...
This makes the compounding visible. The user should see that cyberdart is getting
...
1. Read `.factory/skills/cyberdart/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
Check if the diff touches frontend files using `cyberdart-diff-scope`:

```bash
source <($CYBERDART_BIN/cyberdart-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.factory/skills/cyberdart/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
$CYBERDART_BIN/cyberdart-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <($CYBERDART_BIN/cyberdart-diff-scope <base> 2>/dev/null) || true
...
```bash
$CYBERDART_BIN/cyberdart-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `$CYBERDART_ROOT/review/specialists/testing.md`
2. **Maintainability** — read `$CYBERDART_ROOT/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `$CYBERDART_ROOT/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `$CYBERDART_ROOT/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `$CYBERDART_ROOT/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `$CYBERDART_ROOT/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `$CYBERDART_ROOT/review/design-checklist.md`

### Adaptive gating

After scope-based selection, apply adaptive gating based on specialist hit rates:

For each conditional specialist that passed scope gating, check the `cyberdart-specialist-stats` output above:
...
```bash
$CYBERDART_BIN/cyberdart-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `$CYBERDART_ROOT/review/specialists/red-team.md`
...
```bash
$CYBERDART_ROOT/bin/cyberdart-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
> You are classifying Greptile review comments for a /ship workflow. Read `.factory/skills/cyberdart/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
OLD_CFG=$($CYBERDART_ROOT/bin/cyberdart-config get codex_reviews 2>/dev/null || true)
...
```bash
$CYBERDART_ROOT/bin/cyberdart-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$CYBERDART_BIN/cyberdart-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```bash
$CYBERDART_ROOT/bin/cyberdart-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
3. **Queue-aware version pick (workspace-aware ship, v1.6.4.0+).** Call `bin/cyberdart-next-version` to see what's already claimed by open PRs + active sibling Conductor worktrees, then render the queue state to the user:

   ```bash
   QUEUE_JSON=$(bun run bin/cyberdart-next-version \
     --base <base> \
     --bump "$BUMP_LEVEL" \
...
Read `.factory/skills/cyberdart/review/TODOS-format.md` for the canonical format reference.

**1. Check if TODOS.md exists** in the repository root.

**If TODOS.md does not exist:** Use AskUserQuestion:
- Message: "Cyberdart recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
```bash
# Export [cyberdart-context] blocks from all WIP commits on this branch.
# This file becomes input to the CHANGELOG entry and may inform PR body context.
mkdir -p "$(git rev-parse --show-toplevel)/.cyberdart"
git log <base>..HEAD --grep="^WIP:" --format="%H%n%B%n---END---" > \
  "$(git rev-parse --show-toplevel)/.cyberdart/wip-context-before-squash.md" 2>/dev/null || true
```
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.factory/skills/cyberdart/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
**Always update the PR title to start with `v$NEW_VERSION`.** PR titles use the workspace-aware format `v<NEW_VERSION> <type>: <summary>` — version ALWAYS first, no exceptions, no "custom title kept intentionally" escape hatch. The shared helper `bin/cyberdart-pr-title-rewrite.sh` is the single source of truth for the rule.

1. Read the current title: `CURRENT=$(gh pr view --json title -q .title)` (or `glab mr view -F json | jq -r .title`).
2. Compute the corrected title: `NEW_TITLE=$($CYBERDART_ROOT/bin/cyberdart-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
# (See Step 19 idempotency block + bin/cyberdart-pr-title-rewrite.sh for the rule.)
...
Append to `~/.cyberdart/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.cyberdart/projects/$SLUG/$BRANCH-reviews.jsonl
```
