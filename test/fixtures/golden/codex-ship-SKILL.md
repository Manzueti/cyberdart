...
CYBERDART_ROOT="$HOME/.codex/skills/cyberdart"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/cyberdart" ] && CYBERDART_ROOT="$_ROOT/.agents/skills/cyberdart"
CYBERDART_BIN="$CYBERDART_ROOT/bin"
CYBERDART_BROWSE="$CYBERDART_ROOT/browse/dist"
CYBERDART_DESIGN="$CYBERDART_ROOT/design/dist"
_UPD=$($CYBERDART_BIN/cyberdart-update-check 2>/dev/null || .agents/skills/cyberdart/bin/cyberdart-update-check 2>/dev/null || true)
...
_VENDORED="no"
if [ -d ".agents/skills/cyberdart" ] && [ ! -L ".agents/skills/cyberdart" ]; then
  if [ -f ".agents/skills/cyberdart/VERSION" ] || [ -d ".agents/skills/cyberdart/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
> This project has cyberdart vendored in `.agents/skills/cyberdart/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .agents/skills/cyberdart/`
2. Run `echo '.agents/skills/cyberdart/' >> .gitignore`
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.agents/skills/cyberdart/review/TODOS-format.md`).
...
1. Read `.agents/skills/cyberdart/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.agents/skills/cyberdart/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
> You are classifying Greptile review comments for a /ship workflow. Read `.agents/skills/cyberdart/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
Read `.agents/skills/cyberdart/review/TODOS-format.md` for the canonical format reference.
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.agents/skills/cyberdart/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
