...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-ship-work-'));
...
describeIfSelected('cyberdart-upgrade E2E', ['cyberdart-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('cyberdart-upgrade-happy-path', async () => {
    const mockCyberDart = path.join(upgradeDir, '.claude', 'skills', 'cyberdart');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/cyberdart — use that exact path.`,
    ...
    recordE2E(evalCollector, '/cyberdart-upgrade happy path', 'cyberdart-upgrade E2E', result, {
