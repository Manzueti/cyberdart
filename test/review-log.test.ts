...
function run(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/cyberdart-review-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-revlog-'));
  // cyberdart-review-log uses cyberdart-slug which needs a git repo — create the projects dir
