...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testConcurrentIfSelected('operational-learning', async () => {
    ...
    const cyberdartHome = path.join(opDir, '.cyberdart-home');
    ...
    for (const script of ['cyberdart-learnings-log', 'cyberdart-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // cyberdart-learnings-log will create the project dir automatically via cyberdart-slug
...
\`\`\`bash
CYBERDART_HOME="${cyberdartHome}" ${binDir}/cyberdart-learnings-log '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
Log the operational learning now. Then say what you logged.`,
    ...
    const projectsDir = path.join(cyberdartHome, 'projects');
