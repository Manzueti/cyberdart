 ...
 * for /rewind, silently shadowing the cyberdart /checkpoint skill. Users
 ...
 *
 * This file is the "never again" test. It enumerates every cyberdart skill name
 * from every SKILL.md.tmpl file in the repo and cross-checks against a
 * per-host list of known built-in slash commands. If any cyberdart skill name
 ...
 * below. If a cyberdart skill needs to coexist with a built-in anyway (e.g.,
...
const KNOWN_BUILTINS: Record<string, string[]> = {
  ...
  // invoke codex from cyberdart, we don't install skills INTO codex the same
...
interface CyberDartSkill {
  name: string;
  templatePath: string;
}

function enumerateCyberDartSkills(): CyberDartSkill[] {
  const skills: CyberDartSkill[] = [];
...
describe('skill-collision-sentinel', () => {
  const skills = enumerateCyberDartSkills();
  ...
  test('no duplicate skill names within cyberdart', () => {
  ...
  // Hard check: no cyberdart skill name collides with a known host built-in
    ...
    test(`no skill name collides with a ${host} built-in (or has written justification)`, () => {
      ...
      if (collisions.length > 0) {
        const msg = collisions.map(c =>
          ...
          `    Fix: rename the cyberdart skill (precedent: /checkpoint → /context-save+/context-restore),\n` +
  ...
  test('KNOWN_COLLISIONS_TOLERATED entries are all still active collisions', () => {
    ...
    for (const name of Object.keys(KNOWN_COLLISIONS_TOLERATED)) {
      if (!skillNames.has(name)) {
        stale.push(`  "${name}" is in KNOWN_COLLISIONS_TOLERATED but no cyberdart skill has that name — remove the exception`);
