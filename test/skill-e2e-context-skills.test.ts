...
function setupWorkdir(suffix: string): { workDir: string; cyberdartHome: string; slug: string } {
  ...
  for (const script of [
    'cyberdart-timeline-log', 'cyberdart-timeline-read', 'cyberdart-slug',
    'cyberdart-learnings-log', 'cyberdart-learnings-search',
    'cyberdart-update-check', 'cyberdart-config', 'cyberdart-repo-mode',
  ]) {
...
Environment:
- Use CYBERDART_HOME="${cyberdartHome}" for all cyberdart bin scripts.
...
`);
  ...
  return { workDir, cyberdartHome, slug };
}

// Helper: seed a saved-context file into the storage dir.
function seedSave(cyberdartHome: string, slug: string, filename: string, frontmatter: Record<string, string>, body: string) {
  const dir = path.join(cyberdartHome, 'projects', slug, 'checkpoints');
...
describeIfSelected('Context Skills E2E (live-fire)', [
  ...
  testConcurrentIfSelected('context-save-routing', async () => {
    const { workDir, cyberdartHome, slug } = setupWorkdir('routing');

    // Prompt pattern: the slash command + explicit "invoke via Skill tool"
    // instruction. The CYBERDART_HOME / ./bin bash setup that used to be in
    ...
    const checkpointDir = path.join(cyberdartHome, 'projects', slug, 'checkpoints');
  ...
  testConcurrentIfSelected('context-save-then-restore-roundtrip', async () => {
    const { workDir, cyberdartHome, slug } = setupWorkdir('roundtrip');
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('context-restore-fragment-match', async () => {
    const { workDir, cyberdartHome, slug } = setupWorkdir('fragment');

    // Seed three saves with distinct titles.
    seedSave(cyberdartHome, slug, '20260101-120000-alpha-feature.md',
    ...
    seedSave(cyberdartHome, slug, '20260202-120000-middle-payments.md',
    ...
    seedSave(cyberdartHome, slug, '20260303-120000-omega-release.md',
  ...
  testConcurrentIfSelected('context-restore-empty-state', async () => {
    const { workDir, cyberdartHome, slug } = setupWorkdir('empty');
  ...
  testConcurrentIfSelected('context-restore-list-delegates', async () => {
    const { workDir, cyberdartHome, slug } = setupWorkdir('delegates');
    seedSave(cyberdartHome, slug, '20260101-120000-seed.md',
  ...
  testConcurrentIfSelected('context-restore-legacy-compat', async () => {
    const { workDir, cyberdartHome, slug } = setupWorkdir('legacy');
    ...
    // exactly this reason.
    seedSave(cyberdartHome, slug, '20260301-120000-legacy-pre-rename-work.md',
      {
        status: 'in-progress',
  ...
  testConcurrentIfSelected('context-save-list-current-branch', async () => {
    const { workDir, cyberdartHome, slug } = setupWorkdir('list-current');

    // Seed 3 files on 3 different branches. Current branch is "main".
    seedSave(cyberdartHome, slug, '20260101-120000-main-work.md',
      { status: 'in-progress', branch: 'main', timestamp: '2026-01-01T12:00:00Z' },
      '## Working on: main work LISTCURR_MAIN_TOKEN\n');
    seedSave(cyberdartHome, slug, '20260202-120000-feat-alpha.md',
      { status: 'in-progress', branch: 'feat/alpha', timestamp: '2026-02-02T12:00:00Z' },
      '## Working on: alpha LISTCURR_ALPHA_TOKEN\n');
    seedSave(cyberdartHome, slug, '20260303-120000-feat-beta.md',
  ...
  testConcurrentIfSelected('context-save-list-all-branches', async () => {
    const { workDir, cyberdartHome, slug } = setupWorkdir('list-all');
