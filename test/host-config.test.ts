...
describe('validateHostConfig', () => {
  function makeValid(): HostConfig {
    return {
      ...
      cliCommand: 'testcli',
      globalRoot: '.test/skills/cyberdart',
      localSkillRoot: '.test/skills/cyberdart',
      hostSubdir: '.test',
      usesEnvVars: true,
  ...
  test('paths with $ and ~ are valid', () => {
    const c = makeValid();
    c.globalRoot = '$HOME/.test/skills/cyberdart';
    c.localSkillRoot = '~/.test/skills/cyberdart';
    expect(validateHostConfig(c)).toEqual([]);
  });
...
describe('validateAllConfigs', () => {
  ...
  test('duplicate hostSubdir detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.claude', globalRoot: '.dup/skills/cyberdart' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
    expect(errors.some(e => e.includes('Duplicate hostSubdir'))).toBe(true);
  });

  test('duplicate globalRoot detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.dup', globalRoot: '.claude/skills/cyberdart' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
...
describe('HOST_PATHS derivation from configs', () => {
  test('Claude uses literal home paths (no env vars)', () => {
    expect(HOST_PATHS.claude.skillRoot).toBe('~/.claude/skills/cyberdart');
    expect(HOST_PATHS.claude.binDir).toBe('~/.claude/skills/cyberdart/bin');
    expect(HOST_PATHS.claude.browseDir).toBe('~/.claude/skills/cyberdart/browse/dist');
    expect(HOST_PATHS.claude.designDir).toBe('~/.claude/skills/cyberdart/design/dist');
  });

  test('Codex uses $CYBERDART_ROOT env vars', () => {
    expect(HOST_PATHS.codex.skillRoot).toBe('$CYBERDART_ROOT');
    expect(HOST_PATHS.codex.binDir).toBe('$CYBERDART_BIN');
    expect(HOST_PATHS.codex.browseDir).toBe('$CYBERDART_BROWSE');
    expect(HOST_PATHS.codex.designDir).toBe('$CYBERDART_DESIGN');
  });

  test('every host with usesEnvVars=true gets env var paths', () => {
    for (const config of ALL_HOST_CONFIGS) {
      if (config.usesEnvVars) {
        expect(HOST_PATHS[config.name].skillRoot).toBe('$CYBERDART_ROOT');
        expect(HOST_PATHS[config.name].binDir).toBe('$CYBERDART_BIN');
      }
    }
  });
...
describe('host-config-export.ts CLI', () => {
  ...
  test('get returns string field', () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toBe('.codex/skills/cyberdart');
  });
...
describe('golden-file regression', () => {
  ...
  test('Codex ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'cyberdart-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
  ...
  test('Factory ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.factory', 'skills', 'cyberdart-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
});
...
describe('host config correctness', () => {
  ...
  test('codex has sidecar config', () => {
    ...
    expect(codex.sidecar!.path).toBe('.agents/skills/cyberdart');
  });
