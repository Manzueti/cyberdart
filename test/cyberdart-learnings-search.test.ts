...
const BIN = path.join(ROOT, 'bin', 'cyberdart-learnings-search');

const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-search-test-'));
const tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-search-cwd-'));
// cyberdart-slug derives slug from git remote (none here) → falls back to basename of cwd.
...
describe('cyberdart-learnings-search token-OR query semantics', () => {
