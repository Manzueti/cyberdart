/**
 * cyberdart-gbrain-detect — gbrain_mcp_mode + cyberdart_artifacts_remote tests.
 *
...
const DETECT_BIN = path.join(ROOT, 'bin', 'cyberdart-gbrain-detect');
...
function runDetect(extraEnv: Record<string, string> = {}): { code: number; json: any; stderr: string } {
  ...
  const r = spawnSync(DETECT_BIN, [], {
    env: {
      ...
      HOME: tmpHome,
      CYBERDART_HOME: path.join(tmpHome, '.cyberdart'),
      ...extraEnv,
    },
    encoding: 'utf-8',
  });
...
describe('cyberdart_artifacts_remote', () => {
  test('reads ~/.cyberdart-artifacts-remote.txt when present', () => {
    fs.writeFileSync(
      path.join(tmpHome, '.cyberdart-artifacts-remote.txt'),
      'https://github.com/garrytan/cyberdart-artifacts-garrytan\n'
    );
    expect(runDetect().json.cyberdart_artifacts_remote).toBe(
      'https://github.com/garrytan/cyberdart-artifacts-garrytan'
    );
  });

  test('migration-window fallback: reads ~/.cyberdart-brain-remote.txt if artifacts file is missing', () => {
    fs.writeFileSync(
      path.join(tmpHome, '.cyberdart-brain-remote.txt'),
      'git@github.com:garrytan/cyberdart-brain-garrytan.git\n'
    );
    expect(runDetect().json.cyberdart_artifacts_remote).toBe(
      'git@github.com:garrytan/cyberdart-brain-garrytan.git'
    );
  ...
  test('artifacts file wins over brain file when both exist', () => {
    ...
    expect(runDetect().json.cyberdart_artifacts_remote).toBe('https://github.com/x/new');
  });
