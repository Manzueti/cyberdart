/**
 * cyberdart-gbrain-detect + cyberdart-gbrain-install — Slice 2 of /setup-gbrain.
 *
 * Detect: state-reporter JSON with presence, version, config, doctor health,
 * and cyberdart-brain-sync mode. Pure introspection, no side effects.
 *
...
const DETECT = path.join(ROOT, 'bin', 'cyberdart-gbrain-detect');
const INSTALL = path.join(ROOT, 'bin', 'cyberdart-gbrain-install');
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gbrain-detect-cyberdart-'));
...
describe('cyberdart-gbrain-detect', () => {
  test('emits valid JSON even when nothing is configured', () => {
    ...
    try {
      ...
      expect(j.cyberdart_brain_sync_mode).toBe('off');
      expect(j.cyberdart_brain_git).toBe(false);
    } finally {
      fs.rmSync(emptyBin, { recursive: true, force: true });
    }
  });

  test('reports cyberdart_brain_git: true when CYBERDART_HOME has a .git dir', () => {
    fs.mkdirSync(path.join(tmpHome, '.git'));
    const emptyBin = fs.mkdtempSync(path.join(os.tmpdir(), 'empty-bin-'));
    try {
      const r = run(DETECT, [], { env: { PATH: `${emptyBin}:${SAFE_PATH}` } });
      const j = JSON.parse(r.stdout);
      expect(j.cyberdart_brain_git).toBe(true);
    } finally {
      fs.rmSync(emptyBin, { recursive: true, force: true });
    }
  });
...
describe('cyberdart-gbrain-install D5 detect-first', () => {
...
describe('cyberdart-gbrain-install D19 PATH-shadow validation', () => {
...
describe('cyberdart-gbrain-install argument handling', () => {
  test('--help prints usage without exiting non-zero', () => {
    const r = run(INSTALL, ['--help']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('cyberdart-gbrain-install');
  });
