...
const SETTINGS_HOOK = path.join(ROOT, 'bin', 'cyberdart-settings-hook');
const SESSION_UPDATE = path.join(ROOT, 'bin', 'cyberdart-session-update');
const TEAM_INIT = path.join(ROOT, 'bin', 'cyberdart-team-init');

function mkTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-team-test-'));
}
...
describe('cyberdart-settings-hook', () => {
  let tmpDir: string;
  ...
  test('add creates settings.json if missing', () => {
    const result = run(`${SETTINGS_HOOK} add /path/to/cyberdart-session-update`, {
      env: { CYBERDART_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
    expect(settings.hooks.SessionStart[0].hooks[0].command).toBe('/path/to/cyberdart-session-update');
  });
  ...
  test('add deduplicates (running twice does not double-add)', () => {
    run(`${SETTINGS_HOOK} add /path/to/cyberdart-session-update`, {
      env: { CYBERDART_SETTINGS_FILE: settingsFile },
    });
    run(`${SETTINGS_HOOK} add /path/to/cyberdart-session-update`, {
      env: { CYBERDART_SETTINGS_FILE: settingsFile },
  ...
  test('remove removes the hook', () => {
    ...
    const result = run(`${SETTINGS_HOOK} remove /path/to/cyberdart-session-update`, {
      env: { CYBERDART_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
  ...
  test('remove preserves other hooks', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [
          { hooks: [{ type: 'command', command: '/path/to/cyberdart-session-update' }] },
          { hooks: [{ type: 'command', command: '/other/hook' }] },
        ],
      },
    }, null, 2));
    run(`${SETTINGS_HOOK} remove /path/to/cyberdart-session-update`, {
      env: { CYBERDART_SETTINGS_FILE: settingsFile },
    });
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
...
describe('cyberdart-session-update', () => {
  let tmpDir: string;
  let cyberdartDir: string;
  let stateDir: string;

  beforeEach(() => {
    tmpDir = mkTmpDir();
    cyberdartDir = path.join(tmpDir, 'cyberdart');
    stateDir = path.join(tmpDir, 'state');
    fs.mkdirSync(cyberdartDir, { recursive: true });
    fs.mkdirSync(stateDir, { recursive: true });

    // Init a git repo to pass the .git guard
    execSync('git init', { cwd: cyberdartDir });
    execSync('git commit --allow-empty -m "init"', { cwd: cyberdartDir });
    fs.writeFileSync(path.join(cyberdartDir, 'VERSION'), '0.1.0');

    // Create a minimal cyberdart-config that returns auto_upgrade=true
    const binDir = path.join(cyberdartDir, 'bin');
    fs.mkdirSync(binDir, { recursive: true });
    fs.writeFileSync(path.join(binDir, 'cyberdart-config'), '#!/bin/bash\necho "true"');
    fs.chmodSync(path.join(binDir, 'cyberdart-config'), 0o755);
  });
  ...
  test('exits 0 when .git is missing', () => {
    fs.rmSync(path.join(cyberdartDir, '.git'), { recursive: true });
    const result = run(SESSION_UPDATE, {
      env: { CYBERDART_DIR: cyberdartDir, CYBERDART_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });

  test('exits 0 when auto_upgrade is not true', () => {
    // Override cyberdart-config to return false
    fs.writeFileSync(path.join(cyberdartDir, 'bin', 'cyberdart-config'), '#!/bin/bash\necho "false"');
    const result = run(SESSION_UPDATE, {
      env: { CYBERDART_DIR: cyberdartDir, CYBERDART_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  ...
  test('always exits 0 (non-fatal)', () => {
    ...
    const result = run(SESSION_UPDATE, {
      env: { CYBERDART_DIR: '/nonexistent/path', CYBERDART_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });
});

describe('cyberdart-team-init', () => {
  let tmpDir: string;
  ...
  test('optional: creates CLAUDE.md with recommended section', () => {
    ...
    expect(claude).toContain('## cyberdart (recommended)');
  ...
  test('required: creates CLAUDE.md with required section', () => {
    ...
    expect(result.exitCode).toBe(0);
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    expect(claude).toContain('## cyberdart (REQUIRED');
    expect(claude).toContain('CYBERDART_MISSING');
  });

  test('required: creates enforcement hook', () => {
    run(`${TEAM_INIT} required`, { cwd: tmpDir });
    const hookPath = path.join(tmpDir, '.claude', 'hooks', 'check-cyberdart.sh');
    expect(fs.existsSync(hookPath)).toBe(true);
    const hook = fs.readFileSync(hookPath, 'utf-8');
    expect(hook).toContain('BLOCKED: cyberdart is not installed');
  ...
  test('required: creates project settings.json with PreToolUse hook', () => {
    ...
    expect(settings.hooks.PreToolUse[0].hooks[0].command).toContain('check-cyberdart');
  });
  ...
  test('idempotent: running twice does not duplicate CLAUDE.md section', () => {
    ...
    run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    const matches = claude.match(/## cyberdart/g);
    expect(matches).toHaveLength(1);
  });

  test('removes vendored copy when present', () => {
    // Create a fake vendored cyberdart with VERSION file
    const vendoredDir = path.join(tmpDir, '.claude', 'skills', 'cyberdart');
    ...
    // Track it in git
    execSync('git add .claude/skills/cyberdart/', { cwd: tmpDir });
    execSync('git commit -m "add vendored cyberdart"', { cwd: tmpDir });

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Found vendored cyberdart copy');
    ...
    expect(gitignore).toContain('.claude/skills/cyberdart/');
  });

  test('skips when no vendored copy present', () => {
    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored cyberdart copy');
  });

  test('skips when .claude/skills/cyberdart is a symlink', () => {
    ...
    fs.symlinkSync(targetDir, path.join(skillsDir, 'cyberdart'));

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored cyberdart copy');
    // Symlink should still exist
    expect(fs.lstatSync(path.join(skillsDir, 'cyberdart')).isSymbolicLink()).toBe(true);
  ...
  test('does not duplicate .gitignore entry on re-run', () => {
    ...
    const gitignore = fs.readFileSync(path.join(tmpDir, '.gitignore'), 'utf-8');
    const matches = gitignore.match(/\.claude\/skills\/cyberdart\//g);
    expect(matches).toHaveLength(1);
  });
});
...
describe('setup --team / --no-team / -q', () => {
  ...
  test(
    ...
    () => {
      ...
      // The key test is that the "Skill naming:" prompt and "cyberdart ready" messages are suppressed
      expect(result.stdout).not.toContain('Skill naming:');
      expect(result.stdout).not.toContain('cyberdart ready');
    },
    180_000,
  );
