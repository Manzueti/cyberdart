/**
 * Unit tests for bin/cyberdart-memory-ingest.ts (Lane A).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "cyberdart-memory-ingest.ts");

// ── Helpers ────────────────────────────────────────────────────────────────

function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "cyberdart-memory-ingest-"));
}
...
describe("cyberdart-memory-ingest CLI", () => {
  it("prints usage on --help and exits 0", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: cyberdart-memory-ingest");
  ...
  it("--probe on empty home reports 0 files", () => {
    ...
    const r = runScript(["--probe"], { HOME: home, CYBERDART_HOME: cyberdartHome });
    expect(r.exitCode).toBe(0);
  ...
  it("--probe finds cyberdart artifacts (learnings, eureka, ceo-plan)", () => {
    const home = makeTestHome();
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(join(cyberdartHome, "analytics"), { recursive: true });
    mkdirSync(join(cyberdartHome, "projects", "foo-bar", "ceo-plans"), { recursive: true });

    writeFileSync(join(cyberdartHome, "analytics", "eureka.jsonl"), '{"insight":"lake first"}\n');
    writeFileSync(join(cyberdartHome, "projects", "foo-bar", "learnings.jsonl"), '{"key":"a","insight":"b"}\n');
    writeFileSync(join(cyberdartHome, "projects", "foo-bar", "ceo-plans", "2026-05-01-test.md"), "# Plan\n");
  ...
  it("--sources filter limits the walk to specific types", () => {
    ...
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(join(cyberdartHome, "analytics"), { recursive: true });
    mkdirSync(join(cyberdartHome, "projects", "foo", "ceo-plans"), { recursive: true });

    writeFileSync(join(cyberdartHome, "analytics", "eureka.jsonl"), '{"insight":"x"}\n');
    writeFileSync(join(cyberdartHome, "projects", "foo", "learnings.jsonl"), '{"key":"a"}\n');

    const r = runScript(["--probe", "--sources", "eureka"], { HOME: home, CYBERDART_HOME: cyberdartHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
    expect(r.stdout).toContain("eureka");
...
describe("cyberdart-memory-ingest state file", () => {
  it("--incremental on empty home creates state file with schema_version: 1", () => {
    const home = makeTestHome();
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(cyberdartHome, { recursive: true });
    const r = runScript(["--incremental", "--quiet"], { HOME: home, CYBERDART_HOME: cyberdartHome });
    expect(r.exitCode).toBe(0);
...
describe("cyberdart-memory-ingest security: untrusted cwd cannot trigger shell substitution", () => {
  it("does not invoke /bin/sh when a transcript record contains $() in cwd", () => {
    ...
    const markerDir = mkdtempSync(join(tmpdir(), "cyberdart-mi-cwd-marker-"));
    ...
    const r = runScript(["--incremental", "--quiet"], {
      ...
      CYBERDART_MEMORY_INGEST_NO_WRITE: "1",
    });
...
describe("internal: parseTranscriptJsonl + buildTranscriptPage shape", () => {
  it("parses a Claude Code JSONL session", async () => {
    const dir = mkdtempSync(join(tmpdir(), "cyberdart-mi-parse-"));
    ...
    const r = runScript(["--probe"], { HOME: home, CYBERDART_HOME: join(home, ".cyberdart") });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
...
describe("cyberdart-memory-ingest --limit", () => {
...
describe("cyberdart-memory-ingest writer (gbrain v0.20+ batch `import` interface)", () => {
