...
describeIfSelected('PlanTune E2E', ['plan-tune-inspect'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of [
      'cyberdart-slug',
      'cyberdart-config',
      'cyberdart-question-log',
      'cyberdart-question-preference',
      'cyberdart-developer-profile',
      'cyberdart-builder-profile',
    ]) {
    ...
    // cyberdart-developer-profile --derive imports from scripts/ — copy those too.
    ...
    const cfgDir = path.join(cyberdartHome);
...
IMPORTANT:
- Use CYBERDART_HOME="${cyberdartHome}" as an environment variable for all bin calls.
- Replace any ~/.claude/skills/cyberdart/bin/ references with ./bin/ (relative path).
- Replace any ~/.claude/skills/cyberdart/scripts/ references with ./scripts/.
