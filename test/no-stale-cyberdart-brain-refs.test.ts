/**
 * Regression: no stale `cyberdart-brain-init`, `gbrain_sync_mode`, or
 * `~/.cyberdart-brain-remote.txt` references survive the v1.27.0.0 rename.
 *
 ...
 *
 * Allowlist: the migration script (`cyberdart-upgrade/migrations/v1.27.0.0.sh`)
...
const ALLOWLIST = [
  ...
  'cyberdart-upgrade/migrations/v1.27.0.0.sh',
  // Older migration scripts — historical references; these document past state.
  'cyberdart-upgrade/migrations/v1.17.0.0.sh',
  ...
  'test/cyberdart-upgrade-migration-v1_17_0_0.test.ts',
  ...
  // The bin/cyberdart-config comment explicitly preserves the rename note.
  'bin/cyberdart-config',
  // Detect script's "renamed in v1.27.0.0" comment + brain-remote-fallback path.
  'bin/cyberdart-gbrain-detect',
  ...
  'bin/cyberdart-brain-restore',
  'bin/cyberdart-gbrain-source-wireup',
  'bin/cyberdart-brain-uninstall',
  ...
  // cyberdart-upgrade.test.ts may exercise old migration behavior.
  'test/cyberdart-upgrade.test.ts',
  // This test itself references the patterns to grep for.
  'test/no-stale-cyberdart-brain-refs.test.ts',
...
describe('no stale cyberdart-brain refs (v1.27.0.0 rename)', () => {
