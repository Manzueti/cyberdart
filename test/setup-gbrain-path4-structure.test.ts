...
//     by unit tests for cyberdart-gbrain-mcp-verify and cyberdart-artifacts-init.
//   - The structural prose is the source of regressions for AUQ pacing
//     (the failure mode the cyberdart repo has tracked since v1.26.x:
...
describe('setup-gbrain Path 4 (Remote MCP) — structural contract', () => {
  ...
  test('Step 4c invokes cyberdart-gbrain-mcp-verify and STOPs on failure', () => {
    expect(tmpl).toContain('cyberdart-gbrain-mcp-verify');
  ...
  test('Step 7 calls cyberdart-artifacts-init with --url-form-supported flag', () => {
    expect(tmpl).toMatch(/cyberdart-artifacts-init.*--url-form-supported/);
  });
