...
const MIGRATION = path.join(ROOT, 'cyberdart-upgrade', 'migrations', 'v1.1.3.0.sh');
...
function setupFakeCyberDartRoot(tmpHome: string): string {
  // A real target that the cyberdart symlink can resolve into.
  const cyberdartDir = path.join(tmpHome, '.claude', 'skills', 'cyberdart');
  fs.mkdirSync(path.join(cyberdartDir, 'checkpoint'), { recursive: true });
  fs.writeFileSync(path.join(cyberdartDir, 'checkpoint', 'SKILL.md'), '# fake cyberdart checkpoint\n');
  return cyberdartDir;
}
...
describe('migration v1.1.3.0 — checkpoint ownership guard', () => {
  ...
  beforeEach(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-migration-ownership-'));
  });
  ...
  test('scenario A: directory symlink into cyberdart → removed', () => {
    setupFakeCyberDartRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const cyberdartCheckpoint = path.join(skillsDir, 'cyberdart', 'checkpoint');
    const topLevel = path.join(skillsDir, 'checkpoint');
    fs.symlinkSync(cyberdartCheckpoint, topLevel);

    const result = runMigration(tmpHome);
    expect(result.exitCode).toBe(0);
    expect(fs.existsSync(topLevel)).toBe(false);
    // Also removes the cyberdart-owned inner copy (Shape 2 cleanup).
    expect(fs.existsSync(cyberdartCheckpoint)).toBe(false);
    expect(result.stdout).toContain('Removed stale /checkpoint symlink');
  });

  test('scenario B: directory with SKILL.md symlinked into cyberdart → removed', () => {
    setupFakeCyberDartRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const cyberdartSKILL = path.join(skillsDir, 'cyberdart', 'checkpoint', 'SKILL.md');
    const topLevel = path.join(skillsDir, 'checkpoint');
    fs.mkdirSync(topLevel, { recursive: true });
    fs.symlinkSync(cyberdartSKILL, path.join(topLevel, 'SKILL.md'));
  ...
  test('scenario C: user-owned regular directory with custom content → preserved', () => {
    ...
    expect(result.stdout).toContain('not a cyberdart-owned install');
  });

  test('scenario D: symlink pointing outside cyberdart → preserved', () => {
    setupFakeCyberDartRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const topLevel = path.join(skillsDir, 'checkpoint');
    ...
    expect(result.stdout).toContain('Leaving');
    expect(result.stdout).toContain('outside cyberdart');
  });

  test('scenario E: nothing to do → no-op exit 0 (idempotent)', () => {
    // No checkpoint install at all. First run: nothing removed.
    setupFakeCyberDartRoot(tmpHome);
    // Delete the inner cyberdart/checkpoint to simulate post-upgrade state.
    fs.rmSync(path.join(tmpHome, '.claude', 'skills', 'cyberdart', 'checkpoint'), { recursive: true, force: true });
  ...
  test('scenario F: cyberdart not installed → no-op exit 0', () => {
    // No ~/.claude/skills/cyberdart/ at all. Also no checkpoint install.
  ...
  test('scenario G: SKILL.md is a symlink pointing outside cyberdart → preserved', () => {
    setupFakeCyberDartRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const topLevel = path.join(skillsDir, 'checkpoint');
    fs.mkdirSync(topLevel, { recursive: true });
    // A directory containing SKILL.md that's a symlink pointing outside cyberdart.
