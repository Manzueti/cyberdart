...
const BIN = path.join(ROOT, 'bin', 'cyberdart-paths');
...
// helper from inside a bash block (`eval "$(~/.claude/skills/cyberdart/bin/cyberdart-paths)"`)
...
function run(env: Record<string, string | undefined>): Record<string, string> {
  ...
  if (result.status !== 0) {
    throw new Error(`cyberdart-paths failed (status ${result.status}): ${result.stderr}`);
  }
...
describe('cyberdart-paths', () => {
  test('CYBERDART_HOME wins over CLAUDE_PLUGIN_DATA and HOME', () => {
    const got = run({
      CYBERDART_HOME: '/tmp/explicit-state',
      CLAUDE_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.CYBERDART_STATE_ROOT).toBe('/tmp/explicit-state');
  });

  test('CLAUDE_PLUGIN_DATA wins over HOME when CYBERDART_HOME unset', () => {
    const got = run({
      CLAUDE_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.CYBERDART_STATE_ROOT).toBe('/tmp/plugin-data');
  });

  test('HOME-derived state root when CYBERDART_HOME and CLAUDE_PLUGIN_DATA unset', () => {
    const got = run({ HOME: '/tmp/myhome' });
    expect(got.CYBERDART_STATE_ROOT).toBe('/tmp/myhome/.cyberdart');
  });
  ...
  test('CWD fallback when HOME also unset (container env)', () => {
    ...
    const got = run({ HOME: '' });
    expect(got.CYBERDART_STATE_ROOT).toBe('.cyberdart');
  });

  test('PLAN_ROOT chain: CYBERDART_PLAN_DIR > CLAUDE_PLANS_DIR > HOME > CWD', () => {
    expect(run({ CYBERDART_PLAN_DIR: '/tmp/explicit', HOME: '/h' }).PLAN_ROOT).toBe('/tmp/explicit');
