/**
 * cyberdart-upgrade/migrations/v1.0.0.0.sh — writing style migration.
 *
 * Coverage:
...
const MIGRATION = path.join(ROOT, 'cyberdart-upgrade', 'migrations', 'v1.0.0.0.sh');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-mig-test-'));
});
