...
describe('findBudgetRegressions', () => {
  ...
  test('respects CYBERDART_BUDGET_RATIO env override', () => {
    const c = makeComparison([
      makeDelta('a', { Bash: 10 }, { Bash: 16 }), // 1.6×
    ]);
    const prev = process.env.CYBERDART_BUDGET_RATIO;
    try {
      process.env.CYBERDART_BUDGET_RATIO = '1.5';
      expect(findBudgetRegressions(c).length).toBe(1);
      process.env.CYBERDART_BUDGET_RATIO = '2.0';
      expect(findBudgetRegressions(c).length).toBe(0);
    } finally {
      if (prev === undefined) delete process.env.CYBERDART_BUDGET_RATIO;
      else process.env.CYBERDART_BUDGET_RATIO = prev;
    }
  });
...
describe('assertNoBudgetRegression', () => {
  ...
  test('throws with all violations and the cap value in the message', () => {
    ...
    expect(err!.message).toContain('CYBERDART_BUDGET_RATIO');
  });
});
