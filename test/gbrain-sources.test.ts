 ...
 * invocations. The same trick `test/cyberdart-gbrain-source-wireup.test.ts` uses.
 */
...
describe("probeSource", () => {
  it("returns absent when source id is not in the list", () => {
    ...
    const state = probeSource("cyberdart-code-foo", fake.env);
  ...
  it("returns match when source id is registered (path included)", () => {
    const fake = makeFakeGbrain({
      sources: [{ id: "cyberdart-code-foo", local_path: "/Users/me/repo" }],
    });
    const state = probeSource("cyberdart-code-foo", fake.env);
...
describe("ensureSourceRegistered", () => {
  it("adds source when absent, returns changed=true", async () => {
    ...
    const result = await ensureSourceRegistered("cyberdart-code-foo", "/Users/me/repo", {
      federated: true,
      env: fake.env,
    });
    ...
    expect(log).toContain("sources add cyberdart-code-foo --path /Users/me/repo --federated");
  ...
  it("is a no-op when source is already at the correct path, returns changed=false", async () => {
    ...
    const result = await ensureSourceRegistered("cyberdart-code-foo", "/Users/me/repo", { env: fake.env });
  ...
  it("recreates source when path differs (gbrain has no `sources update`), returns changed=true", async () => {
    const fake = makeFakeGbrain({
      sources: [{ id: "cyberdart-code-foo", local_path: "/old/path" }],
    });
    const result = await ensureSourceRegistered("cyberdart-code-foo", "/new/path", {
      federated: true,
      env: fake.env,
    });
    expect(result.changed).toBe(true);
    ...
    const log = readFileSync(fake.logPath, "utf-8");
    expect(log).toContain("sources remove cyberdart-code-foo --yes");
    expect(log).toContain("sources add cyberdart-code-foo --path /new/path --federated");
    fake.cleanup();
  });
...
describe("sourcePageCount", () => {
  it("returns the page_count when the source is registered", () => {
    const fake = makeFakeGbrain({
      sources: [
        { id: "cyberdart-code-foo", local_path: "/x", page_count: 1247 },
        { id: "other-source", local_path: "/y", page_count: 99 },
      ],
    });
    expect(sourcePageCount("cyberdart-code-foo", fake.env)).toBe(1247);
