...
(gbrain ships the smarts), client-side (cyberdart ships the smarts), and a hybrid
...
Proceed directly to Phase 4 (Alternatives Generation). Generate 2-3 distinct architectural approaches that differ in KIND (not in coverage). Realistic shapes for this project:
  ...
  B) Client-side: cyberdart ships a helper (bin/cyberdart-brain-context-load) that composes salience client-side from existing MCP tools.
  C) Hybrid: V1 client-side in cyberdart; V1.5 promotes to gbrain server-side once the salience signal is validated.
