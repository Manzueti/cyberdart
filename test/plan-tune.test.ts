...
describe('preamble — QUESTION_TUNING injection', () => {
  test('tier 2+ skills include the Question Tuning section', async () => {
    ...
    const ctx = {
      ...
      paths: {
        skillRoot: '~/.claude/skills/cyberdart',
        localSkillRoot: '.claude/skills/cyberdart',
        binDir: '~/.claude/skills/cyberdart/bin',
        browseDir: '~/.claude/skills/cyberdart/browse/dist',
        designDir: '~/.claude/skills/cyberdart/design/dist',
      },
      preambleTier: 2,
    };
    ...
    expect(out).toContain('cyberdart-question-preference --check');
    expect(out).toContain('cyberdart-question-log');
  ...
  test('codex host produces different paths', async () => {
    ...
    const codexCtx = {
      ...
      host: 'codex' as const,
      paths: {
        skillRoot: '$CYBERDART_ROOT',
        localSkillRoot: '.agents/skills/cyberdart',
        binDir: '$CYBERDART_BIN',
        browseDir: '$CYBERDART_BROWSE',
        designDir: '$CYBERDART_DESIGN',
      },
    };
    const out = generateQuestionTuning(codexCtx);
    expect(out).toContain('$CYBERDART_BIN/cyberdart-question-preference');
    expect(out).toContain('$CYBERDART_BIN/cyberdart-question-log');
  });
});
...
describe('end-to-end pipeline (binaries working together)', () => {
  test('log many expand choices → derive pushes scope_appetite up', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'cyberdart-e2e-'));
    try {
      const env = { ...process.env, CYBERDART_HOME: tmpHome };
      const { spawnSync } = require('child_process');
      const logBin = path.join(ROOT, 'bin', 'cyberdart-question-log');
      const devBin = path.join(ROOT, 'bin', 'cyberdart-developer-profile');
  ...
  test('preference blocks tune: write from inline-tool-output in full pipeline', () => {
    ...
    try {
      const env = { ...process.env, CYBERDART_HOME: tmpHome };
      const { spawnSync } = require('child_process');
      const prefBin = path.join(ROOT, 'bin', 'cyberdart-question-preference');
  ...
  test('migration preserves sessions, builder-profile shim still works', () => {
    ...
    try {
      ...
      const { spawnSync } = require('child_process');
      const devBin = path.join(ROOT, 'bin', 'cyberdart-developer-profile');
      const shimBin = path.join(ROOT, 'bin', 'cyberdart-builder-profile');
