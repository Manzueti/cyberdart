...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  const execOpts: ExecSyncOptionsWithStringEncoding = {
    cwd: ROOT,
    env: { ...process.env, CYBERDART_HOME: tmpDir },
    encoding: 'utf-8',
    timeout: 15000,
  };
  try {
    const stdout = execSync(`${BIN}/cyberdart-learnings-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runSearch(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/cyberdart-learnings-search ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-learn-'));
...
describe('cyberdart-learnings-log', () => {
...
describe('cyberdart-learnings-search', () => {
...
describe('cyberdart-learnings-log edge cases', () => {
...
describe('cyberdart-learnings-search edge cases', () => {
