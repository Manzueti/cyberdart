/**
 * cyberdart-upgrade/migrations/v1.17.0.0.sh — migration script unit tests.
 *
 * The migration runs on /cyberdart-upgrade for users with brain-sync configured but
 * never wired up to gbrain. It has 4 skip conditions and one happy path.
 *
 * Strategy: stub cyberdart-config and cyberdart-gbrain-source-wireup binaries on PATH
...
const MIGRATION = path.join(ROOT, 'cyberdart-upgrade', 'migrations', 'v1.17.0.0.sh');
...
function makeFakeStubs(opts: {
  configValue?: string; // value cyberdart-config returns for gbrain_sync_mode
  configMissing?: boolean; // cyberdart-config binary itself missing (test edge)
  wireupMissing?: boolean; // wireup binary missing
  wireupExitCode?: number;
}) {
  const skillsBin = path.join(tmpHome, '.claude', 'skills', 'cyberdart', 'bin');
  fs.mkdirSync(skillsBin, { recursive: true });

  if (!opts.configMissing) {
    const cfg = `#!/bin/bash
echo "cyberdart-config $@" >> "${stubLog}"
[ "$1" = "get" ] && [ "$2" = "gbrain_sync_mode" ] && echo "${opts.configValue ?? ''}"
exit 0
`;
    fs.writeFileSync(path.join(skillsBin, 'cyberdart-config'), cfg, { mode: 0o755 });
  }

  if (!opts.wireupMissing) {
    const wu = `#!/bin/bash
echo "cyberdart-gbrain-source-wireup $@" >> "${stubLog}"
exit ${opts.wireupExitCode ?? 0}
`;
    fs.writeFileSync(path.join(skillsBin, 'cyberdart-gbrain-source-wireup'), wu, { mode: 0o755 });
  }
}

function makeBrainGitRepo() {
  const cyberdartHome = path.join(tmpHome, '.cyberdart');
  fs.mkdirSync(path.join(cyberdartHome, '.git'), { recursive: true });
}
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-migration-test-'));
...
describe('migrations/v1.17.0.0.sh', () => {
  ...
  test('gbrain_sync_mode = off: exit 0 silently (no helper invoked)', () => {
    ...
    expect(calls.some((c) => c.startsWith('cyberdart-gbrain-source-wireup'))).toBe(false);
  });
  ...
  test('no ~/.cyberdart/.git: exit 0 silently (no brain-sync configured)', () => {
    makeFakeStubs({ configValue: 'full' });
    // Do NOT call makeBrainGitRepo() — no .cyberdart/.git directory exists
    const r = run();
    expect(r.status).toBe(0);
    const calls = stubCalls();
    expect(calls.some((c) => c.startsWith('cyberdart-gbrain-source-wireup'))).toBe(false);
  ...
  test('happy path: invokes the helper', () => {
    ...
    expect(r.status).toBe(0);
    const calls = stubCalls();
    expect(calls.some((c) => c.startsWith('cyberdart-gbrain-source-wireup'))).toBe(true);
    // Note: migration invokes WITHOUT --strict (benign-skip semantics for batch upgrade)
    const helperCall = calls.find((c) => c.startsWith('cyberdart-gbrain-source-wireup'));
