...
describeE2E('/setup-gbrain Path 4 — bad token STOPs cleanly', () => {
  test('AUTH classifier fires, no MCP registration, no CLAUDE.md mutation', async () => {
    ...
    const cyberdartHome = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-bad-'));
    ...
    fs.writeFileSync(path.join(cyberdartHome, 'CLAUDE.md'), ORIGINAL_CLAUDE_MD);
    ...
    try {
      ...
      const finalClaudeMd = fs.readFileSync(path.join(cyberdartHome, 'CLAUDE.md'), 'utf-8');
