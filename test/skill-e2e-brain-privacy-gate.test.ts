 ...
 * on PATH), runs a cheap cyberdart skill through the Agent SDK, intercepts
...
describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('cyberdart skill preamble fires the 3-option AskUserQuestion when gbrain is detected', async () => {
    // Stage a fresh CYBERDART_HOME with artifacts_sync_mode_prompted=false.
    const cyberdartHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-cyberdart-'));
    const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-bin-'));

    // Seed the config so the gate's condition passes.
    fs.writeFileSync(
      path.join(cyberdartHome, 'config.yaml'),
    ...
    // don't inherit them.
    const origCyberDartHome = process.env.CYBERDART_HOME;
    ...
    try {
      ...
      if (origCyberDartHome === undefined) delete process.env.CYBERDART_HOME;
      else process.env.CYBERDART_HOME = origCyberDartHome;
  ...
  test('privacy gate does NOT fire when artifacts_sync_mode_prompted is already true', async () => {
    ...
    const cyberdartHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-off-'));
