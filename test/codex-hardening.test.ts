...
const PROBE = path.join(ROOT, 'bin/cyberdart-codex-probe');
...
function tempHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-codex-probe-home-'));
}

describe('cyberdart-codex-probe: auth probe', () => {
  test('CODEX_API_KEY set → AUTH_OK', () => {
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: '_cyberdart_codex_auth_probe',
  ...
  test('${CODEX_HOME:-~/.codex}/auth.json exists → AUTH_OK', () => {
    ...
    try {
      ...
      const r = runProbe({ snippet: '_cyberdart_codex_auth_probe', home });
      expect(r.stdout.trim()).toBe('AUTH_OK');
      expect(r.status).toBe(0);
    } finally {
      fs.rmSync(home, { recursive: true, force: true });
  ...
  test('alternate $CODEX_HOME → checks the alternate path', () => {
    const home = tempHome();
    const altCodex = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-alt-codex-'));
    try {
...
function tempStubCodex(versionOutput: string, bool_command_fails = false): {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-codex-stub-'));
...
function runVersionCheck(versionOutput: string): string {
  ...
  try {
    const r = runProbe({
      snippet: '_cyberdart_codex_version_check',
...
describe('cyberdart-codex-probe: version check (anchored regex per Tension I)', () => {
  // Matches (should WARN)
...
describe('cyberdart-codex-probe: timeout wrapper + namespace hygiene', () => {
  test('bin/cyberdart-codex-probe is syntactically valid bash (bash -n)', () => {
  ...
  test('timeout wrapper executes command directly when neither binary present', () => {
    ...
    const r = runProbe({
      snippet: `_cyberdart_codex_timeout_wrapper 5 echo hello_world`,
  ...
  test('timeout wrapper resolves gtimeout preferentially when on PATH', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-gto-stub-'));
    try {
      const stub = path.join(dir, 'gtimeout');
      fs.writeFileSync(stub, '#!/bin/bash\necho gtimeout_chosen_$1\n');
      fs.chmodSync(stub, 0o755);
      const r = runProbe({
        snippet: `_cyberdart_codex_timeout_wrapper 5 echo nope`,
...
describe('cyberdart-codex-probe: telemetry event emission', () => {
  test('_cyberdart_codex_log_event writes jsonl when _TEL != off', () => {
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: `_cyberdart_codex_log_event "codex_test_event" "42"; cat "$HOME/.cyberdart/analytics/skill-usage.jsonl"`,
        env: { _TEL: 'community' },
        home,
      });
  ...
  test('_cyberdart_codex_log_event skips write when _TEL = off', () => {
    const home = tempHome();
    try {
      runProbe({
        snippet: `_cyberdart_codex_log_event "codex_test_event" "99"`,
        env: { _TEL: 'off' },
        home,
      });
      const jsonl = path.join(home, '.cyberdart/analytics/skill-usage.jsonl');
  ...
  test('payload never contains prompt content, env values, or auth tokens (schema check)', () => {
    ...
    try {
      const r = runProbe({
        snippet: `_cyberdart_codex_log_event "codex_test_event" "1"; cat "$HOME/.cyberdart/analytics/skill-usage.jsonl"`,
        env: {
          _TEL: 'community',
