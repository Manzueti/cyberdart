...
describe('setup: Conductor worktree guard', () => {
  test('setup contains the real-dir guard before the ln -snf into ~/.claude/skills/', () => {
    ...
    const lnIdx = content.indexOf('ln -snf "$SOURCE_CYBERDART_DIR" "$CLAUDE_CYBERDART_LINK"');
  ...
  test('guard resolves the existing real dir with `pwd -P` and compares against source', () => {
    const content = fs.readFileSync(SETUP_SCRIPT, 'utf-8');
    expect(content).toContain('[ -d "$CLAUDE_CYBERDART_LINK" ] && [ ! -L "$CLAUDE_CYBERDART_LINK" ]');
    expect(content).toContain('cd "$CLAUDE_CYBERDART_LINK" 2>/dev/null && pwd -P');
    expect(content).toContain('"$_EXISTING_REAL" != "$SOURCE_CYBERDART_DIR"');
  });
  ...
  test('skip branch prints "registration skipped" + remediation hint', () => {
    ...
    expect(content).toContain('rm -rf $CLAUDE_CYBERDART_LINK');
  });
  ...
  test('BSD ln -snf into an existing real dir creates a child symlink (bug reproduces)', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberdart-setup-guard-'));
    try {
  ...
  test('guard logic refuses to ln when dest is a real dir pointing elsewhere', () => {
    ...
    try {
      ...
      const script = `
        set -e
        SOURCE_CYBERDART_DIR='${source}'
        CLAUDE_CYBERDART_LINK='${dest}'
        _SKIP_CLAUDE_REGISTER=0
        if [ -d "$CLAUDE_CYBERDART_LINK" ] && [ ! -L "$CLAUDE_CYBERDART_LINK" ]; then
          _EXISTING_REAL=$(cd "$CLAUDE_CYBERDART_LINK" 2>/dev/null && pwd -P || echo "")
          if [ -n "$_EXISTING_REAL" ] && [ "$_EXISTING_REAL" != "$SOURCE_CYBERDART_DIR" ]; then
        ...
        if [ "$_SKIP_CLAUDE_REGISTER" -eq 1 ]; then
          ...
          ln -snf "$SOURCE_CYBERDART_DIR" "$CLAUDE_CYBERDART_LINK"
          echo "LINKED"
        fi
      `;
  ...
  test('guard allows ln when dest is a real dir already pointing to source (self-rerun)', () => {
    ...
    try {
      const source = path.join(tmp, 'source-worktree');
      fs.mkdirSync(source);
      // Mirror setup's SOURCE_CYBERDART_DIR resolution (`pwd -P`) so the comparison
      ...
      const script = `
        set -e
        SOURCE_CYBERDART_DIR='${resolvedSource}'
        CLAUDE_CYBERDART_LINK='${dest}'
        _SKIP_CLAUDE_REGISTER=0
        if [ -d "$CLAUDE_CYBERDART_LINK" ] && [ ! -L "$CLAUDE_CYBERDART_LINK" ]; then
          _EXISTING_REAL=$(cd "$CLAUDE_CYBERDART_LINK" 2>/dev/null && pwd -P || echo "")
