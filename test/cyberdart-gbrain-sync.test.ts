/**
 * Unit tests for bin/cyberdart-gbrain-sync.ts (Lane B).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "cyberdart-gbrain-sync.ts");

function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "cyberdart-gbrain-sync-"));
}
...
describe("cyberdart-gbrain-sync CLI", () => {
  it("--help exits 0 with usage text", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: cyberdart-gbrain-sync");
  ...
  it("--dry-run with --code-only reports the code import preview only", () => {
    ...
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(cyberdartHome, { recursive: true });

    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, CYBERDART_HOME: cyberdartHome });
    expect(r.exitCode).toBe(0);
    ...
    expect(r.stdout).not.toContain("cyberdart-memory-ingest --probe");
    expect(r.stdout).not.toContain("cyberdart-brain-sync --discover-new");
  ...
  it("--dry-run with all stages shows previews for all three", () => {
    ...
    const r = runScript(["--dry-run"], { HOME: home, CYBERDART_HOME: cyberdartHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("would: gbrain sources add");
    expect(r.stdout).toContain("gbrain sync --strategy code");
    expect(r.stdout).toContain("would: cyberdart-memory-ingest");
    expect(r.stdout).toContain("would: cyberdart-brain-sync");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("--no-code skips the code import stage", () => {
    ...
    const r = runScript(["--dry-run", "--no-code"], { HOME: home, CYBERDART_HOME: cyberdartHome });
    expect(r.exitCode).toBe(0);
  ...
  it("dry-run derives a stable source id from the canonical git remote", () => {
    // The source id pattern is `cyberdart-code-<canonicalized-remote>`. For this
    // repo (github.com/garrytan/cyberdart), the slug should appear in the dry-run
    // preview line. We don't pin the exact slug — just verify the prefix +
    // that the preview command would target a source with id cyberdart-code-*.
    const home = makeTestHome();
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(cyberdartHome, { recursive: true });
    ...
    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, CYBERDART_HOME: cyberdartHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/gbrain sources add cyberdart-code-[a-z0-9-]+/);
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source cyberdart-code-[a-z0-9-]+/);
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("derived source ids are gbrain-valid (≤32 chars, alnum + interior hyphens, no dots) for any remote", () => {
    ...
    // For `github.com/<org>/<repo>`, the id was `cyberdart-code-github.com-<org>-<repo>`,
    ...
    const cases = [
      ...
      "git@github.com:garrytan/cyberdart.git",            // SCP-style remote
    ...
    for (const remote of cases) {
      ...
      const cyberdartHome = join(home, ".cyberdart");
      mkdirSync(cyberdartHome, { recursive: true });
      const repo = mkdtempSync(join(tmpdir(), "cyberdart-source-id-repo-"));
      ...
      const r = spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        ...
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, CYBERDART_HOME: cyberdartHome },
      });
      ...
      expect(id.startsWith("cyberdart-code-")).toBe(true);
  ...
  it("derives a gbrain-valid source id when the cwd repo has NO origin remote", () => {
    ...
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(cyberdartHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cyberdart-no-origin-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
  ...
  it("derives a gbrain-valid source id when the basename sanitizes to empty", () => {
    ...
    // returned "cyberdart-code-" (invalid trailing hyphen) and was patched to
    ...
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(cyberdartHome, { recursive: true });
    const parent = mkdtempSync(join(tmpdir(), "cyberdart-empty-base-"));
  ...
  it("dry-run does NOT acquire the lock file (lock is for write paths only)", () => {
    ...
    const lockPath = join(cyberdartHome, ".sync-gbrain.lock");
  ...
  it("writes a state file with schema_version: 1 after a non-dry run", () => {
    ...
    const statePath = join(cyberdartHome, ".gbrain-sync-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("cyberdart-gbrain-sync");
  ...
  it("records stage results in state file", () => {
    ...
    const state = JSON.parse(readFileSync(join(cyberdartHome, ".gbrain-sync-state.json"), "utf-8"));
  ...
  it("brain-sync stage resolves the sibling binary, not a HOME-rooted path", () => {
    ...
    // ~/.claude/skills/cyberdart/bin/cyberdart-brain-sync, which silently no-op'd
    // on Codex installs and dev workspaces with the misleading summary
    // "skipped (cyberdart-brain-sync not installed)". Post-fix it resolves
    ...
    const r = runScript(
      ...
      { HOME: home, CYBERDART_HOME: cyberdartHome },
    );
    ...
    expect(combined).not.toContain("skipped (cyberdart-brain-sync not installed)");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("worktree-aware source ID: two worktrees of the same repo get DIFFERENT ids", () => {
    ...
    // ID was slug-only so both worktrees collapsed onto `cyberdart-code-<slug>` and
    ...
    const remote = "https://github.com/garrytan/cyberdart.git";
    const home = makeTestHome();
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(cyberdartHome, { recursive: true });

    const repoA = mkdtempSync(join(tmpdir(), "cyberdart-worktree-a-"));
    const repoB = mkdtempSync(join(tmpdir(), "cyberdart-worktree-b-"));
    ...
    expect(idA.startsWith("cyberdart-code-")).toBe(true);
    expect(idB.startsWith("cyberdart-code-")).toBe(true);
  ...
  it("worktree-aware source ID: same path produces the same id across runs (deterministic)", () => {
    ...
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(cyberdartHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cyberdart-worktree-stable-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", remote], { cwd: repo });
  ...
  it("dry-run preview includes legacy-source removal + attach (post-codex-review hardening)", () => {
    // Codex adversarial flagged: pre-pathhash `cyberdart-code-<slug>` sources stay
    ...
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(cyberdartHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cyberdart-legacy-cleanup-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", "https://github.com/garrytan/cyberdart.git"], { cwd: repo });
    ...
    // remove the legacy source via `gbrain sources remove cyberdart-code-<slug>
    ...
    expect(r.stdout).toMatch(/gbrain sources add cyberdart-code-/);
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source cyberdart-code-/);
    expect(r.stdout).toMatch(/gbrain sources attach cyberdart-code-/);
  ...
  it("dry-run preview includes the `sources attach` step (kubectl-style CWD pin)", () => {
    ...
    const cyberdartHome = join(home, ".cyberdart");
    mkdirSync(cyberdartHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cyberdart-attach-preview-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", "https://github.com/garrytan/cyberdart.git"], { cwd: repo });
