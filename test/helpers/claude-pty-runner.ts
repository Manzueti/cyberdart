...
export interface ClaudePtyOptions {
  ...
  cols?: number;
  rows?: number;
  /** Working directory. Default: process.cwd(). The repo cwd has the cyberdart
...
// ~/.cyberdart/analytics/pty-judge.jsonl for offline analysis.
// ────────────────────────────────────────────────────────────────────────────
...
function logPtyJudge(record: Record<string, unknown>): void {
  try {
    const dir = `${process.env.HOME}/.cyberdart/analytics`;
...
/**
 * Snapshot dump for postmortem debugging when CYBERDART_PTY_LOG=1.
 * Writes the last 4KB of visible TTY plus context to
 * ~/.cyberdart/analytics/pty-snapshots/<testName>-<elapsed>ms.txt.
 */
export function logPtySnapshot(visible: string, ctx: { testName: string; elapsedMs: number; tag?: string }): void {
  if (process.env.CYBERDART_PTY_LOG !== '1') return;
  try {
    const dir = `${process.env.HOME}/.cyberdart/analytics/pty-snapshots`;
    fs.mkdirSync(dir, { recursive: true });
...
export const COMPLETION_SUMMARY_RE =
  /(CYBERDART REVIEW REPORT|## Completion [Ss]ummary|Status:\s*(clean|issues_open)|^VERDICT:)/m;

/**
 * Result of asserting that a plan file ends with `## CYBERDART REVIEW REPORT`
...
/**
 * Assert that `## CYBERDART REVIEW REPORT` is the last `## ` heading in a plan
...
export function assertReviewReportAtBottom(
  ...
  const re = /^## CYBERDART REVIEW REPORT\s*$/m;
  const match = re.exec(content);
  if (!match) {
    return { ok: false, reason: 'no CYBERDART REVIEW REPORT section' };
  }
  ...
  if (trailingHeadings.length > 0) {
    return {
      ok: false,
      reason: 'trailing ## heading(s) after CYBERDART REVIEW REPORT',
      trailingHeadings,
    };
  }
  return { ok: true };
 ...
 * `## CYBERDART REVIEW REPORT` is the last `## ` section. Throws on
...
export function assertReportAtBottomIfPlanWritten(
  ...
  if (!verdict.ok) {
    const trailing = verdict.trailingHeadings?.length
      ...
      : '';
    throw new Error(
      `CYBERDART REVIEW REPORT contract violation in ${obs.planFile}: ${verdict.reason}${trailing}\n` +
        `--- evidence (last 2KB) ---\n${obs.evidence}`,
    );
  }
}
...
export async function runPlanSkillObservation(opts: {
  ...
  try {
    ...
    while (Date.now() - start < budgetMs) {
      ...
      // visible buffer at each judge call when CYBERDART_PTY_LOG=1.
...
export async function runPlanSkillFloorCheck(opts: {
  ...
  try {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      // matches "CYBERDART REVIEW REPORT" anywhere in the buffer, including
