...
describe('isPermissionDialogVisible', () => {
  test('matches "Bash command requires permission" prompts', () => {
    const sample = `
      ...
      Bash command \`cyberdart-config get telemetry\` requires permission to run.
...
describe('classifyVisible (runtime path through the runner classifier)', () => {
  ...
  test('permission dialog (Bash) → returns null (skip, keep polling)', () => {
    const visible = `
      Bash command \`cyberdart-update-check\` requires permission to run.
...
describe('COMPLETION_SUMMARY_RE', () => {
  test('matches CYBERDART REVIEW REPORT heading', () => {
    expect(COMPLETION_SUMMARY_RE.test('## CYBERDART REVIEW REPORT')).toBe(true);
  });
...
`;
    ...
    expect(r.reason).toMatch(/no CYBERDART REVIEW REPORT/);
  });
...
`;
  ...
  test('passes when only ### subheadings follow REVIEW REPORT (deeper nesting allowed)', () => {
    const content = `## CYBERDART REVIEW REPORT
