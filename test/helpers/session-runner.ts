...
const CYBERDART_DEV_DIR = path.join(os.homedir(), '.cyberdart-dev');
const HEARTBEAT_PATH = path.join(CYBERDART_DEV_DIR, 'e2e-live.json'); // heartbeat stays global
const PROJECT_DIR = path.dirname(getProjectEvalDir()); // ~/.cyberdart/projects/$SLUG/
...
export async function runSkillTest(options: {
   ...
   *  per-test CYBERDART_HOME overrides so the test doesn't have to spell out
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.cyberdart', 'test-transcripts');
