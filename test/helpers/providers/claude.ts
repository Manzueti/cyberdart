...
export class ClaudeAdapter implements ProviderAdapter {
  ...
  async available(): Promise<AvailabilityCheck> {
    // Binary on PATH (or CYBERDART_CLAUDE_BIN override). Routes through the shared
    // resolver so Windows + override paths behave the same as production sites.
    const resolved = resolveClaudeCommand();
    if (!resolved) {
      return { ok: false, reason: 'claude CLI not found on PATH. Install from https://claude.ai/download or npm i -g @anthropic-ai/claude-code (or set CYBERDART_CLAUDE_BIN)' };
    }
  ...
  async run(opts: RunOpts): Promise<RunResult> {
    ...
    const resolved = resolveClaudeCommand();
    if (!resolved) {
      throw new Error('claude CLI not resolvable (set CYBERDART_CLAUDE_BIN or install)');
    }
