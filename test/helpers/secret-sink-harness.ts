 ...
 *   - telemetry JSONL under $HOME/.cyberdart/analytics/ (same walk, but called
...
export interface SinkResult {
  ...
  /** Subset of filesWritten matching .cyberdart/analytics/*.jsonl. */
...
export async function runWithSecretSink(opts: SecretSinkOptions): Promise<SinkResult> {
  ...
  // Make sure .cyberdart exists so bins that append to analytics have somewhere to write.
  fs.mkdirSync(path.join(tmpHome, '.cyberdart', 'analytics'), { recursive: true });
  ...
  for (const [rel, content] of Object.entries(filesWritten)) {
    if (rel.startsWith('.cyberdart/analytics/') && rel.endsWith('.jsonl')) {
  ...
  for (const seed of opts.seeds) {
    ...
    for (const { rule, matchType } of rules) {
      ...
      for (const [rel, content] of Object.entries(filesWritten)) {
        ...
        if (hit !== null) {
          const channel = rel.startsWith('.cyberdart/analytics/') ? 'telemetry' : 'file';
