 ...
 * ~/.cyberdart/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.cyberdart-dev', 'evals');

/**
 * Detect project-scoped eval dir via cyberdart-slug.
 * Falls back to legacy ~/.cyberdart-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local cyberdart-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null || ~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.cyberdart', 'projects', slugMatch[1], 'evals');
 ...
 * `CYBERDART_BUDGET_RATIO` env var. New tests with no prior data are skipped.
 */
export function findBudgetRegressions(
  comparison: ComparisonResult,
  opts?: { ratioCap?: number; minPriorTools?: number; minPriorTurns?: number },
): BudgetRegression[] {
  const envRatio = Number(process.env.CYBERDART_BUDGET_RATIO);
...
export function assertNoBudgetRegression(
  ...
  const cap = opts?.ratioCap ?? (Number(process.env.CYBERDART_BUDGET_RATIO) || 2.0);
  ...
  throw new Error(
    ...
    lines.join('\n') +
    `\n(Override per run: CYBERDART_BUDGET_RATIO=<n>. ${comparison.before_file} vs ${comparison.after_file})`,
  );
}
