...
const SCRIPT = path.join(import.meta.dir, "..", "bin", "cyberdart-learnings-search");

describe("cyberdart-learnings-search injection prevention", () => {
  ...
  test("uses process.env for all user-controlled values", () => {
    ...
    expect(bunBlock).toContain("process.env.CYBERDART_SEARCH_TYPE");
    expect(bunBlock).toContain("process.env.CYBERDART_SEARCH_QUERY");
    expect(bunBlock).toContain("process.env.CYBERDART_SEARCH_LIMIT");
    expect(bunBlock).toContain("process.env.CYBERDART_SEARCH_SLUG");
    expect(bunBlock).toContain("process.env.CYBERDART_SEARCH_CROSS");
  });

  test("env vars are set on the bun command line", () => {
    // The env vars must be passed to bun, not just set in the shell
    expect(script).toContain("CYBERDART_SEARCH_TYPE=");
    expect(script).toContain("CYBERDART_SEARCH_QUERY=");
    expect(script).toContain("CYBERDART_SEARCH_LIMIT=");
    expect(script).toContain("CYBERDART_SEARCH_SLUG=");
    expect(script).toContain("CYBERDART_SEARCH_CROSS=");
  });
});
