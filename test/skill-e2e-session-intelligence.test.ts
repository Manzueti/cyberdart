...
describeIfSelected('Session Intelligence E2E', [
  ...
  beforeAll(() => {
    ...
    cyberdartHome = path.join(workDir, '.cyberdart-home');
    ...
    // Compute slug (same logic as cyberdart-slug without git remote)
  ...
  // Write a timeline event via cyberdart-timeline-log, read it back via cyberdart-timeline-read.
  ...
  testConcurrentIfSelected('timeline-event-flow', async () => {
    const projectDir = path.join(cyberdartHome, 'projects', slug);
    fs.mkdirSync(projectDir, { recursive: true });

    // Write two events via the binary
    const logBin = path.join(workDir, 'bin', 'cyberdart-timeline-log');
    const readBin = path.join(workDir, 'bin', 'cyberdart-timeline-read');
    const env = { ...process.env, CYBERDART_HOME: cyberdartHome };
    ...
    // Read via cyberdart-timeline-read
    ...
    // Verify cyberdart-timeline-read output includes the events
...
IMPORTANT:
- Use CYBERDART_HOME="${cyberdartHome}" as an environment variable when running bin scripts.
- The bin scripts are at ./bin/ (relative to this directory), not at ~/.claude/skills/cyberdart/bin/.
  Replace any references to ~/.claude/skills/cyberdart/bin/ with ./bin/ when running commands.
