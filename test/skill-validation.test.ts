...
describe('Cross-skill path consistency', () => {
  ...
  test('all greptile-history write references specify both per-project and global paths', () => {
    ...
    for (const file of filesToCheck) {
      ...
      const hasBoth = (content.includes('per-project') && content.includes('global')) ||
        (content.includes('$REMOTE_SLUG/greptile-history') && content.includes('~/.cyberdart/greptile-history'));
  ...
  test('greptile-triage.md contains both project and global history paths', () => {
    ...
    expect(content).toContain('~/.cyberdart/greptile-history.md');
  });
...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.cyberdart/qa-reports/');
  });
});
...
describe('office-hours skill structure', () => {
  ...
  test('contains YC apply CTA with ref tracking', () => {
    expect(content).toContain('ycombinator.com/apply?ref=cyberdart');
  });
...
// --- cyberdart-slug helper ---

describe('cyberdart-slug', () => {
  const SLUG_BIN = path.join(ROOT, 'bin', 'cyberdart-slug');
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/cyberdart-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for cyberdart-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*cyberdart-slug', '--include=*.tmpl', '--include=cyberdart-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Report: .cyberdart/qa-reports/');
  });
...
describe('Codex skill', () => {
  ...
  test('codex/SKILL.md contains review log persistence', () => {
    ...
    expect(content).toContain('cyberdart-review-log');
  });
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    ...
    const shipContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'cyberdart-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'cyberdart-review', 'SKILL.md'), 'utf-8');
...
describe('Skill trigger phrases', () => {
  ...
  // Excluded: root cyberdart (browser tool), cyberdart-upgrade (cyberdart-specific),
  // humanizer (text tool)
...
describe('Codex skill validation', () => {
  ...
  test('all skills (except /codex) have both Claude and Codex variants', () => {
    for (const skillDir of CLAUDE_SKILLS_WITH_TEMPLATES) {
      ...
      // Codex variant
      const codexName = skillDir.startsWith('cyberdart-') ? skillDir : `cyberdart-${skillDir}`;
  ...
  test('/claude skill is external-host-only — no Claude-host variant', () => {
    ...
    expect(fs.existsSync(path.join(AGENTS_DIR, 'cyberdart-claude', 'SKILL.md'))).toBe(true);
  });

  test('Codex skill names follow cyberdart-{name} convention', () => {
    const codexDirs = fs.readdirSync(AGENTS_DIR);
    for (const dir of codexDirs) {
      // Every directory should start with cyberdart
      expect(dir.startsWith('cyberdart')).toBe(true);
      // Root is just 'cyberdart', others are 'cyberdart-{name}'
      if (dir !== 'cyberdart') {
        expect(dir.startsWith('cyberdart-')).toBe(true);
      }
    }
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    ...
    expect(content).toContain('cyberdart-repo-mode');
  });
...
//
// Browser-skills are bundled in <cyberdart-root>/browser-skills/<name>/. Each
