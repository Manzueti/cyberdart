...
description: |
  ...
  guidance in CLAUDE.md. Wraps the cyberdart-gbrain-sync orchestrator with
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"sync-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"sync-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
It does NOT touch `~/.cyberdart/` indexing (the existing `cyberdart-gbrain-source-wireup`
...
`~/.claude/skills/cyberdart/bin/cyberdart-gbrain-sync.ts`.
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-gbrain-detect 2>/dev/null
```
...
Also check the per-repo trust policy. If `cyberdart-gbrain-repo-policy get` for
...
```bash
bun run ~/.claude/skills/cyberdart/bin/cyberdart-gbrain-sync.ts <user-args>
```
...
still run. State is persisted to `~/.cyberdart/.gbrain-sync-state.json` via
tmp-file + atomic rename. Concurrent runs are blocked by a lock file at
`~/.cyberdart/.sync-gbrain.lock` (5-min stale-takeover).
...
```bash
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' ~/.cyberdart/.gbrain-sync-state.json 2>/dev/null \
...
from `<!-- cyberdart-gbrain-search-guidance:start -->` through
`<!-- cyberdart-gbrain-search-guidance:end -->`. If those markers are missing,
...
gbrain status: GREEN
  ...
  CWD source ...... OK   <cyberdart-code-{repo_slug}> (page_count=<N>)
  ~/.cyberdart source. OK   <cyberdart-brain-{user}> (page_count=<N>) — managed by /setup-gbrain
...
Mac. The orchestrator acquires a lock at `~/.cyberdart/.sync-gbrain.lock` before
...
and travels with `git push`/`git pull` — NOT through `~/.cyberdart/.brain-allowlist`
(which is for `~/.cyberdart/` brain-sync only). On a different Mac with a synced
