...
The skill asks once per repo the first time you run a cyberdart skill there. After that the decision is sticky — every worktree + branch of the same git remote shares the same policy, so you set it once and it follows you.
...
# Or directly:
~/.claude/skills/cyberdart/bin/cyberdart-gbrain-repo-policy set "github.com/foo/bar" read-only
```

**To see every policy:**

```bash
~/.claude/skills/cyberdart/bin/cyberdart-gbrain-repo-policy list
```

Storage: `~/.cyberdart/gbrain-repo-policy.json`, mode 0600, schema-versioned so future migrations stay deterministic.
...
**If migration hangs:** another cyberdart session may be holding a lock on the source brain. The timeout fires at 3 minutes with an actionable message. Close other workspaces and re-run.

## Cyberdart memory sync (a separate concern)

This is different from gbrain itself. Your cyberdart state (`~/.cyberdart/` — learnings, plans, retros, timeline, developer profile) is machine-local by default. "Cyberdart memory sync" optionally pushes a curated, secret-scanned subset to a private git repo so your memory follows you across machines — and, if you're running gbrain, that git repo becomes indexable there too.
...
**On a new machine:** Copy `~/.cyberdart-brain-remote.txt` over, run `cyberdart-brain-restore`, and yesterday's learnings surface on today's laptop.
...
| Bin | Purpose |
|---|---|
| `cyberdart-gbrain-detect` | Emit current state as JSON: gbrain on PATH, version, config engine, doctor status, sync mode |
| `cyberdart-gbrain-install` | Detect-first installer (probes `~/git/gbrain`, `~/gbrain`, then fresh clone). Has `--dry-run` and `--validate-only` flags. PATH-shadow check exits 3 with remediation menu. |
| `cyberdart-gbrain-lib.sh` | Sourced, not executed. Provides `read_secret_to_env VARNAME "prompt" [--echo-redacted "<sed-expr>"]` |
| `cyberdart-gbrain-supabase-verify` | Structural URL check. Rejects direct-connection URLs (`db.*.supabase.co:5432`) with exit 3 |
| `cyberdart-gbrain-supabase-provision` | Management API wrapper. Subcommands: `list-orgs`, `create`, `wait`, `pooler-url`, `list-orphans`, `delete-project`. All require `SUPABASE_ACCESS_TOKEN` in env. `create` and `pooler-url` also require `DB_PASS`. `--json` mode available on every subcommand. |
| `cyberdart-gbrain-repo-policy` | Per-remote trust triad. Subcommands: `get`, `set`, `list`, `normalize` |
| `cyberdart-gbrain-source-wireup` | Registers your `~/.cyberdart/` brain repo with gbrain as a federated source via `gbrain sources add` + `git worktree`, then runs an initial `gbrain sync`. Idempotent. Replaces the dead `consumers.json + /ingest-repo` HTTP wireup from v1.12.x. Flags: `--strict`, `--source-id <id>`, `--no-pull`, `--uninstall`, `--probe`. |

### gbrain CLI (upstream tool)

Gbrain itself ships with these that cyberdart wraps:
...
| `~/.cyberdart/gbrain-repo-policy.json` | Per-remote trust triad. Schema v2. Mode 0600. |
| `~/.cyberdart/.setup-gbrain.lock.d` | Concurrent-run lock (atomic mkdir). Released on normal exit + SIGINT. |
| `~/.cyberdart/.brain-queue.jsonl` | Pending sync entries for cyberdart memory sync |
| `~/.cyberdart/.brain-last-push` | Timestamp of last sync push (for `/health` scoring) |
| `~/.cyberdart-brain-remote.txt` | URL of your cyberdart memory sync remote (safe to copy between machines) |
| `~/.cyberdart/.setup-gbrain-inflight.json` | Reserved for future `--resume-provision` persisted state |

### Environment variables

| Var | Where it's read | What it does |
|---|---|---|
| `SUPABASE_ACCESS_TOKEN` | `cyberdart-gbrain-supabase-provision` | PAT for Management API calls. Discarded after each setup run. |
| `DB_PASS` | `cyberdart-gbrain-supabase-provision` (create, pooler-url) | Generated DB password. Never in argv. |
...
| `SUPABASE_API_BASE` | `cyberdart-gbrain-supabase-provision` | Override the Management API host. Used by tests to point at a mock server. |
| `GBRAIN_INSTALL_DIR` | `cyberdart-gbrain-install` | Override default install path (`~/gbrain`) |
| `CYBERDART_HOME` | every bin helper | Override `~/.cyberdart` state dir. Heavy test use. |
...
- CI grep test fails if `--insecure`, `-k`, or `NODE_TLS_REJECT_UNAUTHORIZED=0` appear in `bin/cyberdart-gbrain-supabase-provision`
...
```bash
rm -rf ~/.cyberdart/.setup-gbrain.lock.d
```
...
You edited `~/.cyberdart/gbrain-repo-policy.json` by hand with legacy `allow` values? No problem. On the next read, cyberdart auto-migrates `allow` → `read-write` and adds `_schema_version: 2`. One log line on stderr, idempotent, deterministic.
...
Another cyberdart session in a sibling Conductor workspace may be holding a lock on your local PGLite file via its preamble's `cyberdart-brain-sync` call. Close other workspaces, re-run `/setup-gbrain --switch`. The timeout is bounded at 180s so you'll never actually wait forever.
...
**Why not bundle gbrain into cyberdart?** Gbrain is a separate, actively-developed project with its own release cadence, schema migrations, and MCP surface. Bundling would mean cyberdart has to gate gbrain updates, which slows gbrain improvements from reaching users. Separate-but-integrated lets each ship on its own cadence.
...
- `/cyberdart-upgrade` — keeps cyberdart itself up to date. Does NOT upgrade gbrain independently. To bump gbrain, update `PINNED_COMMIT` in `bin/cyberdart-gbrain-install` and re-run `/setup-gbrain`.
