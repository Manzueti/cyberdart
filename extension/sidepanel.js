...
document.getElementById('chat-cookies-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('[cyberdart sidebar] Failed to open cookie picker:', err.message);
  }
});
...
async function ensureSseSessionCookie() {
  ...
  try {
    ...
    console.warn('[cyberdart sidebar] Failed to mint SSE session cookie:', err && err.message);
    return false;
  }
}
...
async function connectSSE() {
  ...
  eventSource.addEventListener('activity', (e) => {
    try { addEntry(JSON.parse(e.data)); } catch (err) {
      console.error('[cyberdart sidebar] Failed to parse activity event:', err.message);
    }
  });
  ...
  eventSource.addEventListener('gap', (e) => {
    try {
      ...
      console.error('[cyberdart sidebar] Failed to parse gap event:', err.message);
    }
  });
}
...
async function fetchRefs() {
  ...
  try {
    ...
    console.error('[cyberdart sidebar] Failed to fetch refs:', err.message);
  }
}
...
inspectorSendBtn.addEventListener('click', () => {
  ...
  const ok = window.cyberdartInjectToTerminal?.(message + '\n');
  if (!ok) {
    console.warn('[cyberdart sidebar] Inspector send needs an active Terminal session.');
  }
});
...
async function runCleanup(...buttons) {
  ...
  const sent = window.cyberdartInjectToTerminal?.(cleanupPrompt + '\n');
  if (!sent) {
    console.warn('[cyberdart sidebar] Cleanup needs an active Terminal session.');
  }
...
async function runScreenshot(...buttons) {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn('[cyberdart sidebar] Screenshot failed:', text);
    } else {
      console.log('[cyberdart sidebar] Screenshot:', text);
    }
  } catch (err) {
    console.error('[cyberdart sidebar] Screenshot error:', err.message);
  } finally {
  ...
  try {
    ...
    inspectorSSE.addEventListener('inspectResult', (e) => {
      try {
        ...
        console.error('[cyberdart sidebar] Failed to parse inspectResult:', err.message);
      }
    });
    ...
    console.debug('[cyberdart sidebar] Inspector SSE not available:', err.message);
  }
}
...
function updateConnection(url, token) {
  ...
  if (url) {
    try { window.cyberdartServerPort = parseInt(new URL(url).port, 10); } catch {}
    window.cyberdartAuthToken = token || null;
  } else {
    window.cyberdartServerPort = null;
    window.cyberdartAuthToken = null;
  }
  if (url) {
...
document.getElementById('conn-copy').addEventListener('click', () => {
  navigator.clipboard.writeText('/open-cyberdart-browser').then(() => {
    const btn = document.getElementById('conn-copy');
    btn.textContent = 'copied!';
    setTimeout(() => { btn.textContent = '/open-cyberdart-browser'; }, 2000);
  });
});
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'browserTabState') {
    document.dispatchEvent(new CustomEvent('cyberdart:tab-state', {
