/**
 * cyberdart browse — background service worker
 *
...
async function loadAuthToken() {
  ...
  try {
    ...
    console.error('[cyberdart bg] Failed to load auth token:', err.message);
  }
}
...
async function checkHealth() {
  ...
  try {
    ...
    console.error('[cyberdart bg] Health check failed:', err.message);
    setDisconnected();
  }
}
...
function setConnected(healthData) {
  ...
  chrome.runtime.sendMessage({ type: 'health', data: healthData }).catch((err) => {
    console.debug('[cyberdart bg] No listener for health broadcast:', err.message);
  });
...
function setDisconnected() {
  ...
  chrome.runtime.sendMessage({ type: 'health', data: null }).catch((err) => {
    console.debug('[cyberdart bg] No listener for disconnect broadcast:', err.message);
  });
...
async function notifyContentScripts(type) {
  try {
    ...
    console.error('[cyberdart bg] Failed to query tabs for notification:', err.message);
  }
}
...
async function fetchAndRelayRefs() {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn(`[cyberdart bg] Refs endpoint returned ${resp.status}`);
      return;
    }
    const data = await resp.json();
    ...
    console.error('[cyberdart bg] Failed to fetch/relay refs:', err.message);
  }
}
...
async function injectInspector(tabId) {
  ...
  try {
    ...
    try {
      ...
      console.debug('[cyberdart bg] Inspector CSS injection failed (non-fatal):', err.message);
    }
    // Send startPicker to the injected inspector.js
    try {
      await chrome.tabs.sendMessage(tabId, { type: 'startPicker' });
    } catch (err) {
      console.warn('[cyberdart bg] Failed to send startPicker:', err.message);
    }
    inspectorMode = 'full';
    ...
    try {
      ...
      console.error('[cyberdart bg] Inspector injection failed completely:', err.message, '| Basic fallback:', err2.message);
      inspectorMode = 'full';
...
async function stopInspector(tabId) {
  try {
    ...
    console.debug('[cyberdart bg] Failed to stop picker on tab', tabId, ':', err.message);
  }
  return { ok: true };
}
...
async function postInspectorPick(selector, frameInfo, basicData, activeTabUrl) {
  ...
  try {
    ...
    console.debug('[cyberdart bg] Inspector pick server unavailable, using basic mode:', err.message);
    return { mode: 'basic', selector, basicData, frameInfo };
  }
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (sender.id !== chrome.runtime.id) {
    console.warn('[cyberdart] Rejected message from unknown sender:', sender.id);
    return;
  }
  ...
  if (!ALLOWED_TYPES.has(msg.type)) {
    console.warn('[cyberdart] Rejected unknown message type:', msg.type);
    return;
  }
  ...
  if (msg.type === 'getToken') {
    if (sender.tab) {
      console.warn('[cyberdart] Rejected getToken from content script context');
  ...
  if (msg.type === 'openSidePanel') {
    if (chrome.sidePanel?.open && sender.tab) {
      chrome.sidePanel.open({ tabId: sender.tab.id }).catch((err) => {
        console.warn('[cyberdart bg] Failed to open side panel:', err.message);
      });
    }
    return;
  }
      ...
      postInspectorPick(msg.selector, frameInfo, msg.basicData, activeTabUrl)
        .then(result => {
          ...
          chrome.runtime.sendMessage({
            ...
            console.warn('[cyberdart bg] Failed to forward inspectResult to sidepanel:', err.message);
          });
          sendResponse({ ok: true });
        });
    });
  ...
  if (msg.type === 'pickerCancelled') {
    chrome.runtime.sendMessage({ type: 'pickerCancelled' }).catch((err) => {
      console.debug('[cyberdart bg] No listener for pickerCancelled:', err.message);
    });
    return;
  }
  ...
  if (msg.type === 'sidebar-command') {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      fetch(`${base}/sidebar-command`, {
        ...
        .then(r => {
          if (!r.ok) {
            console.error(`[cyberdart bg] sidebar-command failed: ${r.status} ${r.statusText}`);
        ...
        .catch(err => {
          console.error('[cyberdart bg] sidebar-command error:', err.message);
...
if (chrome.sidePanel && chrome.sidePanel.setPanelBehavior) {
  chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true }).catch((err) => {
    console.warn('[cyberdart bg] Failed to set panel behavior:', err.message);
  });
}
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      if (wins.length > 0) {
        ...
        console.log(`[cyberdart] Side panel opened on attempt ${attempt + 1}`);
        return; // success
      }
    } catch (e) {
      // May throw if window isn't ready or user gesture required
      console.log(`[cyberdart] Side panel open attempt ${attempt + 1} failed:`, e.message);
    }
    // Backoff: 500ms, 1000ms, 2000ms, 3000ms, 5000ms
    await new Promise(r => setTimeout(r, [500, 1000, 2000, 3000, 5000][attempt]));
  }
  console.log('[cyberdart] Side panel auto-open failed after 5 attempts');
}
  ...
  loadPort().then(() => {
    ...
    const startupCheck = setInterval(async () => {
      ...
      if (isConnected || startupAttempts >= 15) {
        ...
        if (!isConnected) {
          console.log('[cyberdart] Startup health checks failed after 15 attempts, falling back to 10s polling');
        }
      }
    }, 1000);
  });
