/**
 * cyberdart browse — content script
 *
...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'cyberdart-status-pill';
  ...
  statusPill.innerHTML = `<span class="cyberdart-pill-dot"></span> cyberdart${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'cyberdart-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'cyberdart-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'cyberdart-ref-panel';

  const header = document.createElement('div');
  header.className = 'cyberdart-ref-panel-header';
  header.textContent = `cyberdart refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'cyberdart-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'cyberdart-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'cyberdart-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'cyberdart-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'cyberdart-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'cyberdart-ref-panel-more';
...
// Do NOT dispatch cyberdart-extension-ready here — the extension being loaded
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'sidebarOpened') {
    document.dispatchEvent(new CustomEvent('cyberdart-extension-ready'));
    return;
  }
