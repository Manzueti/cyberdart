...
(function () {
  ...
  if (!Terminal) {
    console.error('[cyberdart terminal] xterm not loaded');
    return;
  }
   ...
   * exposes the relevant fields on window.cyberdartHealth (set below in init()).
   * If terminalPort is missing, the agent isn't ready yet.
   */
  function getHealth() {
    return window.cyberdartHealth || {};
  }

  function getServerPort() {
    return window.cyberdartServerPort || null;
  }

  function getAuthToken() {
    return window.cyberdartAuthToken || null;
  }
   ...
   */
  window.cyberdartInjectToTerminal = function (text) {
  ...
  async function connect() {
    ...
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`cyberdart-pty.${ptySessionToken}`]);
    ...
    ws.addEventListener('error', (err) => {
      console.error('[cyberdart terminal] ws error', err);
    });
  }
  ...
  function init() {
    ...
    document.addEventListener('cyberdart:tab-state', (ev) => {
   ...
   * window.cyberdartServerPort + window.cyberdartAuthToken (which it does as soon
