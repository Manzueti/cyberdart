...
description: |
  ...
  Proactively suggest at the end of a work week or sprint. (cyberdart)
allowed-tools:
  - Bash
  - Read
  - Write
...
gbrain:
  ...
  context_queries:
    - id: prior-retros
      kind: filesystem
      glob: "~/.cyberdart/projects/{repo_slug}/retros/*.md"
      sort: mtime_desc
      limit: 5
      render_as: "## Prior retros for this project"
    - id: recent-timeline
      kind: filesystem
      glob: "~/.cyberdart/projects/{repo_slug}/timeline.jsonl"
      tail: 30
      render_as: "## Recent timeline events"
    - id: recent-learnings
      kind: filesystem
      glob: "~/.cyberdart/projects/{repo_slug}/learnings.jsonl"
      tail: 10
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"retro","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
[ -f ~/.cyberdart/retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```

If `RETRO_CONTEXT_FOUND`: read `~/.cyberdart/retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
cat ~/.cyberdart/greptile-history.md 2>/dev/null || true
...
# 12. cyberdart skill usage telemetry (if available)
cat ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Greptile signal (if history exists):** Read `~/.cyberdart/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `~/.cyberdart/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `~/.cyberdart/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**Note:** Only include the `greptile` field if `~/.cyberdart/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)"
cat ~/.cyberdart/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
```bash
DISCOVER_BIN=""
[ -x ~/.claude/skills/cyberdart/bin/cyberdart-global-discover ] && DISCOVER_BIN=~/.claude/skills/cyberdart/bin/cyberdart-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .claude/skills/cyberdart/bin/cyberdart-global-discover ] && DISCOVER_BIN=.claude/skills/cyberdart/bin/cyberdart-global-discover
[ -z "$DISCOVER_BIN" ] && which cyberdart-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which cyberdart-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/cyberdart-global-discover.ts ] && DISCOVER_BIN="bun run bin/cyberdart-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the cyberdart directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/cyberdart-discover-stderr
```

Read the stderr output from `/tmp/cyberdart-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: cyberdart-global-discover" + "feat: /retro global template".
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.cyberdart/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.cyberdart/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.cyberdart/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: cyberdart (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.cyberdart/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
