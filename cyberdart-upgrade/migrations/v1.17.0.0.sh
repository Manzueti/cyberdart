...
#
# Pre-1.17.0.0 /setup-gbrain wrote ~/.cyberdart/consumers.json with a placeholder
...
#   - No ~/.cyberdart/.git (brain-init never ran)
...
# CLI is a benign skip rather than blocking the rest of /cyberdart-upgrade.
set -euo pipefail
...
WIREUP_BIN="${BIN_DIR}/cyberdart-gbrain-source-wireup"

# Skip if user never opted into brain-sync.
SYNC_MODE=""
if [ -x "$CONFIG_BIN" ]; then
  # Trim whitespace defensively: cyberdart-config can emit trailing newlines,
