...
# shadowing the cyberdart checkpoint skill. The skill has been split into
...
# ~/.claude/skills/cyberdart/. A user's own /checkpoint skill (regular file,
# or symlink pointing elsewhere) is preserved.
#
# Three supported install shapes to handle:
#   1. ~/.claude/skills/checkpoint is a directory symlink into cyberdart.
#   2. ~/.claude/skills/checkpoint is a regular directory whose ONLY file
#      is a SKILL.md symlink into cyberdart (cyberdart's prefix-install shape).
...
OLD_NAMESPACED="${SKILLS_DIR}/cyberdart/checkpoint"
CYBERDART_ROOT_REAL=""
...
# Resolve the canonical path of the cyberdart skills root. If cyberdart isn't
# installed here, there's nothing to migrate.
if [ -d "${SKILLS_DIR}/cyberdart" ]; then
  CYBERDART_ROOT_REAL=$(resolve_real "${SKILLS_DIR}/cyberdart")
fi
...
if [ -L "$OLD_TOPLEVEL" ]; then
  ...
  if [ -n "$CYBERDART_ROOT_REAL" ] && path_inside "$target_real" "$CYBERDART_ROOT_REAL"; then
    rm -- "$OLD_TOPLEVEL"
    echo "  [v1.1.3.0] Removed stale /checkpoint symlink (was shadowing Claude Code's /rewind alias)."
    removed_any=1
  else
    echo "  [v1.1.3.0] Leaving $OLD_TOPLEVEL alone — symlink target is outside cyberdart (or unresolvable)."
  fi
elif [ -d "$OLD_TOPLEVEL" ]; then
  # Regular directory. Only remove if it contains exactly one file named
  # SKILL.md that's a symlink into cyberdart (cyberdart's prefix-install shape).
  ...
  if [ "$file_count" = "0" ] && [ "$symlink_count" = "1" ] && [ -L "$OLD_TOPLEVEL/SKILL.md" ]; then
    ...
    if [ -n "$CYBERDART_ROOT_REAL" ] && path_inside "$target_real" "$CYBERDART_ROOT_REAL"; then
      ...
      rm -r -- "$OLD_TOPLEVEL"
      echo "  [v1.1.3.0] Removed stale /checkpoint install directory (cyberdart prefix-mode)."
      removed_any=1
    else
      echo "  [v1.1.3.0] Leaving $OLD_TOPLEVEL alone — SKILL.md symlink target is outside cyberdart."
    fi
  else
    echo "  [v1.1.3.0] Leaving $OLD_TOPLEVEL alone — not a cyberdart-owned install (has custom content)."
  fi
fi
# Missing → no-op (idempotency).

# --- Shape 2: ~/.claude/skills/cyberdart/checkpoint/
# Ownership guard applies here too: only remove if this path resolves inside the
# cyberdart skills root. If a user replaced the directory with a symlink pointing
# elsewhere (e.g., at their own fork), respect it.
if [ -L "$OLD_NAMESPACED" ]; then
  target_real=$(resolve_real "$OLD_NAMESPACED")
  if [ -n "$CYBERDART_ROOT_REAL" ] && path_inside "$target_real" "$CYBERDART_ROOT_REAL"; then
    rm -- "$OLD_NAMESPACED"
    echo "  [v1.1.3.0] Removed stale ~/.claude/skills/cyberdart/checkpoint symlink."
    removed_any=1
  else
    echo "  [v1.1.3.0] Leaving $OLD_NAMESPACED alone — symlink target is outside cyberdart."
  fi
elif [ -d "$OLD_NAMESPACED" ]; then
  # Regular directory. This is the cyberdart-prefix install location. Check that
  # it resolves to a path inside the cyberdart root (it should, unless someone
  # hand-edited the tree).
  target_real=$(resolve_real "$OLD_NAMESPACED")
  if [ -n "$CYBERDART_ROOT_REAL" ] && path_inside "$target_real" "$CYBERDART_ROOT_REAL"; then
    rm -rf -- "$OLD_NAMESPACED"
    echo "  [v1.1.3.0] Removed stale ~/.claude/skills/cyberdart/checkpoint/ (replaced by context-save + context-restore)."
    removed_any=1
  else
    echo "  [v1.1.3.0] Leaving $OLD_NAMESPACED alone — resolves outside cyberdart."
  fi
fi
