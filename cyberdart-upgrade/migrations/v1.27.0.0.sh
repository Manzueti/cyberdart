#!/usr/bin/env bash
# Migration: v1.27.0.0 — rename cyberdart-brain-* → cyberdart-artifacts-*
#
# Phase C of the v1.27.0.0 plan. Hard-rename, no compat shim. Steps:
#   1. gh_repo_renamed       — gh/glab repo rename cyberdart-brain-$USER →
#                              cyberdart-artifacts-$USER (skipped on user opt-out)
#   2. remote_txt_renamed    — mv ~/.cyberdart-brain-remote.txt → artifacts-remote.txt
#   3. config_key_renamed    — rewrite gbrain_sync_mode → artifacts_sync_mode
#                              in ~/.cyberdart/config.yaml
...
#
# Interruption-safe via journal at ~/.cyberdart/.migrations/v1.27.0.0.journal:
# each step writes its name on success; re-entry resumes from the next un-done
# step. Done touchfile at ~/.cyberdart/.migrations/v1.27.0.0.done.
#
...
# Configuration
# ---------------------------------------------------------------------------
CYBERDART_HOME="${HOME}/.cyberdart"
SKILLS_DIR="${HOME}/.claude/skills"
BIN_DIR="${SKILLS_DIR}/cyberdart/bin"
CONFIG_BIN="${BIN_DIR}/cyberdart-config"
URL_BIN="${BIN_DIR}/cyberdart-artifacts-url"

MIGRATION_DIR="${CYBERDART_HOME}/.migrations"
...
OLD_REPO_NAME="cyberdart-brain-${USER_NAME}"
NEW_REPO_NAME="cyberdart-artifacts-${USER_NAME}"
OLD_REMOTE_TXT="${HOME}/.cyberdart-brain-remote.txt"
NEW_REMOTE_TXT="${HOME}/.cyberdart-artifacts-remote.txt"
...
#   - presence of ~/.cyberdart-brain-remote.txt (legacy file)
#   - presence of ~/.cyberdart/.git (brain-init ever ran)
HAS_LEGACY_STATE=0
[ -f "$OLD_REMOTE_TXT" ] && HAS_LEGACY_STATE=1
[ -d "$CYBERDART_HOME/.git" ] && HAS_LEGACY_STATE=1

# If nothing to migrate, finalize silently.
if [ "$HAS_LEGACY_STATE" = "0" ]; then
  echo "  [v1.27.0.0] no legacy cyberdart-brain state detected — nothing to migrate." >&2
  touch "$DONE"
...
if [ ! -f "$JOURNAL" ]; then
  ...
  [v1.27.0.0] cyberdart-brain has been renamed to cyberdart-artifacts.
  ...
  This migration will:
    ...
    2. mv ~/.cyberdart-brain-remote.txt → ~/.cyberdart-artifacts-remote.txt
    3. Rename gbrain_sync_mode → artifacts_sync_mode in ~/.cyberdart/config.yaml
...
# ---------------------------------------------------------------------------
# Step 2: rename ~/.cyberdart-brain-remote.txt → ~/.cyberdart-artifacts-remote.txt
# ---------------------------------------------------------------------------
if ! journal_done "remote_txt_renamed"; then
  echo "  [v1.27.0.0] step 2: rename ~/.cyberdart-brain-remote.txt → ~/.cyberdart-artifacts-remote.txt" >&2
  if [ -f "$OLD_REMOTE_TXT" ] && [ ! -f "$NEW_REMOTE_TXT" ]; then
    # Update the URL inside if the rename happened on the host: replace
    # cyberdart-brain-$USER with cyberdart-artifacts-$USER in the URL.
...
if ! journal_done "config_key_renamed"; then
  ...
  CFG="$CYBERDART_HOME/config.yaml"
  if [ -f "$CFG" ]; then
...
if ! journal_done "claude_md_block_rewritten"; then
  ...
  # Look in cwd's CLAUDE.md (where /setup-gbrain wrote it) and ~/.cyberdart/CLAUDE.md
  ...
  for CMD in "$PWD/CLAUDE.md" "$CYBERDART_HOME/CLAUDE.md"; do
    [ -f "$CMD" ] || continue
...
EOF
  ...
  elif command -v gbrain >/dev/null 2>&1 && [ -d "$CYBERDART_HOME/.git" ]; then
    ...
    WORKTREE="${CYBERDART_BRAIN_WORKTREE:-$HOME/.cyberdart-brain-worktree}"
    ...
    echo "    gbrain CLI not available or no ~/.cyberdart/.git — skipping" >&2
    mark_done "sources_swapped"
  fi
fi
