...
# the older V0 prose can set `cyberdart-config set explain_level terse`.
#
...
PROMPTED_FLAG="$CYBERDART_HOME/.writing-style-prompted"
PENDING_FLAG="$CYBERDART_HOME/.writing-style-prompt-pending"

mkdir -p "$CYBERDART_HOME"
...
EXPLAIN_LEVEL_SET="$("${HOME}/.claude/skills/cyberdart/bin/cyberdart-config" get explain_level 2>/dev/null || true)"
