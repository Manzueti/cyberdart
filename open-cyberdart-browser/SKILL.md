...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"open-cyberdart-browser","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"open-cyberdart-browser","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"open-cyberdart-browser","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.cyberdart/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.cyberdart/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.cyberdart/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.cyberdart/chromium-profile"
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.cyberdart/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.claude/skills/cyberdart/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.claude/skills/cyberdart/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.claude/skills/cyberdart/extension/manifest.json" ] && _EXT_PATH="$HOME/.claude/skills/cyberdart/extension"
...
> Chrome is launched with cyberdart control. You should see Playwright's Chromium
...
>    already show the cyberdart icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **cyberdart browse** → click the **pin icon**
> 3. Click the pinned **cyberdart icon** in the toolbar
...
> 2. Look for **"cyberdart browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the cyberdart icon
...
> - Run any cyberdart skill (`/qa`, `/design-review`, `/benchmark`) and watch
