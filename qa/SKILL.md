...
description: |
  ...
  fix evidence, and a ship-readiness summary. For report-only mode, use /qa-only. (cyberdart)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
| Mode | full | `--regression .cyberdart/qa-reports/baseline.json` |
| Output dir | `.cyberdart/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .cyberdart/qa-reports/screenshots
```
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/cyberdart/bin/cyberdart-learnings-search --limit 10 --query "qa testing bug regression flake fixture" --cross-project 2>/dev/null || true
else
  ~/.claude/skills/cyberdart/bin/cyberdart-learnings-search --limit 10 --query "qa testing bug regression flake fixture" 2>/dev/null || true
fi
```
...
1. **Project-scoped test plans:** Check `~/.cyberdart/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)"
   ls -t ~/.cyberdart/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .cyberdart/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.cyberdart/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)" && mkdir -p ~/.cyberdart/projects/$SLUG
```
Write to `~/.cyberdart/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
