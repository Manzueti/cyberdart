/**
 * cyberdart-memory-helpers — shared helpers for the V1 memory ingest + retrieval pipeline.
 *
 * Imported by:
 *   - bin/cyberdart-memory-ingest.ts (Lane A)
 *   - bin/cyberdart-gbrain-sync.ts   (Lane B)
 *   - bin/cyberdart-brain-context-load.ts (Lane C)
 ...
 *   ED1 — engine-tier cache lives in ~/.cyberdart/.gbrain-engine-cache.json (60s TTL)
 *
 ...
 *
 * Examples:
 *   https://github.com/garrytan/cyberdart.git → github.com/garrytan/cyberdart
 *   git@github.com:garrytan/cyberdart.git     → github.com/garrytan/cyberdart
...
function gitleaksAvailable(): boolean {
  ...
  try {
    ...
    process.stderr.write(
      "[cyberdart-memory-helpers] gitleaks not in PATH; secret scanning disabled. " +
 ...
 * Replaces the inadequate regex scanner in bin/cyberdart-brain-sync (which only
...
function cyberdartHome(): string {
  return process.env.CYBERDART_HOME || join(homedir(), ".cyberdart");
}

function engineCachePath(): string {
  return join(cyberdartHome(), ".gbrain-engine-cache.json");
}

function errorLogPath(): string {
  return join(cyberdartHome(), ".gbrain-errors.jsonl");
}

/**
 * Detect which gbrain engine is active (PGLite vs Supabase) and cache the
 * answer for 60s in ~/.cyberdart/.gbrain-engine-cache.json. Caching avoids
...
export function detectEngineTier(): EngineDetect {
  ...
  try {
    ...
    writeFileSync(
      engineCachePath(),
      JSON.stringify({ ...fresh, last_writer: "cyberdart-memory-helpers.detectEngineTier" }, null, 2),
      "utf-8"
    );
  } catch {
 ...
 * runtime read path used by cyberdart-brain-context-load; it tolerates extra
...
const ERROR_LOG_PATH = join(cyberdartHome(), ".gbrain-errors.jsonl");

/**
 * Wrap an op with structured error logging. Logs success/failure + duration
 * to ~/.cyberdart/.gbrain-errors.jsonl for forensic debugging. Replaces ad-hoc
