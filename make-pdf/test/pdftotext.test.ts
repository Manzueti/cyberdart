...
describe("resolvePdftotext (override resolution, v1.24-aligned)", () => {
  test("honors CYBERDART_PDFTOTEXT_BIN when it points at a real executable", () => {
    ...
    const info = withEnv({ CYBERDART_PDFTOTEXT_BIN: REAL_EXE }, () => resolvePdftotext());
    expect(info.bin).toBe(REAL_EXE);
  });

  test("honors PDFTOTEXT_BIN as a back-compat alias", () => {
    const info = withEnv(
      { CYBERDART_PDFTOTEXT_BIN: undefined, PDFTOTEXT_BIN: REAL_EXE },
      () => resolvePdftotext(),
    );
    expect(info.bin).toBe(REAL_EXE);
  });

  test("CYBERDART_PDFTOTEXT_BIN takes precedence over PDFTOTEXT_BIN", () => {
    const info = withEnv(
      { CYBERDART_PDFTOTEXT_BIN: REAL_EXE, PDFTOTEXT_BIN: "/nonexistent/legacy" },
      () => resolvePdftotext(),
    );
    expect(info.bin).toBe(REAL_EXE);
  });
  ...
  test("error message includes Windows install hint and CYBERDART_PDFTOTEXT_BIN", () => {
    let thrown: unknown = null;
    try {
      withEnv(
        {
          CYBERDART_PDFTOTEXT_BIN: "/nonexistent/cyberdart-pdftotext",
    ...
    if (thrown) {
      ...
      expect((thrown as Error).message).toContain("CYBERDART_PDFTOTEXT_BIN");
