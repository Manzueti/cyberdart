...
describe("resolveBrowseBin", () => {
  test("throws BrowseClientError with setup hint when nothing is found", () => {
    ...
    try {
      withEnv(
        {
          CYBERDART_BROWSE_BIN: "/nonexistent/cyberdart-browse-bin",
    ...
    if (thrown) {
      ...
      expect((thrown as BrowseClientError).message).toContain("CYBERDART_BROWSE_BIN");
  ...
  test("honors CYBERDART_BROWSE_BIN when it points at a real executable", () => {
    const resolved = withEnv({ CYBERDART_BROWSE_BIN: REAL_EXE }, () => resolveBrowseBin());
    expect(resolved).toBe(REAL_EXE);
  });

  test("honors BROWSE_BIN as a back-compat alias", () => {
    const resolved = withEnv(
      { CYBERDART_BROWSE_BIN: undefined, BROWSE_BIN: REAL_EXE },
      () => resolveBrowseBin(),
    );
    expect(resolved).toBe(REAL_EXE);
  });

  test("CYBERDART_BROWSE_BIN takes precedence over BROWSE_BIN", () => {
    const resolved = withEnv(
      { CYBERDART_BROWSE_BIN: REAL_EXE, BROWSE_BIN: "/nonexistent/legacy" },
      () => resolveBrowseBin(),
    );
    expect(resolved).toBe(REAL_EXE);
  });
