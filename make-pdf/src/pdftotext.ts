 ...
 *   1. $CYBERDART_PDFTOTEXT_BIN env override (preferred, matches v1.24 CYBERDART_*_BIN pattern)
 ...
 *      and guessing causes false positives. Set CYBERDART_PDFTOTEXT_BIN explicitly.
...
export function resolvePdftotext(env: NodeJS.ProcessEnv = process.env): PdftotextInfo {
  // 1 + 2: env overrides (CYBERDART_PDFTOTEXT_BIN preferred, PDFTOTEXT_BIN back-compat).
  const overrideRaw = env.CYBERDART_PDFTOTEXT_BIN ?? env.PDFTOTEXT_BIN;
  ...
  // positives. Windows users set CYBERDART_PDFTOTEXT_BIN explicitly.
  ...
  throw new PdftotextUnavailableError([
    ...
    "",
    "Or set CYBERDART_PDFTOTEXT_BIN to an explicit path:",
    process.platform === "win32"
      ? '  setx CYBERDART_PDFTOTEXT_BIN "C:\\path\\to\\pdftotext.exe"'
      : "  export CYBERDART_PDFTOTEXT_BIN=/path/to/pdftotext",
  ].join("\n"));
}
