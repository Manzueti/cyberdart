...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"skillify","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"skillify","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"skillify","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
A) Keep "<proposed-name>" at global tier — ~/.cyberdart/browser-skills/<proposed-name>/  (recommended)
B) Keep "<proposed-name>" but at project tier — <project>/.cyberdart/browser-skills/<proposed-name>/
...
The canonical SDK lives at `<cyberdart-install>/browse/src/browse-client.ts`.
The bundled-skill loader walks the install tree to find it; mirror that.

Resolve the cyberdart install dir. Two reliable signals (in order):

1. The bundled `hackernews-frontpage` skill — look at its tier path from
   `$B skill list` (the `bundled` row). The skill dir is
   `<cyberdart-install>/browser-skills/hackernews-frontpage/`, so the install
   dir is two `dirname` calls above its `_lib/browse-client.ts`.
2. The active cyberdart skills install at `~/.claude/skills/cyberdart/`. Read
...
function resolveSdkPath(): string {
  const candidates = [
    path.join(os.homedir(), '.claude', 'skills', 'cyberdart', 'browse', 'src', 'browse-client.ts'),
...
```ts
import { stageSkill } from '<cyberdart-install>/browse/src/browser-skill-write';
...
2. If still failing after two retries, OR the failure is an
   ...
   ```ts
   import { discardStaged } from '<cyberdart-install>/browse/src/browser-skill-write';
...
moves the staged folder into ~/.cyberdart/browser-skills/ where /scrape
...
```ts
import { commitSkill } from '<cyberdart-install>/browse/src/browser-skill-write';
...
- **Bun runtime required.** The codified skill runs as a Bun process
  ...
  For now: the skill works on any machine that has cyberdart installed,
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-learnings-log '{"skill":"skillify","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
