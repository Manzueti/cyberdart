-- cyberdart attack telemetry — schema extension for prompt injection events.
--
-- Ships alongside the cyberdart-telemetry-log `--event-type attack_attempt`
-- flag (bin/cyberdart-telemetry-log, commits 28ce883c + f68fa4a9). These
-- columns are nullable so the existing skill_run events continue inserting
-- unchanged.
--
-- Fields (1:1 with cyberdart-telemetry-log flags):
...
--   * salt is per-device local file (~/.cyberdart/security/device-salt) —
