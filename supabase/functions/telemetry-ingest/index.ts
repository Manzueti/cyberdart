// cyberdart telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/cyberdart-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.cyberdart_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        cyberdart_version: String(event.cyberdart_version).slice(0, 20),
      ...
      if (event.installation_id) {
        installationUpserts.set(event.installation_id, {
          version: event.cyberdart_version,
          os: event.os,
        });
      }
    }
        ...
        .upsert(
          {
            ...
            cyberdart_version: data.version,
            os: data.os,
          },
