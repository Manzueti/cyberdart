// cyberdart community-pulse edge function
...
Deno.serve(async () => {
  ...
  try {
    ...
    const { data: crashes } = await supabase
      .from("crash_clusters")
      .select("error_class, cyberdart_version, total_occurrences, identified_users")
      .limit(5);
    ...
    const { data: versionRows } = await supabase
      .from("telemetry_events")
      .select("cyberdart_version")
      .eq("event_type", "skill_run")
      .gte("event_timestamp", weekAgo)
      .limit(1000);

    for (const row of versionRows ?? []) {
      if (row.cyberdart_version) {
        versionCounts[row.cyberdart_version] = (versionCounts[row.cyberdart_version] ?? 0) + 1;
      }
    }
    ...
    // Fields emitted by cyberdart-telemetry-log --event-type attack_attempt:
    ...
    // cyberdart user who polls /community-pulse. With it, the dashboard shows
