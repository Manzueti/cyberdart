...
description: |
  ...
  "codex challenge", "ask codex", "second opinion", or "consult codex". (cyberdart)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"codex","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"codex","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"codex","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
source ~/.claude/skills/cyberdart/bin/cyberdart-codex-probe 2>/dev/null && _cyberdart_codex_log_event "codex_cli_missing" 2>/dev/null || true
```
...
CLI version isn't in the known-bad list. Sourcing `cyberdart-codex-probe` loads the
...
if ! _cyberdart_codex_auth_probe >/dev/null; then
  _cyberdart_codex_log_event "codex_auth_failed"
  echo "AUTH_FAILED"
fi
_cyberdart_codex_version_check   # warns if known-bad, non-blocking
```
...
**Update the known-bad list** in `bin/cyberdart-codex-probe` when a new Codex CLI version
...
(where ephemeral codex stderr / response captures land) via `bin/cyberdart-paths`.
This keeps the skill working whether installed as a Claude Code plugin
(`CLAUDE_PLANS_DIR` set), a global `~/.claude/skills/cyberdart/` install, or a CI
...
_cyberdart_codex_timeout_wrapper 330 codex review "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .claude/skills/, or agents/. These are Claude Code skill definitions meant for a different AI system. Do NOT modify agents/openai.yaml. Stay focused on repository code only." --base <base> -c 'model_reasoning_effort="high"' --enable web_search_cached < /dev/null 2>"$TMPERR"
_CODEX_EXIT=$?
if [ "$_CODEX_EXIT" = "124" ]; then
  _cyberdart_codex_log_event "codex_timeout" "330"
  _cyberdart_codex_log_hang "review" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
_cyberdart_codex_timeout_wrapper 600 codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 "$PYTHON_CMD" -u -c "
import sys, json
...
if [ "$_CODEX_EXIT" = "124" ]; then
  ...
  _cyberdart_codex_log_hang "challenge" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
...
_cyberdart_codex_timeout_wrapper 600 codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 "$PYTHON_CMD" -u -c "
import sys, json
for line in sys.stdin:
    line = line.strip()
    if not line: continue
...
if [ "$_CODEX_EXIT" = "124" ]; then
  _cyberdart_codex_log_event "codex_timeout" "600"
  _cyberdart_codex_log_hang "consult" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
  echo "Codex stalled past 10 minutes. Common causes: model API stall, long prompt, network issue. Try re-running. If persistent, split the prompt or check ~/.codex/logs/."
fi
```
...
# Fix 1: wrap with timeout (gtimeout/timeout fallback chain via probe helper)
_cyberdart_codex_timeout_wrapper 600 codex exec resume <session-id> "<prompt>" -c 'sandbox_mode="read-only"' -c 'model_reasoning_effort="medium"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 "$PYTHON_CMD" -u -c "
...
if [ "$_CODEX_EXIT" = "124" ]; then
  _cyberdart_codex_log_event "codex_timeout" "600"
  _cyberdart_codex_log_hang "consult-resume" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
  echo "Codex stalled past 10 minutes. Common causes: model API stall, long prompt, network issue. Try re-running. If persistent, split the prompt or check ~/.codex/logs/."
fi
...
- **Detect skill-file rabbit holes.** After receiving Codex output, scan for signs
  that Codex got distracted by skill files: `cyberdart-config`, `cyberdart-update-check`,
  `SKILL.md`, or `skills/cyberdart`. If any of these appear in the output, append a
  warning: "Codex appears to have read cyberdart skill files instead of reviewing your
