 ...
 *
 * Run: OPENAI_API_KEY=$(cat ~/.cyberdart/openai.json | python3 -c "import sys,json;print(json.load(sys.stdin)['api_key'])") bun run design/prototype.ts
 */
...
if (!API_KEY) {
  console.error("No API key found. Set OPENAI_API_KEY or save to ~/.cyberdart/openai.json");
  process.exit(1);
}

const OUTPUT_DIR = "/tmp/cyberdart-prototype-" + Date.now();
