 ...
 *
 * Resolution order:
 * 1. ~/.cyberdart/openai.json → { "api_key": "sk-..." }
...
const CONFIG_PATH = path.join(process.env.HOME || "~", ".cyberdart", "openai.json");

export function resolveApiKey(): string | null {
  // 1. Check ~/.cyberdart/openai.json
  try {
...
/**
 * Save an API key to ~/.cyberdart/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!key) {
    ...
    console.error("  or save to ~/.cyberdart/openai.json: { \"api_key\": \"sk-...\" }");
