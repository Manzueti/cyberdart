...
cyberdart's browser surface in one document. Headless Chromium daemon, ~70+
...
If you've used cyberdart in the last release or two, the productivity loop is the
...
B=./browse/dist/browse           # or ~/.claude/skills/cyberdart/browse/dist/browse
...
/skillify                        # writes ~/.cyberdart/browser-skills/hn-front/...
...
The shipped headline of v1.19.0.0. Two cyberdart skills wrap the browser-skills
...
- **D3 — Atomic write.** Stages everything to `~/.cyberdart/.tmp/skillify-<spawnId>/`,
...
1. **First call.** CLI checks `<project>/.cyberdart/browse.json` for a running
...
collisions. State at `<project>/.cyberdart/browse.json`.

| Workspace | State file | Port |
|-----------|-----------|------|
| `/code/project-a` | `/code/project-a/.cyberdart/browse.json` | random (10000–60000) |
| `/code/project-b` | `/code/project-b/.cyberdart/browse.json` | random (10000–60000) |
...
| Tier | Path | When |
|------|------|------|
| **Project** | `<project>/.cyberdart/browser-skills/<name>/` | Project-specific skills (committed or gitignored) |
| **Global** | `~/.cyberdart/browser-skills/<name>/` | Per-user skills, all projects |
| **Bundled** | `<cyberdart-install>/browser-skills/<name>/` | Ships with cyberdart, read-only |
...
| **Process-side env** | `trusted: true` frontmatter passes `process.env` minus `CYBERDART_TOKEN`. `trusted: false` (default) drops everything except a minimal allowlist (LANG, LC_ALL, TERM, TZ) and pattern-strips secrets (TOKEN/KEY/SECRET/PASSWORD, AWS_*, ANTHROPIC_*, OPENAI_*, GITHUB_*, etc.) | Untrusted (must opt in) |

`CYBERDART_PORT` and `CYBERDART_SKILL_TOKEN` are injected last, so a parent process
can't override them.
...
- `stageSkill(opts)` — writes files to `~/.cyberdart/.tmp/skillify-<spawnId>/<name>/`
...
Storage:
- Per-project: `<project>/.cyberdart/domain-skills/<host>.md`
- Global: `~/.cyberdart/domain-skills/<host>.md`
...
work without captchas), a custom user agent, and the cyberdart extension
...
pipe text into the live Claude PTY via `window.cyberdartInjectToTerminal(text)`,
...
that accepts the Bearer token OR the HttpOnly `cyberdart_sse` session cookie
...
| Content script | `extension/content.js` | Page overlays, `cyberdart` pill |
...
```bash
bin/cyberdart-extension    # opens chrome://extensions, copies path to clipboard
```

Or do it manually: `chrome://extensions` → toggle Developer mode → Load
unpacked → navigate to `~/.claude/skills/cyberdart/extension` → pin the
...
HttpOnly `cyberdart_sse` cookie (never valid against `/command`).
...
`~/.cyberdart/security/attempts.jsonl` — append-only, salted SHA-256 of source
+ domain only (no raw IP, no full request body), rotates at 10MB with 5
generations. Per-device salt at `~/.cyberdart/security/device-salt` (mode 0600).
...
The root token is written to `<project>/.cyberdart/browse.json` with chmod 600.
...
token OR a 30-minute HttpOnly `cyberdart_sse` cookie minted via
...
The Terminal pane uses a separate session cookie, `cyberdart_pty`, minted via
...
- `CYBERDART_SECURITY_OFF=1` — emergency kill switch. Classifier stays off
...
- Classifier model cache: `~/.cyberdart/models/testsavant-small/` (112MB, first
  run only) plus `~/.cyberdart/models/deberta-v3-injection/` (721MB, only when
  ensemble enabled).
- Attack log: `~/.cyberdart/security/attempts.jsonl` (salted SHA-256 + domain
  only, rotates at 10MB, 5 generations).
- Per-device salt: `~/.cyberdart/security/device-salt` (0600).
- Session state: `~/.cyberdart/security/session-state.json` (cross-process,
  atomic).
...
(override via `CYBERDART_BROWSE_MAX_HTML_BYTES`).
...
- Console: `.cyberdart/browse-console.log`
- Network: `.cyberdart/browse-network.log`
- Dialog: `.cyberdart/browse-dialog.log`
...
```bash
$B state save my-session         # save cookies + URLs to .cyberdart/browse-state-my-session.json
...
in `.cyberdart/browser-scout.jsonl`.
...
| **cyberdart browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |
| **cyberdart browse + codified skill** | **~3s** | **~200ms** | **0 tokens** (single skill invocation) |

In a 20-command browser session, MCP tools burn 30,000–40,000 tokens on
protocol framing alone. cyberdart burns zero. The codified-skill path takes a
...
cyberdart skips all of this. Compiled binary. Plain text in, plain text out.
...
project-tier skill at `/code/project-a/.cyberdart/browser-skills/foo/` shadows
the global `~/.cyberdart/browser-skills/foo/` only inside project-a.
...
| `BROWSE_STATE_FILE` | `.cyberdart/browse.json` | Path to state file |
...
| `CYBERDART_BROWSE_MAX_HTML_BYTES` | 52428800 (50MB) | `load-html` size cap |
| `CYBERDART_SECURITY_OFF` | unset | Emergency kill switch — disable ML classifier |
| `CYBERDART_SECURITY_ENSEMBLE` | unset | Set to `deberta` for 3-classifier ensemble (721MB download) |
...
│   ├── tunnel-denial-log.ts     # ~/.cyberdart/security/attempts.jsonl writer (salted)
...
scrape/SKILL.md.tmpl             # /scrape cyberdart skill — match-or-prototype entry point
skillify/SKILL.md.tmpl           # /skillify cyberdart skill — codify last /scrape into permanent skill
```
...
lands at `~/.cyberdart/browser-skills/<name>/` after the test passes.
