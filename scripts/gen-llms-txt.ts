#!/usr/bin/env bun
/**
 * Generate cyberdart/llms.txt — a single discoverable index of every cyberdart
 ...
 *
 * Output: cyberdart/llms.txt at repo root.
 *
...
const OUTPUT = path.join(ROOT, 'cyberdart', 'llms.txt');
 ...
 * the cyberdart convention; we collapse those into a single paragraph.
 */
 ...
 * in a full cyberdart checkout; absent on minimal installs. Returns {} if the
...
export async function generateLlmsTxt(opts: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  const lines: string[] = [];
  lines.push('# cyberdart');
  lines.push('');
  lines.push("> cyberdart is Garry's Stack: AI coding skills + a fast headless browser binary + a design CLI. This file indexes every capability so agents can discover and invoke them without crawling individual SKILL.md files.");
  lines.push('');
  ...
  lines.push('## More');
  lines.push('');
  lines.push('- Repository: https://github.com/garrytan/cyberdart');
  ...
  void (async () => {
    ...
    if (dryRun) {
      ...
      if (existing !== result.content) {
        console.error('[gen-llms-txt] OUT OF DATE — run `bun run gen:skill-docs` to regenerate cyberdart/llms.txt');
        process.exit(1);
      }
