#!/usr/bin/env bun
/**
 * analytics — CLI for viewing cyberdart skill usage statistics.
 *
 * Reads ~/.cyberdart/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.cyberdart', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('cyberdart skill usage analytics');
