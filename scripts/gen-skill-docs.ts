...
function externalSkillName(skillDir: string, frontmatterName?: string): string {
  // Root skill (skillDir === '' or '.') always maps to 'cyberdart' regardless of frontmatter
  if (skillDir === '.' || skillDir === '') return 'cyberdart';
  ...
  // Don't double-prefix: cyberdart-upgrade → cyberdart-upgrade (not cyberdart-cyberdart-upgrade)
  if (baseName.startsWith('cyberdart-')) return baseName;
  return `cyberdart-${baseName}`;
}
...
for (const currentHost of hostsToRun) {
  ...
  try {
    ...
    // Generate cyberdart-lite and cyberdart-full for OpenClaw host
    if (currentHost === 'openclaw' && !DRY_RUN) {
      const openclawDir = path.join(ROOT, 'openclaw');
      if (!fs.existsSync(openclawDir)) fs.mkdirSync(openclawDir, { recursive: true });

      const cyberdartLite = `# cyberdart-lite Planning Discipline
...
`;
      fs.writeFileSync(path.join(openclawDir, 'cyberdart-lite-CLAUDE.md'), cyberdartLite);
      console.log('GENERATED: openclaw/cyberdart-lite-CLAUDE.md');

      const cyberdartFull = `# cyberdart-full Pipeline
...
`;
      fs.writeFileSync(path.join(openclawDir, 'cyberdart-full-CLAUDE.md'), cyberdartFull);
      console.log('GENERATED: openclaw/cyberdart-full-CLAUDE.md');

      const cyberdartPlan = `# cyberdart-plan: Full Review Gauntlet
...
5. Report back to the orchestrator:
   ...
   - Recommended next step (usually: spawn a new session with cyberdart-full to implement)

Do not implement anything. This is planning only.
The orchestrator will persist the plan link to its own memory/knowledge store.
`;
      fs.writeFileSync(path.join(openclawDir, 'cyberdart-plan-CLAUDE.md'), cyberdartPlan);
      console.log('GENERATED: openclaw/cyberdart-plan-CLAUDE.md');
    }
...
if (!DRY_RUN) {
  try {
    const configPath = path.join(process.env.HOME || '', '.cyberdart', 'config.yaml');
    if (fs.existsSync(configPath)) {
      const config = fs.readFileSync(configPath, 'utf-8');
      if (/^skill_prefix:\s*true/m.test(config)) {
        console.log('\nNote: skill_prefix is true. Run cyberdart-relink to re-apply name: patches.');
      }
    }
  } catch { /* non-fatal */ }
}

// Regenerate cyberdart/llms.txt — single-file capability index for AI agents.
  ...
  void (async () => {
    try {
      ...
      if (result.warnings.length > 0) {
        ...
        console.log(`[gen-llms-txt] cyberdart/llms.txt: ${result.skills.length} skills, ${result.browseCommands.length} browse commands`);
      }
    } catch (err) {
