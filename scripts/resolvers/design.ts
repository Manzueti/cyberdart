...
\`\`\`bash
source <(${ctx.paths.binDir}/cyberdart-diff-scope <base> 2>/dev/null)
\`\`\`
...
\`\`\`bash
${ctx.paths.binDir}/cyberdart-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
\`\`\`
...
**Log the result:**
\`\`\`bash
${ctx.paths.binDir}/cyberdart-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
Replace STATUS with "clean" or "issues_found", SOURCE with "codex+subagent", "codex-only", "subagent-only", or "unavailable".`;
}

// ─── Design Hard Rules (OpenAI framework + cyberdart slop blacklist) ───
...
the legacy approved.json aggregate — \`${ctx.paths.binDir}/cyberdart-taste-update\`
