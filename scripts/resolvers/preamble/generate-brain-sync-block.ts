 ...
 *   1. If ~/.cyberdart-artifacts-remote.txt (or legacy ~/.cyberdart-brain-remote.txt
 *      during the v1.27.0.0 migration window) exists AND ~/.cyberdart/.git is
 *      missing, surface a restore-available hint (does NOT auto-run restore).
 *   2. If sync is on, run `cyberdart-brain-sync --once` (drain + push). The
 ...
 * to `cyberdart-brain-sync --discover-new` + `--once`.
 */
...
_BRAIN_SYNC_BIN="${ctx.paths.binDir}/cyberdart-brain-sync"
_BRAIN_CONFIG_BIN="${ctx.paths.binDir}/cyberdart-config"
...
${isBrainHost ? `If output shows \`ARTIFACTS_SYNC: artifacts repo detected\`, offer \`cyberdart-brain-restore\` via AskUserQuestion; otherwise continue.` : ''}
...
\`\`\`bash
"${ctx.paths.binDir}/cyberdart-brain-sync" --discover-new 2>/dev/null || true
"${ctx.paths.binDir}/cyberdart-brain-sync" --once 2>/dev/null || true
\`\`\`
`;
}
