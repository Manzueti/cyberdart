...
export interface HostConfig {
  ...
  // --- Path Configuration ---
  /** Global install path relative to $HOME (e.g., '.config/opencode/skills/cyberdart'). */
  globalRoot: string;
  /** Project-local skill path relative to repo root (e.g., '.opencode/skills/cyberdart'). */
  localSkillRoot: string;
  /** Gitignored directory under repo root for generated docs (e.g., '.opencode'). */
  hostSubdir: string;
  /** Whether preamble generates $CYBERDART_ROOT env vars (true for non-Claude hosts). */
  usesEnvVars: boolean;
  ...
  /** Optional repo-local sidecar config (e.g., Codex uses .agents/skills/cyberdart). */
  sidecar?: {
    /** Sidecar path relative to repo root (e.g., '.agents/skills/cyberdart'). */
    path: string;
  ...
  // --- Install Behavior ---
  install: {
    /** Whether cyberdart-config skill_prefix applies (Claude only). */
    prefixable: boolean;
