 ...
 *      comes from --email CLI flags (repeatable), the CYBERDART_AUTHOR_EMAILS env
 ...
 *      CYBERDART_AUTHOR_EMAILS=a@x.com,b@y.com bun run scripts/garry-output-comparison.ts
...
function resolveAuthorEmails(argv: string[]): string[] {
  ...
  const envVar = process.env.CYBERDART_AUTHOR_EMAILS;
  ...
  process.stderr.write(
    ...
    'set CYBERDART_AUTHOR_EMAILS=a@x.com,b@y.com, or configure git user.email.\n'
  );
  process.exit(1);
}
...
function main() {
  ...
  const output: Output = {
    ...
    caveats_global: [
      ...
      'Authorship attribution relies on commit email matching. Supply historical aliases via --email flags or CYBERDART_AUTHOR_EMAILS.',
    ],
    version: 1,
  };
