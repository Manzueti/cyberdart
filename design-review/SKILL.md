...
description: |
  ...
  wants to polish the look of a live site. (cyberdart)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**Find the cyberdart designer (optional — enables target mockup generation):**
...
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)"
REPORT_DIR="$HOME/.cyberdart/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)"
...
**Local:** `.cyberdart/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)" && mkdir -p ~/.cyberdart/projects/$SLUG
```
Write to: `~/.cyberdart/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
~/.cyberdart/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
If the cyberdart designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null)" && mkdir -p ~/.cyberdart/projects/$SLUG
```
Write a one-line summary to `~/.cyberdart/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-learnings-log '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
