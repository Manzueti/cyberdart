# Contributing to cyberdart

Thanks for wanting to make cyberdart better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

cyberdart skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/cyberdart/` (your global install). But when you're developing cyberdart itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone https://github.com/garrytan/cyberdart.git && cd cyberdart
...
cyberdart automatically learns from failures. At the end of every skill session, the agent
reflects on what went wrong (CLI errors, wrong approaches, project quirks) and logs
operational learnings to `~/.cyberdart/projects/{slug}/learnings.jsonl`. Future sessions
surface these learnings automatically, so cyberdart gets smarter on your codebase over time.

No setup needed. Learnings are logged automatically. View them with `/learn`.

### The contributor workflow

1. **Use cyberdart normally** — operational learnings are captured automatically
2. **Check your learnings:** `/learn` or `ls ~/.cyberdart/projects/*/learnings.jsonl`
3. **Fork and clone cyberdart** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where cyberdart annoyed you)
   ln -sfn /path/to/your/cyberdart-fork .claude/skills/cyberdart
   cd .claude/skills/cyberdart && bun install && bun run build && ./setup
   ```
   Setup creates per-skill directories with SKILL.md symlinks inside (`qa/SKILL.md -> cyberdart/qa/SKILL.md`)
   and asks your prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using cyberdart** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix cyberdart while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ cyberdart sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on cyberdart inside the cyberdart repo

When you're editing cyberdart skills and want to test them by actually using cyberdart
...
```
cyberdart/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── cyberdart -> ../../         <- symlink back to repo root
│   ├── review/                  <- real directory (short name, default)
│   │   └── SKILL.md -> cyberdart/review/SKILL.md
│   ├── ship/                    <- or cyberdart-review/, cyberdart-ship/ if --prefix
│   │   └── SKILL.md -> cyberdart/ship/SKILL.md
...
under `cyberdart/`. Names depend on your prefix setting (`~/.cyberdart/config.yaml`).
Short names (`/review`, `/ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/cyberdart-review`, `/cyberdart-ship`).
...
When E2E tests run, they produce machine-readable artifacts in `~/.cyberdart-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.cyberdart-dev/` for post-mortem debugging and trend analysis.
...
cyberdart's Writing Style section (injected into every tier-≥2 skill's preamble)
...
cyberdart generates SKILL.md files for 8 hosts from one set of `.tmpl` templates.
...
|--------|---------------------------|
| Output directory | `{skill}/SKILL.md` vs `.agents/skills/cyberdart-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, hooks, version) vs minimal (name + description) |
| Paths | `~/.claude/skills/cyberdart` vs `$CYBERDART_ROOT` |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/cyberdart`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop cyberdart.** Symlink your cyberdart checkout
...
```bash
# In your core project (not the cyberdart repo)
ln -sfn /path/to/your/cyberdart-checkout .claude/skills/cyberdart
```

### Step 2: Run setup to create per-skill symlinks

The `cyberdart` symlink alone isn't enough. Claude Code discovers skills through
individual top-level directories (`qa/SKILL.md`, `ship/SKILL.md`, etc.), not through
the `cyberdart/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/cyberdart && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/qa`) or namespaced (`/cyberdart-qa`).
Your choice is saved to `~/.cyberdart/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/cyberdart/`:

```bash
rm .claude/skills/cyberdart
```

The per-skill directories (`qa/`, `ship/`, etc.) contain SKILL.md symlinks that point
to `cyberdart/...`, so they'll resolve to the global install automatically.

### Switching prefix mode

If you installed cyberdart with one prefix setting and want to switch:

```bash
cd .claude/skills/cyberdart && ./setup --no-prefix   # switch to /qa, /ship
cd .claude/skills/cyberdart && ./setup --prefix       # switch to /cyberdart-qa, /cyberdart-ship
```
...
- Renamed or moved config keys in `~/.cyberdart/config.yaml`
- Need to delete orphaned files from a previous version
- Changed the format of `~/.cyberdart/` state files
...
1. Create `cyberdart-upgrade/migrations/v{VERSION}.sh` where `{VERSION}` matches
   the VERSION file for the release that needs the fix.
2. Make it executable: `chmod +x cyberdart-upgrade/migrations/v{VERSION}.sh`
...
"$SCRIPT_DIR/bin/cyberdart-relink" 2>/dev/null || true
```

### How it runs

During `/cyberdart-upgrade`, after `./setup` completes (Step 4.75), the upgrade
skill scans `cyberdart-upgrade/migrations/` and runs every `v*.sh` script whose
...
verifies that all migration scripts in `cyberdart-upgrade/migrations/` are
