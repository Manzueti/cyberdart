...
description: |
  ...
  "watch production", "verify deploy". (cyberdart)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"canary","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"canary","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"canary","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
eval "$(~/.claude/skills/cyberdart/bin/cyberdart-slug 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .cyberdart/canary-reports
mkdir -p .cyberdart/canary-reports/baselines
mkdir -p .cyberdart/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".cyberdart/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.cyberdart/canary-reports/baseline.json`:
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".cyberdart/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".cyberdart/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots:   .cyberdart/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.cyberdart/canary-reports/{date}-canary.md` and `.cyberdart/canary-reports/{date}-canary.json`.
