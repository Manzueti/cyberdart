...
description: |
  ...
  what it could look like. (cyberdart)
triggers:
...
gbrain:
  ...
  context_queries:
    - id: prior-approved-variants
      kind: filesystem
      glob: "~/.cyberdart/projects/{repo_slug}/designs/*/approved.json"
      sort: mtime_desc
      limit: 5
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-shotgun","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cyberdart/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/cyberdart/bin/cyberdart-timeline-log '{"skill":"design-shotgun","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/cyberdart/bin/cyberdart-question-log '{"skill":"design-shotgun","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
_PREV=$(find ~/.cyberdart/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
**Persistent taste profile (v1 schema at `~/.cyberdart/projects/$SLUG/taste-profile.json`):**
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find ~/.cyberdart/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
variant, call `~/.claude/skills/cyberdart/bin/cyberdart-taste-update approved <variant-path>`. When they
explicitly reject a variant, call `~/.claude/skills/cyberdart/bin/cyberdart-taste-update rejected <variant-path>`.
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output ~/.cyberdart/...`
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `~/.cyberdart/projects/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
