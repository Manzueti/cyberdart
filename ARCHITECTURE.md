...
This document explains **why** cyberdart is built the way it is. For setup and commands, see CLAUDE.md. For contributing, see CONTRIBUTING.md.

## The core idea

cyberdart gives Claude Code a persistent browser and a set of opinionated workflow skills. The browser is the hard part — everything else is Markdown.

The key insight: an AI agent interacting with a browser needs **sub-second latency** and **persistent state**. If every command cold-starts a browser, you're waiting 3-5 seconds per tool call. If the browser dies between commands, you lose cookies, tabs, and login sessions. So cyberdart runs a long-lived Chromium daemon that the CLI talks to over localhost HTTP.

```
Claude Code                     cyberdart
─────────                      ──────
                               ┌──────────────────────┐
...
1. **Compiled binaries.** `bun build --compile` produces a single ~58MB executable. No `node_modules` at runtime, no `npx`, no PATH configuration. The binary just runs. This matters because cyberdart installs into `~/.claude/skills/` where users don't expect to manage a Node.js project.
...
The server writes `.cyberdart/browse.json` (atomic write via tmp + rename, mode 0o600):
...
Cookies are the most sensitive data cyberdart handles. The design:

1. **Keychain access requires user approval.** First cookie import per browser triggers a macOS Keychain dialog. The user must click "Allow" or "Always Allow." cyberdart never silently accesses credentials.
...
3. **Database is read-only.** cyberdart copies the Chromium cookie DB to a temp file (to avoid SQLite lock conflicts with the running browser) and opens it read-only. It never modifies your real browser's cookie database.
...
The Chrome sidebar agent has tools (Bash, Read, Glob, Grep, WebFetch) and reads hostile web pages, so it's the part of cyberdart most exposed to prompt injection. Defense is layered, not single-point.
...
**Env knobs:** `CYBERDART_SECURITY_OFF=1` is a real kill switch (skips ML scan, canary still injects). Model cache at `~/.cyberdart/models/testsavant-small/` (112MB, first run) and `~/.cyberdart/models/deberta-v3-injection/` (721MB, opt-in only). Attack log at `~/.cyberdart/security/attempts.jsonl` (salted sha256 + domain, rotates at 10MB, 5 generations). Per-device salt at `~/.cyberdart/security/device-salt` (0600), cached in-process to survive FS-unwritable environments.

**Visibility.** The sidebar header shows a shield icon (green/amber/red) polled via `/sidebar-chat`. A centered banner appears on canary leak or BLOCK verdict with the exact layer scores. `bin/cyberdart-security-dashboard` aggregates local attempts; `supabase/functions/community-pulse` aggregates opt-in community telemetry across users.
...
```
Browser events → CircularBuffer (in-memory) → Async flush to .cyberdart/*.log
```
...
1. **Update check** — calls `cyberdart-update-check`, reports if an upgrade is available.
2. **Session tracking** — touches `~/.cyberdart/sessions/$PPID` and counts active sessions (files modified in the last 2 hours). When 3+ sessions are running, all skills enter "ELI16 mode" — every question re-grounds the user on context because they're juggling windows.
...
```bash
jq '.tests[] | select(.exit_reason == "timeout") | .last_tool_call' ~/.cyberdart-dev/evals/_partial-e2e.json
```
...
`eval:compare` diffs two eval runs. `eval:summary` aggregates stats across all runs in `~/.cyberdart-dev/evals/`.
