#!/usr/bin/env bun
/**
 * cyberdart-brain-context-load — V1 retrieval surface (Lane C).
 *
 * Called from the cyberdart preamble at every skill start. Reads the active skill's
 ...
 *
 * Usage:
 *   cyberdart-brain-context-load --skill office-hours --repo garrytan-cyberdart
 *   cyberdart-brain-context-load --skill-file ./SKILL.md --repo X --user Y
 *   cyberdart-brain-context-load --window 14d --explain
 *   cyberdart-brain-context-load --quiet
 */
...
import { parseSkillManifest, type GbrainManifest, type GbrainManifestQuery, withErrorContext } from "../lib/cyberdart-memory-helpers";
...
function printUsage(): void {
  console.error(`Usage: cyberdart-brain-context-load [options]
...
function resolveSkillFile(args: CliArgs): string | null {
  ...
  // Look in common cyberdart skill locations
  const candidates = [
    join(HOME, ".claude", "skills", args.skill, "SKILL.md"),
    join(HOME, ".claude", "skills", "cyberdart", args.skill, "SKILL.md"),
...
main().catch((err) => {
  console.error(`cyberdart-brain-context-load fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
