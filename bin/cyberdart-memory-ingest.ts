#!/usr/bin/env bun
/**
 * cyberdart-memory-ingest — V1 memory ingest helper.
 *
 * Walks coding-agent transcript sources + ~/.cyberdart/ curated artifacts and writes
 ...
 *
 * Usage:
 *   cyberdart-memory-ingest --probe                 # count what would ingest, no writes
 *   cyberdart-memory-ingest --incremental [--quiet] # default; mtime fast-path; cheap
 *   cyberdart-memory-ingest --bulk [--all-history]  # first-run; full walk
 *   cyberdart-memory-ingest --bulk --benchmark      # time the bulk pass + report
 *   cyberdart-memory-ingest --include-unattributed  # also ingest sessions with no git remote
 *
 * Sources walked:
 ...
 *   ~/.cyberdart/projects/<slug>/learnings.jsonl       — typed: learning
 *   ~/.cyberdart/projects/<slug>/timeline.jsonl        — typed: timeline
 *   ~/.cyberdart/projects/<slug>/ceo-plans/*.md        — typed: ceo-plan
 *   ~/.cyberdart/projects/<slug>/*-design-*.md         — typed: design-doc
 *   ~/.cyberdart/analytics/eureka.jsonl                — typed: eureka
 *   ~/.cyberdart/builder-profile.jsonl                 — typed: builder-profile-entry
 *
 * State: ~/.cyberdart/.transcript-ingest-state.json (LOCAL per ED1, never synced).
 * Secret scanning: gitleaks via lib/cyberdart-memory-helpers#secretScanFile (D19).
...
import {
  ...
  detectEngineTier,
  withErrorContext,
} from "../lib/cyberdart-memory-helpers";
...
interface CliArgs {
   ...
   * default — the cross-machine boundary (cyberdart-brain-sync, git push)
...
const HOME = homedir();
const CYBERDART_HOME = process.env.CYBERDART_HOME || join(HOME, ".cyberdart");
const STATE_PATH = join(CYBERDART_HOME, ".transcript-ingest-state.json");
...
function printUsage(): void {
  console.error(`Usage: cyberdart-memory-ingest [--probe|--incremental|--bulk] [options]
...
Options:
  ...
  --scan-secrets       Opt-in per-file gitleaks scan during prepare. Off by
                       default; cyberdart-brain-sync already gates the git-push
...
function parseArgs(): CliArgs {
  ...
  let noWrite = process.env.CYBERDART_MEMORY_INGEST_NO_WRITE === "1";
  let scanSecrets = process.env.CYBERDART_MEMORY_INGEST_SCAN_SECRETS === "1";
...
function loadState(): IngestState {
  if (!existsSync(STATE_PATH)) {
    return {
      schema_version: 1,
      last_writer: "cyberdart-memory-ingest",
      sessions: {},
    };
  }
  try {
    ...
    if (parsed.schema_version !== 1) {
      ...
      return { schema_version: 1, last_writer: "cyberdart-memory-ingest", sessions: {} };
    }
    return parsed;
  } catch (err) {
...
function saveState(state: IngestState): void {
  ...
  // in cyberdart-gbrain-sync.ts:saveSyncState.
  try {
...
function* walkCyberDartArtifacts(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  const projectsRoot = join(CYBERDART_HOME, "projects");

  // Eureka log: ~/.cyberdart/analytics/eureka.jsonl
  const eurekaLog = join(CYBERDART_HOME, "analytics", "eureka.jsonl");
  if (existsSync(eurekaLog) && ctx.args.sources.has("eureka")) {
    yield { path: eurekaLog, type: "eureka" };
  }

  // Builder profile: ~/.cyberdart/builder-profile.jsonl
  const builderProfile = join(CYBERDART_HOME, "builder-profile.jsonl");
...
function* walkAllSources(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  ...
  yield* walkCyberDartArtifacts(ctx);
}
...
function buildArtifactPage(path: string, type: MemoryType): PageRecord {
  ...
  // Extract repo slug from path: ~/.cyberdart/projects/<slug>/...
  let slug_repo = "_unattributed";
  const m = path.match(/\/\.cyberdart\/projects\/([^/]+)\//);
  if (m) slug_repo = m[1];
 ...
 *
 *   The actual cross-machine exfiltration boundary is `cyberdart-brain-sync`,
 *   which runs a regex-based secret scanner on the staged diff before
 *   `git commit` (see bin/cyberdart-brain-sync:78-110: AWS keys, GitHub
...
function preparePages(
  ...
  for (const { path, type } of walkAllSources(ctx)) {
    ...
    // because cyberdart-brain-sync already gates the cross-machine boundary
...
/**
 * Make a per-run staging directory at ~/.cyberdart/.staging-ingest-<pid>-<ts>/
...
function makeStagingDir(): string {
  const dir = join(CYBERDART_HOME, `.staging-ingest-${process.pid}-${Date.now()}`);
...
async function ingestPass(args: CliArgs): Promise<BulkResult> {
  ...
  if (args.noWrite) {
    ...
    state.last_writer = "cyberdart-memory-ingest";
    saveState(state);
    return {
      written,
...
main().catch((err) => {
  console.error(`cyberdart-memory-ingest fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
