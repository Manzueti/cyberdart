# cyberdart-gbrain-lib.sh — shared helpers for setup-gbrain bin scripts.
#
# This file is NOT executable; source it:
#
#   . "$(dirname "$0")/cyberdart-gbrain-lib.sh"
#
# Provides:
...
# _cyberdart_gbrain_validate_varname <name> — returns 0 if usable, 2 otherwise.
_cyberdart_gbrain_validate_varname() {
  local name="$1"
  case "$name" in
...
read_secret_to_env() {
  ...
  if ! _cyberdart_gbrain_validate_varname "$varname"; then
