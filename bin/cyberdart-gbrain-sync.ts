#!/usr/bin/env bun
/**
 * cyberdart-gbrain-sync — V1 unified sync verb.
 *
 ...
 *   2. Transcripts + curated memory → cyberdart-memory-ingest (typed put_page)
 *   3. Curated artifacts to git    → cyberdart-brain-sync (existing pipeline)
 *
 * Modes:
 ...
 *   - Lock file at ~/.cyberdart/.sync-gbrain.lock (PID + start ts).
 ...
 * than building a cyberdart-side daemon.
 */
...
import { detectEngineTier, withErrorContext, canonicalizeRemote } from "../lib/cyberdart-memory-helpers";
...
const STATE_PATH = join(CYBERDART_HOME, ".gbrain-sync-state.json");
const LOCK_PATH = join(CYBERDART_HOME, ".sync-gbrain.lock");
const STALE_LOCK_MS = 5 * 60 * 1000;

// ── CLI ────────────────────────────────────────────────────────────────────

function printUsage(): void {
  console.error(`Usage: cyberdart-gbrain-sync [--incremental|--full|--dry-run] [options]
...
Options:
  ...
  --no-memory          Skip the cyberdart-memory-ingest stage (transcripts + artifacts).
  --no-brain-sync      Skip the cyberdart-brain-sync git pipeline stage.
 ...
 *
 * Pattern: `cyberdart-code-<slug>-<pathhash8>` where slug comes from origin
...
function deriveCodeSourceId(repoPath: string): string {
  ...
  if (remote) {
    ...
    return constrainSourceId("cyberdart-code", `${slugSource}-${pathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("cyberdart-code", `${base}-${pathHash}`);
}

/**
 * Pre-pathhash source id, kept for orphan detection only.
 *
 * Earlier /sync-gbrain versions registered `cyberdart-code-<slug>` (no pathhash
...
function deriveLegacyCodeSourceId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("cyberdart-code", slugSource);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("cyberdart-code", base);
}
...
function acquireLock(): boolean {
  mkdirSync(CYBERDART_HOME, { recursive: true });
...
async function runCodeImport(args: CliArgs): Promise<StageResult> {
  ...
  // Earlier /sync-gbrain versions registered `cyberdart-code-<slug>` (no path
...
function runMemoryIngest(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "memory", ran: false, ok: true, duration_ms: 0, summary: "would: cyberdart-memory-ingest --probe" };
  }

  const ingestPath = join(import.meta.dir, "cyberdart-memory-ingest.ts");
...
function runBrainSyncPush(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "would: cyberdart-brain-sync --discover-new --once" };
  }

  const brainSyncPath = join(import.meta.dir, "cyberdart-brain-sync");
  if (!existsSync(brainSyncPath)) {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "skipped (cyberdart-brain-sync not installed)" };
  }
  ...
  return {
    ...
    ok: result.status === 0,
    duration_ms: Date.now() - t0,
    summary: result.status === 0 ? "curated artifacts pushed" : `cyberdart-brain-sync exited ${result.status}`,
  };
}
...
function loadSyncState(): SyncState {
  if (!existsSync(STATE_PATH)) {
    return { schema_version: 1, last_writer: "cyberdart-gbrain-sync" };
  }
  try {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!args.noCode) {
      stages.push(await withErrorContext("sync:code", () => runCodeImport(args), "cyberdart-gbrain-sync"));
    }
    if (!args.noMemory) {
      stages.push(await withErrorContext("sync:memory", () => runMemoryIngest(args), "cyberdart-gbrain-sync"));
    }
    if (!args.noBrainSync) {
      stages.push(await withErrorContext("sync:brain-sync", () => runBrainSyncPush(args), "cyberdart-gbrain-sync"));
    }
    ...
    if (!args.quiet || args.mode === "dry-run") {
      console.log(`\ncyberdart-gbrain-sync (${args.mode}):`);
...
main().catch((err) => {
  console.error(`cyberdart-gbrain-sync fatal: ${err instanceof Error ? err.message : String(err)}`);
  releaseLock();
  process.exit(1);
});
