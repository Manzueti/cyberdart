# Remote Browser Access — How to Pair With a Cyberdart Browser

A Cyberdart Browser server can be shared with any AI agent that can make HTTP requests.
...
─────────────                         ────────────
Cyberdart Browser Server                 Any AI agent
...
SSE endpoints (`/activity/stream`, `/inspector/events`) accept either a Bearer token or the HttpOnly `cyberdart_sse` cookie (minted via `POST /sse-session`, 30-minute TTL, stream-scope only — cannot be used against `/command`). As of v1.6.0.0 the `?token=<ROOT>` query-string auth is no longer accepted.
...
- **Path traversal guarded** on `/welcome` — `CYBERDART_SLUG` must match `^[a-z0-9_-]+$` or falls back to the built-in template.
- **SSRF guards** on `goto`, `download`, and scrape paths — validates URL target against a localhost/private-range blocklist.
- **Tunnel surface denial logging.** Every rejection on the tunnel listener (`path_not_on_tunnel`, `root_token_on_tunnel`, `missing_scoped_token`, `disallowed_command:*`) is appended to `~/.cyberdart/security/attempts.jsonl` with timestamp, source IP, path, method. Rate-capped at 60 writes/min.
...
```bash
$B pair-agent --local openclaw    # writes to ~/.openclaw/skills/cyberdart/browse-remote.json
$B pair-agent --local codex       # writes to ~/.codex/skills/cyberdart/browse-remote.json
$B pair-agent --local cursor      # writes to ~/.cursor/skills/cyberdart/browse-remote.json
```
...
3. Save it: `echo 'NGROK_AUTHTOKEN=your_token' > ~/.cyberdart/ngrok.env`
4. Optionally claim a stable domain: `echo 'NGROK_DOMAIN=your-name.ngrok-free.dev' >> ~/.cyberdart/ngrok.env`
