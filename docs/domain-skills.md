...
This is cyberdart's borrow from [browser-use/browser-harness](https://github.com/browser-use/browser-harness).
cyberdart copies the per-site-notes pattern, NOT the self-modifying-runtime
...
on every cyberdart project I work on"), explicitly run
...
- **Per-project**: `~/.cyberdart/projects/<slug>/learnings.jsonl` — same JSONL
  file the `/learn` skill uses. Domain skills are `type:"domain"` rows.
- **Global**: `~/.cyberdart/global-domain-skills.jsonl` — only `state:"global"`
  rows.
...
following events are written to `~/.cyberdart/analytics/browse-telemetry.jsonl`:
...
`cyberdart-config set telemetry off` or `CYBERDART_TELEMETRY_OFF=1`.
