...
This is the preserved design artifact for `cyberdart compact`. Everything above the first `---` divider below gets extracted verbatim to `docs/designs/GCOMPACTION.md` on plan approval. Everything after that divider is archived research (office hours + competitive deep-dive + eng-review notes + codex review + research findings) that informed the design.
...
**Decision.** Shelve `cyberdart compact` entirely. Track Anthropic issue #36843 for the arrival of `updatedBuiltinToolOutput` (or equivalent). When that API ships, this design doc + the 15 locked decisions below + the research archive at the bottom become the unblocking artifacts for a fresh implementation sprint.
...
2. **13-rule launch library.** v1 ships tests (jest/vitest/pytest/cargo-test/go-test/rspec) + git (diff/log/status) + install (npm/pnpm/pip/cargo). Build/lint/log families defer to v1.1, driven by `cyberdart compact discover` telemetry from real users.
...
8. **Pre-compiled rule bundle.** `cyberdart compact install` and `cyberdart compact reload` produce `~/.cyberdart/compact/rules.bundle.json` (deep-merged, regex-compiled metadata cached). Hook reads that single file instead of parsing N source files.
...
`cyberdart compact` was designed as a `PostToolUse` hook that reduces tool-output noise before it reaches an AI coding agent's context window. Deterministic JSON rules would shrink noisy test runners, build logs, git diffs, and package installs. A conditional Claude Haiku verifier would act as a safety net when over-compaction risk was high.
...
**Original positioning (now moot):** *"RTK is fast. cyberdart compact is fast AND safe, and it covers every tool in your toolbox, not just Bash."*
...
- Replacing the agent's own judgement about when to re-run a command with `CYBERDART_RAW=1`.
...
**Existing field (what cyberdart compact joins and differentiates from):**
...
**License compatibility:** Every referenced project is permissive-licensed (MIT or Apache-2.0) and compatible with cyberdart's MIT license. No AGPL, GPL, or other copyleft dependencies. See the "License & attribution" section below for the clean-room policy.
...
│  g. On failure exit code: tee raw to ~/.cyberdart/compact/tee/...  │
...
Three-tier hierarchy (highest precedence wins), same pattern as tokenjuice and cyberdart's existing host-config-export model:

1. Built-in rules: `compact/rules/` shipped with cyberdart
2. User rules: `~/.config/cyberdart/compact-rules/`
3. Project rules: `.cyberdart/compact-rules/`
...
```json
{
  "reduced": "compacted output with [cyberdart-compact: N → M lines, rule: X] header",
  "meta": {
    "rule": "tests/jest",
    "linesBefore": 247,
    "linesAfter": 18,
...
| `userOptIn` | on (env-gated) | `CYBERDART_COMPACT_VERIFY=1` forces verifier for that call |
...
```
[cyberdart-compact: 247 → 18 lines, rule: tests/jest]
[cyberdart-verify: 2 additional lines preserved by Haiku]
...
```json
{
  "verifier": {
    ...
    "triggers": {
      ...
      "userOptIn":           { "enabled": true, "envVar": "CYBERDART_COMPACT_VERIFY" }
    },
...
On any command with exit code ≠ 0, the full unfiltered output is written to `~/.cyberdart/compact/tee/{timestamp}_{cmd-slug}.log`. The compacted output includes a tee-file pointer:

```
[cyberdart-compact: 247 → 18 lines, rule: tests/jest, tee: ~/.cyberdart/compact/tee/20260416-143022_bun-test.log]
```
...
User config (`~/.config/cyberdart/compact.toml`):
...
bundle_path = "~/.cyberdart/compact/rules.bundle.json"
...
output_dir = "~/.cyberdart/compact/benchmark"
...
|---------|---------|--------|
| `cyberdart compact install <host>` | Register PostToolUse hook in host config; builds `rules.bundle.json` | new |
| `cyberdart compact uninstall <host>` | Idempotent removal | new |
| `cyberdart compact reload` | Rebuild `rules.bundle.json` after editing user/project rules | new |
| `cyberdart compact doctor` | Detect drift / broken hook config, offer to repair | tokenjuice |
| `cyberdart compact gain` | Show token/dollar savings over time (per-rule breakdown) | RTK |
| `cyberdart compact discover` | Find commands with no matching rule, ranked by noise volume | RTK |
| `cyberdart compact verify <rule-id>` | Dry-run verifier on a fixture | new |
| `cyberdart compact list-rules` | Show effective rule set after deep-merge (built-in + user + project) | new |
| `cyberdart compact test <rule-id> <fixture>` | Apply a rule to a fixture and show the diff | new |
| `cyberdart compact benchmark` | Run B-series testbench against local transcript corpus (see Benchmark section) | new |

Escape hatch: `CYBERDART_RAW=1` env var bypasses the hook entirely for the duration of a command (same pattern as tokenjuice's `--raw` flag). Hook also auto-reloads the bundle if any source rule file's mtime is newer than the bundle file.
...
| ID | Scenario | Hosts |
|----|----------|-------|
| CH1 | Install hook via `cyberdart compact install <host>` | Claude Code, Codex, OpenClaw |
...
| CH11 | `CYBERDART_RAW=1` env var bypasses hook | All |
| CH12 | Rule ID override works (project rule replaces built-in) | All |
| CH13 | `cyberdart compact doctor` detects drift on each host | All |
...
| V5 | `CYBERDART_COMPACT_VERIFY=1` env var set | Verifier fires once for that call |
...
│              ~/.cyberdart/compact/benchmark/latest/              │
│              "On YOUR 30 days of Claude Code data, cyberdart    │
...
- Output files live under `~/.cyberdart/compact/benchmark/` with mode `0600`.
...
- **E1:** simulated 30-min coding session on a medium TypeScript project. Measure total tokens with/without cyberdart compact enabled. Target: ≥15% reduction.
...
1. Run the tool against a real project (cyberdart itself for TS; popular OSS for Rust/Go/Python).
...
- cyberdart's own test output (eat our own dog food)
- Real failure archives from `~/.cyberdart/compact/tee/` (once volunteers contribute)
...
| RTK | `cyberdart compact tee` for failure-mode raw save | Better than the original `onFailure.preserveFull` design |
| RTK | `cyberdart compact gain` + `cyberdart compact discover` | Trust + continuous improvement |
...
| tokenjuice | `cyberdart compact doctor` | Hooks drift; self-repair matters |
...
2. **v0.1 (1 day):** Claude Code hook integration + `cyberdart compact install` + mtime-based auto-reload. Ship as opt-in; off by default. Ask 10 cyberdart power users to try it; collect feedback.
3. **v0.5 (1 day):** B-series benchmark testbench (`compact/benchmark/`). Ship `cyberdart compact benchmark` so users can measure on their own data. Collect anonymous-from-the-start (nothing uploaded) reduction numbers from dogfooders.
...
5. **v1.1 (+1 day):** Codex + OpenClaw hook integration. Cross-host E2E suite green. Build/lint/log rule families land with `cyberdart compact discover`-derived priorities.
...
| Haiku cost creep (triggers fire more than expected) | Medium | E3 eval + B-series fire-rate metric; cost visible in `cyberdart compact gain`; per-session rate cap in v1.1 if rate >10%. |
...
| Bundle staleness silently breaks user edits | Low | mtime-check on every hook invocation auto-rebuilds; `cyberdart compact reload` is a backup not a requirement. |
...
3. Should the verifier model be pinned, or version-tracked like cyberdart's other AI calls? (Inclined to pin `claude-haiku-4-5-20251001` and bump explicitly in CHANGELOG.)
...
5. Should `cyberdart compact discover` propose auto-generated rules via Haiku? (Deferred to v2; skill-creep risk.)
...
5. **Ship a rule-only v0** (no Haiku verifier, no benchmark). Measure real token savings with current cyberdart evals + early B-series prototype. If <10% on local corpus, the whole premise is weaker than claimed — iterate the rules before adding the verifier on top.

## License & attribution

cyberdart ships under MIT. To keep the license clean for downstream users, this project follows a strict clean-room policy for everything borrowed from the competitive landscape:
