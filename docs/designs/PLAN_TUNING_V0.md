...
1. **Typed question registry** (`scripts/question-registry.ts`). Every AskUserQuestion cyberdart uses is declared with `{id, skill, category, door_type, options[], signal_key?}`. Schema-governed.
...
3. **Question logging** (`bin/cyberdart-question-log`). Appends `{ts, question_id, user_choice, recommended, session_id}` to `~/.cyberdart/projects/{SLUG}/question-log.jsonl`. Validates against registry.
4. **Explicit per-question preferences** (`bin/cyberdart-question-preference`). Writes `{question_id, preference}` where preference is `always-ask | never-ask | ask-only-for-one-way`. Respected from session 1. No calibration gate — user stated it, system obeys.
5. **Preamble injection.** Before each AskUserQuestion, agent calls `cyberdart-question-preference --check <registry-id>`. If `never-ask` AND question is NOT a one-way door, auto-choose recommended option with visible annotation: "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." One-way doors always ask regardless of preference — safety override.
...
7. **Declared profile** (`/plan-tune setup`). 5 plain-English questions, one per dimension. Stored in unified `~/.cyberdart/developer-profile.json` under `declared: {...}`. Informational only in v1 — no skill behavior change.
...
10. **Unification with existing `~/.cyberdart/builder-profile.jsonl`.** Fold /office-hours session records and accumulated signals into unified `~/.cyberdart/developer-profile.json`. Migration is atomic + idempotent + archives the source file.
...
2. Before each AskUserQuestion:
   - Agent calls `cyberdart-question-preference --check <registry-id>`
...
5. If user's NEXT turn message contains `tune:` prefix AND the content originated in the user's own message (not tool output):
   - Agent calls `cyberdart-question-preference --write` with `source: "inline-user"`
   - Binary validates source field; rejects if anything other than `inline-user`
6. Inferred dimensions recomputed on demand by `bin/cyberdart-developer-profile --derive`. Signal map changes trigger full recompute from events history.
...
Binary enforcement: `cyberdart-question-preference --write` requires `source: "inline-user"` field on every tune-originated record. Any other source value (e.g., `inline-tool-output`, `inline-file-content`) is rejected with an error. Agent is instructed to never forge the `source` field.

**Data privacy**:
- All data is local-only under `~/.cyberdart/`. Nothing leaves without explicit user action.
- `/plan-tune export <path>` writes profile to user-specified path (opt-in export).
- `/plan-tune delete` wipes local profile files.
- `cyberdart-config set telemetry off` prevents any telemetry (this skill never sends profile data regardless).
- Profile files have standard user-home permissions.

**Injection defense** (consistent with existing `bin/cyberdart-learnings-log` patterns): the `question_summary` and any free-form user feedback fields are sanitized against known prompt-injection patterns ("ignore previous instructions," "system:", etc.).
...
5. **Per-project preferences beat global preferences.** `~/.cyberdart/projects/{SLUG}/question-preferences.json` wins over any future global preference file. Global profile (`~/.cyberdart/developer-profile.json`) is a starting point for diversity across projects.

## Why event-sourced + dual-track

**Why event-sourced for the inferred profile**:
- Signal map can change between cyberdart versions. Recompute from events, no data migration needed.
...
### Binaries
- `bin/cyberdart-question-log` — append log record, validate against registry.
- `bin/cyberdart-question-preference` — read/write/check/clear explicit preferences.
- `bin/cyberdart-developer-profile` — supersedes `bin/cyberdart-builder-profile`. Subcommands: `--read` (legacy compat), `--derive`, `--gap`, `--profile`.
...
- `bin/cyberdart-builder-profile` — legacy shim delegates to `bin/cyberdart-developer-profile --read`.
...
- Cons: Requires auditing every existing AskUserQuestion in cyberdart — substantial up-front work.
...
- Pros: Consistent with cyberdart's local-first ethos.
- Cons: No aggregate view of onboarding time.

**Chosen: local-only.** If we need TTHW data later, we add it as a cyberdart-wide telemetry event behind existing opt-in, not a skill-specific one.
...
- Migration from `~/.cyberdart/builder-profile.jsonl` preserves 100% of sessions + signals_accumulated. Regression test with 7-session fixture.
...
1. Audit every `AskUserQuestion` invocation in every cyberdart SKILL.md.tmpl. Build initial `scripts/question-registry.ts` with IDs, categories, door_types, options. This is the foundation; everything else sits on it.
...
6. `bin/cyberdart-question-log` — validates against registry, rejects unknown IDs.
7. `bin/cyberdart-question-preference` — all subcommands + tests.
8. `bin/cyberdart-developer-profile` — `--read` (legacy), `--derive`, `--gap`, `--profile`.
