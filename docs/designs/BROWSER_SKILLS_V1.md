...
| **3** | new (`browser-skills-resolver`) | Resolver injection at session start (per-host browser-skill discovery). Mirrors domain-skill injection. `cyberdart-config browser_skillify_prompts` knob. |
...
5. **Three-tier lookup: bundled → global → project.** Bundled skills ship
   read-only with the cyberdart install (`<cyberdart-install>/browser-skills/<name>/`).
   Global at `~/.cyberdart/browser-skills/<name>/`. Per-project at
   `<project>/.cyberdart/browser-skills/<name>/`. Lookup walks tiers in priority
...
12. **Token/port discovery: scoped-token env-only for spawned skills;
    ...
    `$B skill run`, the SDK reads `CYBERDART_PORT` + `CYBERDART_SKILL_TOKEN` from
    env. For standalone `bun run script.ts`, the SDK falls back to
    `<project>/.cyberdart/browse.json` (the actual state-file path per
    `config.ts:50`).
13. **CHANGELOG honesty.** Phase 1 lead: humans can hand-write deterministic
    browser scripts that cyberdart runs. Phase 1 explicitly notes that agent
...
| **Process-side env access** | SKILL.md frontmatter `trusted: true` passes `process.env` minus `CYBERDART_TOKEN`. `trusted: false` (default) drops everything except a minimal allowlist (LANG, LC_ALL, TERM, TZ, locked PATH) and explicitly strips secret-pattern keys (TOKEN/KEY/SECRET/PASSWORD, AWS_*, AZURE_*, GCP_*, ANTHROPIC_*, OPENAI_*, GITHUB_*, etc.). | Untrusted (must opt in). |

`CYBERDART_PORT` and `CYBERDART_SKILL_TOKEN` are always injected last so a parent
...
| 5 | Atomic skill packaging matters more than the index question; symlink defenses | **Closed for Phase 1**: bundled skills ship as part of the cyberdart install (no live writes; atomic by virtue of being read-only files in the install dir). Phase 2's `writeBrowserSkill` will write to a temp dir then rename, and use `realpath`/`lstat` discipline (existing `browse/src/path-security.ts`). |
...
| 7 | Bun runtime regression: skill scripts as standalone Bun reintroduce a Bun runtime requirement | **Open issue for Phase 2 distribution.** Phase 1 sidesteps this because the bundled reference skill ships inside the cyberdart install (which already builds with Bun). Phase 2 needs to decide between (a) shipping a Bun binary with each generated skill, (b) compiling skills to self-contained executables, or (c) using Node.js with `cli.ts`'s HTTP pattern. |
...
- **Bun runtime distribution.** Codex finding #7 stays open. Phase 2a assumes
  Bun is on PATH (cyberdart already requires it via `setup:6-15`). Documented
...
`cyberdart-config browser_skillify_prompts` (default off): when on, end-of-task
