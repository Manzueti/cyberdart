...
cyberdart already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has cyberdart's artifact architecture.

## The Insight

cyberdart already writes structured artifacts to `~/.cyberdart/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.cyberdart/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.cyberdart/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle cyberdart artifacts)
