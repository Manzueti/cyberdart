...
T+1-3s    Extension loads, sidebar opens
            ...
            └── tryAutoConnect() polls until window.cyberdartServerPort + token are set
...
2. Extension calls `new WebSocket(url, ['cyberdart-pty.<token>'])`.
3. Agent reads `Sec-WebSocket-Protocol`, strips `cyberdart-pty.`, validates
...
A `Set-Cookie: cyberdart_pty=...` header is also returned for non-browser
...
| `cyberdart-pty.<...>` (Sec-WebSocket-Protocol) | Browser memory only; agent `validTokens` Set | `/ws` upgrade auth | 30 min, auto-revoked on WS close |
...
| Button | Behavior |
|--------|----------|
| 🧹 Cleanup | `window.cyberdartInjectToTerminal(prompt)` — pipes a "remove ads/banners" instruction into the live PTY. claude in the terminal sees it and acts. |
...
The Inspector's "Send to Code" button uses the same `cyberdartInjectToTerminal`
