# Design: cyberdart Visual Design Generation (`design` binary)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.cyberdart/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/cyberdart-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/cyberdart-mockup-{timestamp}.png`
...
```
Note: Codex runtime setup (`setup` script) must also export `CYBERDART_DESIGN` env var, similar to how `CYBERDART_BROWSE` is set.
...
[ -z "$D" ] && D=~/.claude/skills/cyberdart/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
| `scripts/resolvers/preamble.ts` | Add `CYBERDART_DESIGN` env var export for Codex host |
...
1. Read `~/.cyberdart/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.cyberdart/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/cyberdart/` install path
...
- Auth module (read ~/.cyberdart/openai.json, fallback to env var, guided setup flow)
...
- Add CYBERDART_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
