...
**Estimated effort:** human ~3 days / CC+cyberdart ~2 hr
...
exfiltration boundary is `cyberdart-brain-sync` (bin/cyberdart-brain-sync:78-110,
...
for 5031). That's a gbrain-side perf issue, not cyberdart architecture.
...
1. **gbrain import perf on large dirs** — investigate why 5031 files
   ...
   FTS index updates without batching. Lives in gbrain, not cyberdart.
...
Two compounding bugs in `bin/cyberdart-memory-ingest.ts`:

1. **Subprocess-per-file architecture.** The ingest loop at line 911 walks
   1,841 files in `~/.cyberdart/projects/` and spawns two subprocesses per file:
   - `gitleaks detect --no-git --source <path>` — 46ms cold start (`lib/cyberdart-memory-helpers.ts:157`)
   - `gbrain put <slug>` — 329ms cold start (`bin/cyberdart-memory-ingest.ts:823`)
   - Per-file floor: 375ms × 1841 = 690s (11.5 min) of pure subprocess startup
     before any actual work happens.

2. **Kill-no-save timeout.** Orchestrator at `bin/cyberdart-gbrain-sync.ts:442`
   enforces a 35-min timeout. When it fires, `spawnSync` returns
   `result.status === null`, the child gets SIGTERM, and the in-memory
   ingest state never flushes to `~/.cyberdart/.transcript-ingest-state.json`.
...
| Metric | Value | Source |
|---|---|---|
| Files in walkAllSources | 1,841 | `find ~/.cyberdart/projects -type f \( -name "*.md" -o -name "*.jsonl" \)` |
...
Take everything in `ingestPass` (lines 899-988 of `bin/cyberdart-memory-ingest.ts`)
...
- Created at `~/.cyberdart/.staging-ingest-${pid}-${ts}/`
...
redaction policy in `lib/cyberdart-memory-helpers.ts`).
...
### Step 6: SIGTERM handler in `cyberdart-memory-ingest.ts`
...
In `bin/cyberdart-gbrain-sync.ts:444`:
...
`~/.cyberdart/.ingest-skip-list.json` so the next prepare stage skips that file
entirely. Avoids re-staging a file that will always fail. User can review
the skip list with a new `cyberdart-memory-ingest --skip-list` flag.
...
3. **Benchmark gate (periodic, paid):**
   ...
   - Test fixture: copy of `~/.cyberdart/projects/` snapshot for repeatable timing.

## Rollback strategy

- New `--legacy-ingest` flag on `cyberdart-memory-ingest` keeps the old
  per-file path callable for one release cycle.
- If batch path regresses on a real corpus, set
  `cyberdart-config set memory_ingest_path legacy` to revert without redeploy.
...
4. **Cross-worktree concurrency.** `~/.cyberdart/.staging-ingest-${pid}-${ts}/`
   is pid-namespaced so two concurrent /sync-gbrain runs don't collide.
   But the orchestrator already holds a lock at `~/.cyberdart/.sync-gbrain.lock`
...
6. **Should we deprecate `gbrain put` for memory entirely?** The legacy
   ...
   ingestion? Probably yes (for `~/.cyberdart/.transcript-ingest-state.json`
   updates triggered outside the orchestrator), but worth confirming.

## What this isn't

- Not a gbrain CLI change. All work is in cyberdart.
