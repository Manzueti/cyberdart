...
**Date:** 2026-03-28
**CEO Plan:** ~/.cyberdart/projects/garrytan-cyberdart/ceo-plans/2026-03-28-sidebar-prompt-injection-defense.md

## The Problem

The cyberdart Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any cyberdart component can call these
...
```json
// ~/.cyberdart/security/attempts.jsonl
{
...
```
AskUserQuestion:
  "cyberdart just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all cyberdart users. Can we
...
2. Cache to ~/.cyberdart/models/browsesafe-bench/ (not re-downloaded in CI)
...
- cyberdart becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But cyberdart already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.cyberdart/models/
