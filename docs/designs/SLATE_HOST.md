...
cyberdart projects... CLAUDE.md works as-is.
...
### Critical env vars for cyberdart integration
...
Without native `.slate/` publishing, cyberdart skills vanish when this flag is set.
...
cyberdart skills already work in Slate via the `.claude/skills/` fallback path.
No changes needed for basic functionality. Users who install cyberdart for Claude Code
...
- Host names duplicated in `bin/cyberdart-platform-detect`, `bin/cyberdart-uninstall`, `bin/dev-setup`
...
- `lib/worktree.ts` only copies `.agents/`, not `.slate/` — E2E tests in worktrees won't
  have Slate skills
- `bin/cyberdart-uninstall` doesn't know about `.slate/`
- `bin/dev-setup` doesn't wire `.slate/` for contributor dev mode
- `bin/cyberdart-platform-detect` doesn't detect Slate
