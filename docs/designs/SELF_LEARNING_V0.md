# Design: Cyberdart Self-Learning Infrastructure
...
Cyberdart runs 30+ skills across sessions but learns nothing between them. A /review
...
Structured, typed, confidence-scored learnings that every cyberdart skill can read and
write. The goal: after 20 sessions on the same codebase, cyberdart knows every
...
YC founders building with AI. The people who run cyberdart on real codebases 20+ times
...
| **Cyberdart** | **Per-project JSONL** | **Cross-session, cross-skill** | **Typed, scored, decaying** |

---

## State Systems

cyberdart has four distinct persistence layers. They share storage patterns
(JSONL in `~/.cyberdart/projects/$SLUG/`) but serve different purposes:
...
### Release 1: "Cyberdart Learns" (v0.13-0.14) — SHIPPED

**Headline:** Every session makes the next one smarter.

What shipped:
- Learnings persistence at `~/.cyberdart/projects/{slug}/learnings.jsonl`
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via cyberdart-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read cyberdart-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via cyberdart-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.cyberdart/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), cyberdart suggests what you probably want next.
...
- Timeline binaries: `bin/cyberdart-timeline-log` and `bin/cyberdart-timeline-read`.
...
**Headline:** Cyberdart respects your time without compromising your safety.
...
The self-learning roadmap was inspired by ideas from the [Compound Engineering](https://github.com/nicobailon/compound-engineering) project by Nico Bailon. Their exploration of learnings persistence, parallel review agents, and autonomous pipelines catalyzed the design of Cyberdart's approach. We adapted every concept to fit Cyberdart's template system, voice, and architecture rather than porting directly.
