...
cyberdart writes a lot of useful state to `~/.cyberdart/` — learnings, retros, CEO
...
- A clean off-ramp (`cyberdart-brain-uninstall`) that never touches your data.
...
The exact allowlist lives in `~/.cyberdart/.brain-allowlist`. The CLI manages
...
1. Turns `~/.cyberdart/` into a git repo.
2. Asks for a remote URL (default: `gh repo create --private
   cyberdart-brain-$USER`). Any git remote works — GitHub, GitLab, Gitea,
   self-hosted.
3. Pushes an initial commit with just the config.
4. Writes `~/.cyberdart-brain-remote.txt` (URL-only, no secrets —
   safe to copy to another machine).
5. Wires the cyberdart-brain repo into your local gbrain as a federated
   ...
   lives in `bin/cyberdart-gbrain-source-wireup`. The old
   `cyberdart-brain-reader add --ingest-url ...` HTTP path was removed in
...
- **Decline**: keep everything local. You can turn sync on later with
  `cyberdart-config set gbrain_sync_mode full`.

Your answer is persisted. You won't be asked again.

## Cross-machine workflow

On machine A: run `cyberdart-brain-init` once. That's it — every skill
invocation now drains the sync queue at its start and end boundaries
(~200–800 ms network pause per skill).

On machine B:

1. Copy `~/.cyberdart-brain-remote.txt` from machine A to machine B
   (password manager, dotfile repo, USB stick — your call).
2. Run any cyberdart skill. The preamble sees the URL file and prints:
   ```
   BRAIN_SYNC: brain repo detected: <url>
   BRAIN_SYNC: run 'cyberdart-brain-restore' to pull your cross-machine memory
   ```
3. Run `cyberdart-brain-restore`. That clones the repo, rehydrates your
...
4. Re-enter consumer tokens (they're machine-local and NOT synced —
   `cyberdart-config set gbrain_token <your-token>`).
...
```bash
cyberdart-config set gbrain_sync_mode full
cyberdart-config set gbrain_sync_mode off
```
...
2. If the match is a false positive on content you explicitly want to
   sync, run `cyberdart-brain-sync --skip-file <path>` to permanently
   exclude that path.
3. Otherwise, edit the file to remove the secret and re-run any skill.

There's a defense-in-depth hook at `~/.cyberdart/.git/hooks/pre-commit` that
...
(cached via `~/.cyberdart/.brain-last-pull`). You don't need to think about
this — it happens automatically at the first skill invocation each day.

## Uninstall

```bash
cyberdart-brain-uninstall
```

This:

- Removes `~/.cyberdart/.git/` and all `.brain-*` config files.
- Clears `gbrain_sync_mode` in `cyberdart-config`.
...
Re-init anytime with `cyberdart-brain-init`.

## Troubleshooting

See [gbrain-sync-errors.md](gbrain-sync-errors.md) for an index of every
error message cyberdart-brain may print, with problem / cause / fix for each.
