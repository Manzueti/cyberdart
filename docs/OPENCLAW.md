# cyberdart x OpenClaw Integration

cyberdart integrates with OpenClaw as a methodology source, not a ported codebase.
OpenClaw's ACP runtime spawns Claude Code sessions natively. cyberdart provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │   investigate, retro                   │
       │                                        ├── Generates cyberdart-lite
       ├── sessions_spawn(runtime: "acp")       │   (planning discipline)
       │       │                                │
       │       └── Claude Code                  ├── Generates cyberdart-full
       │           └── cyberdart installed at      │   (complete pipeline)
       │               ~/.claude/skills/cyberdart  │
...
OpenClaw decides at spawn time which tier of cyberdart support to use:

| Tier | When | Prompt prefix |
|------|------|---------------|
| **Simple** | One-file edits, typos, config changes | No cyberdart context injected |
| **Medium** | Multi-file features, refactors | cyberdart-lite CLAUDE.md appended |
| **Heavy** | Specific cyberdart skill needed | "Load cyberdart. Run /X" |
| **Full** | Complete features, objectives, projects | cyberdart-full pipeline appended |
| **Plan** | "Help me plan a Claude Code project" | cyberdart-plan pipeline appended |
...
The complete ready-to-paste section lives in `openclaw/agents-cyberdart-section.md`.
Copy it into your OpenClaw AGENTS.md.

Key behavioral rules (these go ABOVE the dispatch tiers):

1. **Always spawn, never redirect.** When the user asks to use ANY cyberdart skill,
...
cyberdart-lite/full as a new section. Do not replace the repo's existing instructions.

## What cyberdart generates for OpenClaw

All artifacts live in the `openclaw/` directory and are generated by
`bun run gen:skill-docs --host openclaw`:

### cyberdart-lite (Medium tier)
`openclaw/cyberdart-lite-CLAUDE.md` — ~15 lines of planning discipline:
...
### cyberdart-full (Full tier)
`openclaw/cyberdart-full-CLAUDE.md` — chains existing cyberdart skills:
...
### cyberdart-plan (Plan tier)
`openclaw/cyberdart-plan-CLAUDE.md` — full review gauntlet, no implementation:
...
- `cyberdart-openclaw-office-hours` — Product interrogation (6 forcing questions)
- `cyberdart-openclaw-ceo-review` — Strategic challenge (10-section review, 4 modes)
- `cyberdart-openclaw-investigate` — Operational debugging (4-phase methodology)
- `cyberdart-openclaw-retro` — Operational retrospective (weekly review)

Source lives in `openclaw/skills/` in the cyberdart repo. These are hand-crafted
adaptations of the cyberdart methodology for OpenClaw's conversational context.
No cyberdart infrastructure (no browse, no telemetry, no preamble).

## Spawned session detection

When Claude Code runs inside a session spawned by OpenClaw, the `OPENCLAW_SESSION`
environment variable should be set. cyberdart detects this and adjusts:
...
For OpenClaw users: tell your OpenClaw agent "install cyberdart for openclaw."

The agent should:
1. Install cyberdart-lite CLAUDE.md into its coding session templates
2. Install the 4 native methodology skills
3. Add dispatch routing to AGENTS.md
4. Verify with a test spawn

For cyberdart developers: `./setup --host openclaw` outputs this documentation.
...
- No SOUL.md from cyberdart (OpenClaw has its own)
