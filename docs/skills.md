...
Detailed guides for every cyberdart skill — philosophy, workflow, and examples.
...
| [`/learn`](#learn) | **Memory** | Manage what cyberdart learned across sessions. Review, search, prune, and export project-specific patterns and preferences. |
...
Both modes end with a design doc written to `~/.cyberdart/projects/` — and that doc feeds directly into `/plan-ceo-review` and `/plan-eng-review`. The full lifecycle is now: `office-hours → plan → implement → review → QA → ship → retro`.
...
Visions and decisions are persisted to `~/.cyberdart/projects/` so they survive beyond the conversation. Exceptional visions can be promoted to `docs/designs/` in your repo for the team.
...
Eng Review is the only required gate (disable with `cyberdart-config set skip_eng_review true`). CEO and Design are informational — recommended for product and UI changes respectively.

### Plan-to-QA flow

When `/plan-eng-review` finishes the test review section, it writes a test plan artifact to `~/.cyberdart/projects/`. When you later run `/qa`, it picks up that test plan automatically — your engineering review feeds directly into QA testing with no manual copy-paste.
...
Claude: [Runs full 80-item visual audit on the live site]
        ...
        [Report with before/after screenshots saved to .cyberdart/design-reports/]
```
...
6. The approved variant saves to `~/.cyberdart/projects/$SLUG/designs/` with an `approved.json`
...
Claude: Approved Variant A. Saved to ~/.cyberdart/projects/myapp/designs/
...
Claude: Saved to ~/.cyberdart/projects/myapp/designs/hero-20260330/finalized.html
```
...
Claude: [Explores 12 pages, fills 3 forms, tests 2 flows]
        ...
        [Full report with screenshots saved to .cyberdart/qa-reports/]
```
...
Keep cyberdart current with one command. It detects your install type (global at `~/.claude/skills/cyberdart` vs vendored in your project at `.claude/skills/cyberdart`), runs the upgrade, syncs both copies if you have dual installs, and shows you what changed.

```
You:   /cyberdart-upgrade
...
Set `auto_upgrade: true` in `~/.cyberdart/config.yaml` to skip the prompt entirely — cyberdart upgrades silently at the start of each session when a new version is available.
...
Install Greptile on your GitHub repo at [greptile.com](https://greptile.com) — it takes about 30 seconds. Once it's reviewing your PRs, cyberdart picks up its comments automatically. No additional configuration.
...
cyberdart solves this. `/review` and `/ship` are now Greptile-aware. They read Greptile's comments, classify each one, and take action:
...
Every false positive you confirm gets saved to `~/.cyberdart/greptile-history.md`. Future runs auto-skip known FP patterns for your codebase. And `/retro` tracks Greptile's batting average over time — so you can see whether the signal-to-noise ratio is improving.
